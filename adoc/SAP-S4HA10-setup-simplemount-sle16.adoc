...
version 3. New features like SAP Rolling Kernel Switch (RKS) and migration of
ASCS are only supported with this new version.
The package`{s4sClConnector3}` with version 3.1.x supports in addition the maintenance
mode of cluster resources triggered from SAP tools.
...
////////////////////////////////////
TODO PRIO1: We need to adapt this section, if the new enqueue2 is published.
////////////////////////////////////

With the latest version of ENSA2, the ASCS instance can be started on the same host.
The ASCS instance receives the enqueue lock table over the network from the ERS instance.
If no other node is available, the ASCS instance will be started on the same host where
the ERS instance is running.
