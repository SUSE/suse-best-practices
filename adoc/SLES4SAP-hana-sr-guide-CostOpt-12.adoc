...
// TODO PRIO1: CostOpt12 - feedback BS - SAP allows multiple non-replicated systems on the secondary site - should we also allow this (SAP doku chapter 4.3)?
// TODO PRIO1: CostOpt12 - feedback BS - SAP allows productive non-replicated systems on the secondary site - should we also allow this?
...
For an overview about supported scenarios see also section <<Scale-Up Scenarios and Resource Agents>>.
...
While {SAPHANA} is only available on the Linux platform this triggers
...
// TODO PRIO1: CostOpt - check if variants match CostOpt (supported scenarios)
// TODO PRIO1: CostOpt - limit CostOpt *NOT* to support Multi-SID
// TODO PRIO2: CostOpt - does read-enabled makes sense?
...
- public cloud deployment (usualy needs additional documentaion focussing on the cloud specific implementation details)
...
* *Performance optimized* (_A => B_).  This scenario and setup is described in another document available from the documentation Web page ({reslibrary}). The document for _performance optimized_ is named _"{docPerfOpt}"_.
+
...
It is mentioned here to give an overview of our portfolio of solutions.
...
// TODO PRIO1: CostOpt12 - Still only non-replicated databases on the secondary side?
...
In case of failure of the primary {SAPHANA} on the primary site the cluster first tries to restart
...
Alternatively you could also configure a different resource handling procedure, but we recommend
to try to restart {SAPHANA} locally first, as a takeover with non preloaded tables could
consume much time and also the needed graceful stop of the non-replicated system will take additional time. Thus
in many environments the local restart will be faster.

To achieve an automation of this resource handling process, we can utilize the {SAPHANA}
...
So while `SAPHana` and `SAPHanaTopology` are driving the automation of the {SAPHANA} system replication,
...
user secure store keys setup procedure. See also our blog "SAP HANA Cost-optimized – An alternative Route is available" available at https://suse.com/c/sap-hana-cost-optimized-an-alternative-route-is-available/

The automated shutdown of the non-replicated {SAPHANA} database (e.g. QAS) is achieved by cluster
rules. Concrete it is an anti-colocation of {SAPHANA} promoted vs. {SAPHANA} non-replicated. This means
...
The takeover to the secondary site takes a long period of time, because the non-replicated database needs to
be stopped gracefully prior to takeover the productive database. This prolonged takeover time
is the main disadvantage of the cost optimized scenario. So, the cost optimized scenario might be combined with persistent memory to benefit from SAP HANA’s persistent memory features.

NOTE: If you like to achieve a very fast takeover you need to go with the performance optimized scenario.

Beside the description of the concept in this best practice, please also read the corresponding
{SAP} documentation like "Using Secondary Servers for Non-Productive systems". The
section is available e.g. for {HANA} 2.0 SPS05 at https://help.sap.com/viewer/6b94445c94ae495c83a19646e7c3fd56/2.0.05/en-US/5447545b91a04cf8a0d6133a026f2be5.html.
...
If automated registration is activated, the cluster will also automatically register
a former failed primary to become the new secondary.

// TODO PRIO1: CostOpt12 - Check the limitation of "manual migrations", also if we
...
// TODO PRIO1: CostOpt12 - Check supported scenarios and prerequisites
...
** Any STONITH mechanism supported by SUSE Linux Enterprise {prodnr} High Availability Extension (like SBD, IPMI)
is supported with SAPHanaSR.
// TODO PRIO2: Is stonith-ssh a supported scenario?
...
{HANA} 2.0 SPS04 and later do support persistent memory.
...
For informaton on supported hardware and virtualisation see {SUSE} release notes and hardware compatibility database
 https://www.suse.com/releasenotes/
 https://www.suse.com/yessearch/

See also {HANA} product availability matrix, e.g. by following
...
// TODO PRIO1: CostOpt12 - Should we mention the 'QAS'-DB in the Database installation title like
...
tools and services. Refer to {saphana} TDI documentation for allowed storage
...
- 1 VM with 32GB RAM, 50GB disk space for the system
- 1 VM with 48GB RAM, 50GB disk space for the system
- 1 shared disk for SBD with 10 MB disk space
- 2 data disks (one per site) with a capacity of each 96GB for {saphana}
- 1 data disk (for the non-replicated database) with a capacity of 96GB for {saphana}
...
// TODO PRIO1: CostOpt12 - add 'QAS' system to the parameter sheet
...
// TODO PRIO1: CostOpt12 - add 'QAS' DB system
...
. Verify that both databases (of the system repliated pair) are up and all processes of these databases
...
// TODO PRIO1: Describe the installation of the non-replicated database with memory limits

* Stop secondary (unlimitted database)
* Install non-replicated {HANA} with memeory limits
* Stop the non-repliated {HANA}
...
The key files need to copied-over from the primary to the secondary
site.
...
==========
Add the memoty limits to the global.ini. Again SUSE could not provide a sizing guide here. {HANA} sizing needs to be done ...
// TODO PRIO1: Add sizing refrence here
[subs="specialchars,attributes,quotes"]
----
[memorymanager]
global_allocation_limit = <size_in_mb_for_secondary>
----
==========

.Switch table pre-load to off
==========
To allow the {HANA} secondary with less memory we need to switch-off table pre-load.
[subs="attributes,quotes"]
----
[system_replication]
...
// TODO PRIO1: CostOpt12 - Installation of SAP HANA HOOK for the SR secondary
// TODO PRIO1: CostOpt12 - Limit the SR secondary (memory consumption and table-preload)
// TODO PRIO1: CostOpt12 - Installation of the 'QAS' system
// TODO PRIO1: CostOpt12 - Limit the 'QAS' system (memory consumption) to allow tiny secondary in parallel
...
// TODO PRIO1: CostOpt12 - Check the following procedure - still correct steps and order?
...
We implement two {hana} HA/DR provider hooks. One hook is named SAPHanaSR and shipped with the SAPHanaSR
package. The other is named srTakeover and needs to be adopted to your database according to
your specific username/password and connection.

This steps in this section must be done on both sites. {SAPHANA} must be stopped to change the
...
- Integrate the hooks into global.ini ({saphana} needs to be stopped for doing that offline)
...
{mySite1FirstNode}~ # cp /usr/share/SAPHanaSR/SAPHanaSR.py /hana/shared/myHooks
...
* _logreplay_
* (_logreplay_readaccess_ does not make sense for the cost optimized scenario)
...
=== Implementing Hook for srPostTakeover

The parameters added to global.ini imply that there should be a srTakeover hook installed on
the second node. We provide a sample code which needs to be adapted for your environment.
Currently you need to provide a user name / password combination inside the hook. We are
trying to improve that in the future together with SAP at the LinuxLab.

The srTakeover-Hook is based on sample code of {SAP} as well as the hook provided by DELL
in SAP Note 2196941.

// TODO PRIO1: Check for the bugzilla entry regarding CostOpt12
// TODO PRIO2: Check best place to install the hook
...
// TODO PRIO2: Dont quote the complete hook, once it has been added to the package (as example)
// TODO PRIO1: should srTakeover be names srCostOptMemConfig?

NOTE: This hook is given "as-it-is". It must be installed at node 2 as
/hana/shared/srHook/srTakeover.py to undo the changes to global_allocation_limit and preload_column_tables in case
of a takeover.
...
"""
Sample for a HA/DR hook provider for method srPreTakeover().
...
In the same directory (/hana/shared/srHook) you need to install (link) some python files from the
{hana} client software to enable the hook to run the database connect and SQL queries.

You need the following files: dbapi.py, __init__.py, resultrow.py

It is recommended to use symlink to have those python libraries available. Using symlink the files are
always up to date, if you update {hana}.
...
After changing the global.ini on node 2 and implementing the srTakeover hook you should start
...
This chapter describes the configuration of the cluster software {sleha}, which is part of the {sles4sap}, and {hana} Database
integration.
...
// TODO PRIO1: From ENSA2 take the -u / -S ... optiona and skip UDP reconfiguration
...
In your system the file might have additional parameters not discussed here.
...
configured in _/etc/sysconfig/sbd_, e.g. by calling `cs_show_sbd_devices`.
...
Or just call `cs_show_sbd_devices` again.
Next check if all nodes have joined the cluster.
To do so call `crm_mon`. We use the option _"-r"_ to also see resources, which are
...
// DONE PRIO1: CostOpt12 - Adopt parameters e.g. migration-threshold="3" for CostOpt
// TODO PRIO1: Discuss migration-threshold="3"
...
The next configuration part defines an SBD disk STONITH resource with a appropriate
_pcmk_delay_max_.
...
For an advanced SBD setup please refer to the {sleha} product documentation ({haAdminGuide12}).

For an advanced SBD setup please refer to the SLE-HA product documentation, e.g.
...
split-brain, look for the "Predictable Static Delays" configuration example.
See also <<Example for Determenistic SBD STONITH>>.

For fencing with IPMI/ILO see section <<Using IPMI as fencing mechanism>>.

==== Using IPMI as fencing mechanism

This section is only relevant, if the recommended disk-based or disk-less SBD
fencing is not used.
...
// TODO PRIO1: Discuss naming scheme SAPHanaTop SAPHanaCon vs old scheme
...
This step is to define resource needed, to control the replicated {HANA} pair.
...
// TODO PRIO1: CostOpt12 - Adop SAPHana parameters e.g. PREFER_SITE_TAKEOVER="false"
...
promote, monitors, stop) are typical tuneables. The parameter _AUTOMATED_REGISTER_
can used to adapt the level of system replication automation.

==== The virtual IP address for The Primary Site

The last resource to be added for SAPHanaSR is covering the virtual IP
address.
...
In most on-premise installations, only the parameter ip needs to be set to the
...
// TODO PRIO1: CostOpt12 - Check colocation score 2000 (PerfOpt) vs 3000 (CostOpt)
...
==== Add the cluster resource for the non-replicated {SAPHANA} database

// TODO PRIO1: CostOpt12 - Check the resource priories (of all resources)
// TODO PRIO1: CostOpt12 - Use variables as placeholders

For the non-replicated {SAPHANA} database a new resource is added to the cluster.
In former documents we used the resource agent `SAPDatabase` to control that
...
communicate with the cluster. This configuration is to complex and error prone.
`SAPInstance` uses the sapstartsrv API to do the work. This should solve the issue.
The new concept has already been published a {SUSE} towardsZeroDoentime blog at:
https://suse.com/c/sap-hana-cost-optimized-an-alternative-route-is-available/

// DONE PRIO1: Lars asked: is the hostname part of the instance profile name or not?
...
==== Adding the cluster rules for the automatic shutdown of the non-replicated {SAPHANA}

// We refer to the cluster nodes as suse01 and suse02 respectively:
// TODO PRIO1: CostOpt12 - Use variables as placeholders
In the following again {sapnode1} and {sapnode2} are used as the two active cluster nodes.
...
// TODO PRIO1: CostOpt12 - CostOpt should be running without Read-Enabled as this is resource counter productive
...
// TODO PRIO1: CostOpt - Configure resources needed for 'QAS' system (IP + SAPInstance(!))
//             Constraints, ban QAS on node1 and so on
...
// TODO PRIO1: CostOpt12 - tests to be adopted (e.g. local recover vs takeover)
...
nodes are booted, normal members of the cluster and the HANA RDBMS is running.
...
See also manual pages `SAPHanaSR-showAttr(8)`, `crm_mon(8)`, `crm(8)`, `cs_clusterstate(8)`,
...
`PREFER_SITE_TAKEOVER="false"` and `AUTOMATED_REGISTER="false".`
...
.{testExpect}
 * Primary restarts on site A (PREFER_SITE_TAKEOVER=false) till failcount >= migration-threshold
 * If takeover occurs
...
.{testExpect}
 * Primary restarts on site B (PREFER_SITE_TAKEOVER=false) till failcount >= migration-threshold
...
.{testRecover}
 -  Recovery after takeover:
. Optionally cleanup sbd slot for node 1
. Start cluster framework on node 1
. Wait till node 1 joins the cluster
. Register site A to site B
==========
...
.{testRecover}
 - Recovery after takeover:
. Optionally cleanup sbd slot for node 2
. Start cluster framework on node 2
. Wait till node 2 joins the cluster
. Register site B to site A
==========
...
.{testRecover}
. cleanup non-replicated database resource
==========
...
.{testDescr}
 - Pull LAN port down or block SR packages. Corosync network still available
...
. Wait till System replication status is _SOK_ again
==========

==== Test Maintenance Procedures

Please also test the maintenance procedures mentioned in section <<Maintenance>>.

==== Additional Generic Cluster Tests should be performed
...
In the following test descriptions we assume
`PREFER_SITE_TAKEOVER="false"` and `AUTOMATED_REGISTER="true".`
 ...
 * Non-replicated database is stopped on node 2 (site B)
 * Secondary database is promoted as primary
 * Later, once node 1 joins the cluster again
 ** SiteA is registered to SiteB
 ...
 * Cluster takeover to site A
 * Non-replicated database not available (no takeover to site A)
 * Later, once node 2 joins the cluster again
...
==== Test: Repeat previous test till Primary migrates
...
// TODO PRIO1: CostOpt12 - Check for needed adoptions
...
* Start with PREFER_SITE_TAKEOVER=”false”, AUTOMATED_REGISTER=”false” and
...
location rules mentioned in this setup guide are allowed. For public cloud please refer to the cloud specific documentation.
* As "migrating" or "moving" resources in crm-shell, HAWK or other tools
would add client-prefer location rules, support is limited to maintenance procedures described in this document. See <<Testing the Cluster>> and
...
You can use an web browser to check the cluster status.
...
In case you have installed the ClusterTools2 package, you might also look
into manual pages cs_clusterstate(8) and cs_show_hana_info(8).
...
// TODO PRIO1: CostOpt - Check if this is still usable for CostOpt
...
More information is available at respective websites, e.g.
...
of {sle} {ha} Extension Administration Guide chapter _Upgrading Your Cluster and Updating Software Packages_.
...
----
+
* Before proceeding, make very sure the primary HANA database is stopped.
...
Post Migrate::
* Wait some time till `SAPHanaSR-showAttr` shows both {SAPHANA} database
...
There two channels available for opening support requests. For issues which might also need {SAP} to
investigate the preferred method is to open an {SAP} ticket on support queue BC-OP-LNX-SUSE. See {SAP} note
1056161, link see <<SAP Notes>>.

The other channel is to use SUSE support only. {SUSE} customer center (SCC) is the central access point for managing support entitlements.
It also is one of the channels for opening support requests and is available at https://scc.suse.com/login.

More information on accessing support can be foud at https://www.suse.com/support/ and https://www.suse.com/support/faq/.

The {sles4sap} product documentation lines out how to collect information usually needed during a support request,
https://documentation.suse.com/sles/12-SP4/html/SLES-all/cha-adm-support.html.  See also manual pages crm_report(8), supportconfg(8), cs_show_hana_info(8),
ha_related_suse_tids(7).

There are also {SUSE} support TIDs, e.g.
...
The SUSE support technical information database (TID) is available at https://www.suse.com/support/kb/.
...
Blog about SAPInstance instead of SAPDatabase::
 https://suse.com/c/sap-hana-cost-optimized-an-alternative-route-is-available/
...
// TODO PRIO1: CostOpt - exchange this example by a real cost-opt implementation
...
// TODO PRIO1: 12 SP4: property $id="cib-bootstrap-options" \ ?
// TODO PRIO1: 12 SP4: rsc_defaults $id="rsc-options" \ ?
// TODO PRIO1: 12 SP4: op_defaults $id="op-options" \ ?
...
==== Example for Determenistic SBD STONITH

This SBD resources make sure suse01 will win in case of split-brain.
