...
NOTE: The setup with cluster-controlled file system resources has been obsoleted by the
so-called simple-mount setup. *For deploying new HA clusters, please use the simple-mount setup, described in
...
- The described concept differs from the old stack that used a multi-state architecture.
...
NOTE: Version for {sapS4pl} on Linux on AMD64/Intel 64 and PowerLE. More information about
...
////
TODO PRIO3: we used a non shared disk. Forgot to mount during installation?!?!?!
////
...
////
TODO PRIO3: multinode cluster extension
...
See also manual page mount.nfs(8).
==============================================
...
----
////
TODO PRIO3: one partition will be mounted mkdir must be run again after mount
////
...
=== Installing the database on {myNode1}
...
=== Installing the primary application server (PAS) on {myNode1}
...
////////////////////////////////////
TODO PRIO3: We need to adap this section, if the new enqueue2 is published.
////////////////////////////////////
...
----
////
TODO PRIO3: for 2+ node cluster different SBD settings
...
----
See also manual pages ocf_heartbeat_SAPInstance(7), ocf_heartbeat_IPaddr2(7) and ocf_heartbeat_Filesystem(7).
================================================
...
See also manual pages sap_suse_cluster_connector(8), cs_clusterstate(8), crm(8) and crm_mon(8).
...
// TODO PRIO3: use resource maintenance instead of cluster maintenance
...
==== Testing ASCS migration after operating system failure
...
==== Restarting ASCS in-place using _Stop_ and _Start_
...
////
TODO PRIO3: Is the statement still valid? I guess no.
...
. Prepare the OS and install the cluster software.
. Prepare watchdog and SBD parameters for diskless SBD.
...
This includes the setup of NTP as well as installing the software pattern *ha_sles* using *YaST* or *zypper*.
...
. Back up the current cluster.
. Install the OS on the new node.
. Patch the existing nodes.
. Prepare the new node's operating system.
. Prepare SAP.
. Install the cluster software on the new node.
. Joining the cluster.
...
- Set up *chrony* (this is best done with *YaST*) and enable it:
+
[subs="specialchars,attributes,quotes"]
----
# yast ntp-client
...
# /usr/sap/hostctrl/exe/sapstartsrv pf=/usr/sap/<SID>/SYS/profile/<SID>_ERS<instanceNumberErs>_<virtHostNameErs
> -reg
# /usr/sap/hostctrl/exe/sapstartsrv pf=/usr/sap/<SID>/SYS/profile/<SID>_ASCS<instanceNumberAscs>_<virtHostNameA
scs> -reg
----
+
NOTE: This must be done for each instance. Call sapstartsrv with parameters *pf=*<profile-of-the-sap-instance>
and *-reg*.
+
...
In multi node cluster you will find additional node entries like *node 3: {my3nd3}*.
...
//   - Format corrections
// REVISION 1.2 2021/12
//   - pointer to simple-mount guide
// REVISION 1.2 2021/02
