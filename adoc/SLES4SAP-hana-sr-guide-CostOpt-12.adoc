...
** The cluster automates one single takeover in case of failed primary.
After that happend, the initial state has to be restored by administrative
...
* Register former primary as new secondary as user _<sid>adm_
+
[subs="specialchars,attributes,quotes"]
----
...
* Cleanup failcount for {HANA} resource as user root
+
[subs="specialchars,attributes,quotes"]
----
...
* Recover {HANA} global.ini back to initial state, as user _<sid>adm_
+
[subs="specialchars,attributes,quotes"]
----
{sapnode2} > cdcoc
...
* Move {HANA} Primary back to {sapnode1}, as root user 
+
[subs="specialchars,attributes,quotes"]
# crm resource move <multi-state-resource-name> force
----
Wait until the cluster has finihed the transition and is idle.
