...
//
= {SAPHana} System Replication Scale-Out High Availability in AWS
...
for {saphana} Scale-Out system replication with automated failover in the Amazon Web Services Cloud.
...
one {saphana} Scale-Out cluster to a secondary {saphana} cluster in different AWS Availability Zone.
...
descisions based on the implemented algorythms.
...
This document describes how to set up a {saphana} Scale-Out
...
image::images/SAPHanaSR-ScaleOut-Plan-Phase0.svg[scaledwidth="100%"]
...
image::images/SAPHanaSR-ScaleOut-Cluster.svg[scaledwidth="100%"]
...
.Planning <<OsSetup>> <<SAPHanaInst>> <<SAPHanaHsr>> <<Integration>> <<Cluster>> <<Testing>>
image::images/SAPHanaSR-ScaleOut-Plan-Phase1.svg[scaledwidth="100%"]

What you need before you start:

- EC2 instances created using _"{sles4sap} 12 SP4"_ , _"{sles4sap} 15"_ or later created from an Amazon Machine Image (AMI). If using a Bring Your Own Subscription (BYOS) AMI a valid SUSE subscription is required.
...
This section defines minimum requirements to install {saphana} Scale-Out and create a cluster in AWS.

NOTE: The minimum requirements mentioned here do not include SAP sizing information. For sizing information please use official SAP sizing tools and services.

NOTE: The preferred method to deploy SAP HANA scale-out clusters in AWS is to use AWS QuickStarts using "single Availability Zone (AZ) and multi-node architecture" deployment option. If you're installing SAP HANA Scale-Out manually, please refer to link:https://docs.aws.amazon.com/sap/latest/sap-hana/welcome.html[AWS SAP HANA Guides] documentation for detailed installation instructions, including recommended storage configuration and filesystems.

.Simplified cluster architecture for {saphana} Scale-Out system replication across two Availability Zones
image::images/ScaleOutCluster-AWS.png[scaledwidth="100%"]

As example, this guide will go through the implementation of a SUSE cluster composed of two 3-node SAP HANA Scale-Out cluster (one per Availability Zone), plus the a Majority Maker (mm) node. But apart from the number of EC2 instances, the other requirements should be the same for different number of SAP HANA nodes:
...
Planning the cluster implementation can be very complex, and we recommend the installation
to be planned properly and it is recommended to have all required parameters already in place before starting the deployment. It is a good practice to first fill-out
...
|EFS filesystem AZ-a (/hana/shared) |
|EFS filesystem AZ-b (/hana/shared) |
|Watchdog driver |
|Placement Group Name |
|=========================================================

=== SAP Scale-Out Scenario in AWS with SUSE High Availability

SAP HANA Scale-Out cluster in AWS requires the use of a majority maker node in a 3rd Availability Zone. A majority maker, also known as tie-breaker node, will ensure cluster quorum is maintained in case of the loss of one Availability Zone. In AWS, in order for the Scale-Out cluster to maintain functionality, at least all nodes in one Availability Zone plus the majority maker node needs to be running, otherwise cluster quorum will be lost and any remaining SAP HANA node will be automatically rebooted.

It is also recommended that each set of cluster nodes have its own link:https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/placement-groups.html[EC2 placement group] using "cluster" mode. This is required to ensure that nodes can achieve the low-latency and high throughput network performance needed for node-to-node communication that is required by SAP HANA in an Scale-Out deployment.

To automate the failover, SUSE High Availability Extension (HAE) built into 
_{sles4sap}_ is used, and it includes two resource agents which have been created to support
SAP HANA Scale-Out High Availability.

The first is *SAPHanaController* resource agent (RA), which checks and
...
The second resource agent is *SAPHanaTopology* and this RA has been created to make configuring the cluster
as simple as possible.  It runs on all SAP HANA nodes (except the majority maker) of a SLES HAE 12 cluster. It gathers information about status and
...
image::images/SAPHanaSR-ScaleOut-Cluster-Resources02-AWS.svg[scaledwidth="100%"]

{SAPHANA} system replication for Scale-Out is supported in the following
scenarios or use cases:

Performance optimized, single container (A > B)::
In the performance optimized scenario a {saphana} RDBMS on site "A" is synchronizing with a
...
In case of failure of the primary {saphana} Scale-Put cluster on AZ-a SUSE's HAE tries to
start the takeover process. This allows to use the already loaded data at the SAP HANA Scale-Out located in 
AZ-b and typically the takeover is much faster than the local restart.

A site is noticed as "down" or "on error", if the *LandscapeHostConfiguration status* 
reflects this (return code 1). This happens when worker nodes are going down without any local SAP HANA
standby nodes left.
...
* Since HANA primary and secondary reside in different link:https://aws.amazon.com/about-aws/global-infrastructure/regions_az/[Availability Zones (AZs)] an link:https://docs.aws.amazon.com/es_es/sap/latest/sap-hana/sap-ha-overlay-ip.html[Overlay IP] addresses is required
* Linux users and groups, such as _{refsidadm}_ are defined *locally* in the
...
* EC2 instances must have different hostnames
...
* You need at least SAPHanaSR-ScaleOut version 0.161, _"{sles4sap} 12 SP4"_ or _"{sles4sap} 15"_ and
  {SAPHANA} 1.0 SPS12 (122) or {SAPHANA} 2.0 SPS03 for all mentioned setups. Please, refer to SAP Note 2235581.

IMPORTANT: You must implement a valid STONITH method. Without a valid STONITH
method, the complete cluster is unsupported and will not work properly. For this guide
...
a Proof-of-Concept (PoC) with {SUSE} and AWS. This PoC will focus on testing the existing
solution in your scenario.

== Using AWS with SUSE High Availability Extension Clusters

SUSE High Availability Extension (HAE) cluster will be installed in an AWS Region. An AWS Region
consists of multiple Availability Zones (AZs). An Availability Zone (AZ) is one or more discrete data 
centers with redundant power, networking, and connectivity in an AWS Region. AZ’s give customers 
the ability to operate production applications and databases that are more highly available, fault 
tolerant, and scalable than would be possible from a single data center. All AZ’s in an AWS Region 
are interconnected with high-bandwidth, low-latency networking, over fully redundant, dedicated metro 
fiber providing high-throughput, low-latency networking between AZ’s. All traffic between AZ’s is 
encrypted. The network performance is sufficient to accomplish synchronous replication between AZ’s. 
AZ’s are physically separated by a meaningful distance, many kilometers, from any other AZ, although 
...
-	Use a routing table attached to the the subnets
...
This is an specific routing entry which can send network traffic to an instance,
...
On premises users like HANA Studio will not be able to reach this IP address
...
Here are the prerequisites which have to be met before starting the installation in AWS:
...
NOTE: Using AWS SAP HANA QuickStart will automatically deploy all the required AWS resources listed above, and is the
...
Please, note that this section lists the ports which need to be available for the SUSE cluster only and it doesn't include SAP related ports.

=== Placement Group

One cluster placement group per Availability Zone is required to ensure that SAP HANA nodes will achieve high network throughput required by SAP HANA. For more information about placement groups refer to AWS documentation:
* https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/placement-groups.html

=== AWS EC2 Instance Creation

Create all EC2 instances to configure the SLE HAE cluster. The EC2 instances will be located in 3 different Availability Zones to make them independent of each other.
...
* Use a "SLES for SAP" AMI. Search for "suse-sles-sap-12-sp4" or "suse-sles-sap-15" in
...
Register your system with SUSE SMT, SUSE Manager or directly with SUSE customer center!

* Use the AWS Marketplace AMI
_SUSE Linux Enterprise Server for SAP Applications 15_
which already includes the SUSE subscription and HAE software components.
...
By default, the EC2 instances will have automatically generated hostnames, but it is recommended to assign hostnames that comply with SAP requirements, see SAP note 611361.
You'll have to edit /etc/cloud/cloud.cfg for hostnames to persist:
----
preserve_hostname: true
----

NOTE: To learn how to change the default hostname for an EC2 instance running SUSE Linux, please refer to the AWS' public documentation:
...
the _root_ user on all instances. SUSE cluster resource agents require a profile
...
keys in these profiles. Just leave these fields blank and attach an EC2 IAM profile
...
they need HTTP/HTTPS access to AWS API endopoints. Systems which don't offer
transparent Internet access may require a HTTP/HTTPS proxy.
...
SUSE HAE also requires to add the proxy configurations to _/etc/sysconfig/pacemaker_ configuration file in the following format:
...
Make sure that the SUSE instance is able to reach the EC2 instance metadadata address URL http://169.254.169.254/latest/meta-data as multiple system components will required to access it. Therefore it is recommended to disable any firewall rules that restricts it, and to disable proxy access to this URL.

For more information about EC2 Instance metadata server, please refer to AWS' documentation:
* https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-metadata.html

=== Disable the Source/Destination Check for the Cluster Instances

The source/destination check has to be disabled on all EC2 instances that are part of the cluster. This can be done
...
image::images/SourceDestinationCheck.png[PNG]

=== Avoid Deletion of Overlay IP Address on the eth0 Interface

SUSE's cloud-netconfig-ec2 package may errouneaously remove any secondary IP address which is managed by the cluster
agents from the eth0 interface. This can cause service interruptions for users of the cluster service. Perform the following task on all cluster nodes.
...
SLES cluster software and its agents needs a number of AWS IAM privileges to operate the cluster, and an IAM Security Role is required to be attached to the EC2 instance that are part of the cluster. A single IAM Role can be used across the cluster, and associated to all EC2 instances. 

This IAM Security Role will require the following IAM Security Policies.

==== AWS Data Provider Policy
SAP systems on AWS require installation of the “AWS Data Provider for SAP”, which needs a policy to access AWS resources.
Use the policy shown in the
link:https://docs.aws.amazon.com/sap/latest/general/aws-data-provider.html[“AWS Data Provider for SAP Installation and Operations Guide“] and attach it to the IAM Security Role to be used by the cluster EC2 instance. This policy can be used by all SAP systems.
...
The Overlay IP address has to be outside of the CIDR range of the VPC.
Use the AWS console and search for “VPC”.

*	Select VPC
*	Click on “Route Tables” in the left column
*	Select route table used the subnets from one of your SAP EC2 instances
and their application servers
...
name to EC2 instance to be the SAP HANA Master. The resource agent will modify this later one automatically as required.
Save your changes by clicking on “Save”.

NOTE: Important: The routing table, which will contain the routing entry has to be
inherited to all subnets in the VPC which have consumers of the service.
Please check the AWS VPC documentation
(http://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/VPC_Introduction.html)
for more details on routing table inheritance.

=== Create EFS Filesystems

Each set of {sapHana} Scale-Out cluster needs to have its own Amazon Elastic Filesystem (EFS). To create an EFS filesystem
please review AWS public documentation which contains a step-by-step guide of how to create and mount it:
...
If using SLES for SAP 12:
- *1984787* SUSE LINUX Enterprise Server 12: Installation Notes
- *2205917* SAP HANA DB: Recommended OS settings for SLES 12 / SLES for SAP Applications 12

If using SLES for SAP 15:
...
Please follow these instructions on each node if you have installed the systems based on the existing AWS Amazon Machine Images (AMIs), or deployed the SAP HANA nodes using the AWS QuickStart.
...
==== Install the latest available SUSE updates
...
This setup is based on only one SSH keypair which enables SSH access to all cluster nodes.
...
The table on this website lists the minimum required number of EBS volumes, volume size
as well as IOPS (for IO1) for your desired EC2 instance type. You can choose
...
/usr/sap::
This is the path to the local SAP system instance directories. It is recommeded to have a separate volume for /usr/sap.

=== Configure hostname resolution

To configure hostname resolution you can either use a DNS server or modify the _/etc/hosts_ on *all* cluster nodes.

With maintaining the _/etc/hosts_ file, you minimize the impact of a failing
DNS service. Edit the _/etc/hosts_ file on *all* cluster nodes and add all cluster nodes' hostname and IPs to it.
...
.<<Planning>> <<OsSetup>> SAPHanaInst <<SAPHanaHsr>> <<Integration>> <<Cluster>> <<Testing>>
image::images/SAPHanaSR-ScaleOut-Plan-Phase3.svg[scaledwidth="100%"]
...
Installation Guide. In order to do this install the HANA primary master server
...
----
This way the HANA database won't expect shared log/data directories across all nodes.
...
.<<Planning>> <<OsSetup>> <<SAPHanaInst>> SAPHanaHsr <<Integration>> <<Cluster>> <<Testing>>
image::images/SAPHanaSR-ScaleOut-Plan-Phase4.svg[scaledwidth="100%"]
...
.<<Planning>> <<OsSetup>> <<SAPHanaInst>> <<SAPHanaHsr>> Integration <<Cluster>> <<Testing>>
image::images/SAPHanaSR-ScaleOut-Plan-Phase5.svg[scaledwidth="100%"]
...
SUSE's SAPHanaSR-ScaleOut resource agent includes a SAP HANA integration script
...
srConnectionChanged() is called on the master nameserver as soon as one of the replicating 
...
.<<Planning>> <<OsSetup>> <<SAPHanaInst>> <<SAPHanaHsr>> <<Integration>> Cluster <<Testing>>
image::images/SAPHanaSR-ScaleOut-Plan-Phase6.svg[scaledwidth="100%"]
...
no additional action is required in order to use the hardware watchdog, and non-bare metal
...
If no watchdog is available (on virtualized EC2 instances) you can enable
the _softdog_ .

In order to enable softdog persistently across reboots execute the following step in *all* EC2 instances that are going to be part of the cluster (including the majority maker node)
...
After distributing the encryption key, create an initial _/etc/corosync/corosync.conf_ configuration using these parameters for cluster timings, transport protocol and encryption. Exchange the _bindnetaddr_ as well as the _ring0_addr_ (IPv4 addresses) of all cluster nodes in the nodelist according to your network topology.
...
{sleha} of the SUSE Linux Enterprise High Availability Administration Guide (see https://www.suse.com/documentation/sle-ha-12/singlehtml/book_sleha/book_sleha.html#cha.ha.manual_config ).
...
step-by-step to the cluster, the best way to avoid unexpected cluster reactions is to first set the
complete cluster to maintenance mode, then after all needed changes as last step, the cluster can be removed from maintenance mode.
...
not start this {SAPHANA} RDBMS till it is registered to avoid double primary up
situations.
...
Replace "<SID>" by SAP SID, "<Inst>" by SAP HANA instance number, and "<majority maker>" by the majority maker node hostname.
...
If maintenance mode has been enabled to configure the cluster then as last step it is required to remove the cluster from maintenence mode.

It may take several minutes for the cluster to estabilize as it may be required to start SAP HANA and other cluster services on the required nodes. 
...
.<<Planning>> <<OsSetup>> <<SAPHanaInst>> <<SAPHanaHsr>> <<Integration>> <<Cluster>> Testing
image::images/SAPHanaSR-ScaleOut-Plan-Phase7.svg[scaledwidth="100%"]
...
* Simulate a maintenance procedure with cluster contineously running.
...
image::images/cluster_status_hawk_1.png[scaledwidth="95%"]
...
image::images/hana_studio_landscape.png[scaledwidth="95%"]
