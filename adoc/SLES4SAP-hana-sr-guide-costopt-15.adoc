...
Another is hook is called by {SAPHANA} after an SR take-over has happened. This can be
...
. Create a database user key in {refsidadm}´s key store on fail-over site.
...
=== Create a database user key in {refsidadm}´s key store
...
NOTE: For security reasons on production systems always a dedicated user with
restricted permission needs to be created.
...
users, permissions and the user key store.
