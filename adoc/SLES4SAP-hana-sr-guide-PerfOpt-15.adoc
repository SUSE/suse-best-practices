...
** The SAP HANA site name is from 2 up to 32 characters long. It starts with a
...
Please refer to {sap} documentation for the {saphana} version.
...
Another hook is called by {SAPHANA} before an SR takeover is processed. This method can 
be used to block a manual takeover during normal cluster operation.
...
As user {refsidadm} check the {SAPHANA} tracefiles, if susTkOver.py did succesffuly blocked or permitted the takeover request.

----
 ~> cdtrace
# check for permitted takeover requests
grep susTkOver.preTakeover.*permit nameserver_*.trc
# check for blocked takeover requests
...
This parameter must match your storage environment. If you expect your storage could hang for 30s, then
