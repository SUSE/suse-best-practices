...
= SAP NetWeaver High Availability Cluster 7.40 for the AWS Cloud - Setup Guide for SLES for SAP Applications 15
...
NOTE: If there is a error message like this:
Clean up environment.
Environment cleaned up.
Error: Fail to connect to "PRIM" site SAP ASE at "<hostname>:4901".  Please check the host name/port number are correct and server is running.
...
NOTE: In some circumstances the installation is not successful. It could help to set up the primary system again and install the companion afterwards.

NOTE: If the system is reinstalled and the companion system reports *Missing read/write permissions* for this folder _/tmp/.SQLAnywhere_ check the permission on both node. After a change for the ownership was needed, run the setup again on both nodes. Start with the *Primary*.
...
NOTE: In case of a reinstallation it might be better to overwrite the existing username and password in the secure store for the _sapadm_ and _DR_admin_ if the old values are not 100% known.
...
The application server environment should be adapted in case the DB has been switch (takeover). On each host which is providing a dialog server (PAS; AAS) the _.dbenv.sh_ and or _.dbenv.csh_ file has to extended.
...
==========
Add the missing value and extend the settings as below shown on each host who runs an dialog application server.
...
* starting PAS and AAS instancen
* optional: release cluster maintenance mode, if the SAP system was started manually
** filesystem must be mounted and IP must be set manually
...
* stopping PAS and AAS instancen
...
Important for each High Availablity solution is a extensive testing procedure. That takes care that the solution is working as expected in case of a failure.
...
Killing the FM process more 5 times will bring pacemaker in action. Up to 5 time the saphostagent will take care for the SAP process. If this failcount is reached in a specific time windows the service will not be restart.
