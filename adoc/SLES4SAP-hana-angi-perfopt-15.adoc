...
site must have been connected to the secondary (chain topology). With current {HANA}
...
* Colocation constraints between the {SAPHanaRA} RA and other resources are
...
constraints. Thus it is not allowed to define rules forcing an {SAPHanaRA}
...
* If _susChkSrv.py_ parameter action_on_lost=stop is set and the RA {SAPHanaRA}
...
The {HANA} database instance on the secondary site must be stopped
...
/hana/shared/<SID>. The RA just monitors the file system, but neither does mount nor
...
Additional information about all parameters of the {SAPHanaRA} RA can be
found in manual page ocf_suse_{SAPHanaRA}(7).

[subs="attributes,quotes"]
----
# vi crm-saphanacon.txt
# enter the following to crm-saphanacon.txt
primitive rsc_SAPHanaCon_{sapsid}_HDB{sapino} ocf:suse:{SAPHanaRA} \
...
resource agents {SAPHanaRA} and {SAPHanaTopRA} .
...
.  After some time the cluster shows the srHook of the stopped
...
* Tune the timeouts of operations of {SAPHanaRA} and {SAPHanaTopRA}.
...
------------------------------------------------------
global Mon May 08 09:10:11 2023 {sapsite1}  {sapsite2} {sapsid} ScaleUp
...
//      - fixed RA name
// REVISION 0.4 2026/04
//      - SAPHanaController ON_FAIL_ACTION=fence , stop timeout=600
//      - susChkSrv.py action_on_lost=fence
//
