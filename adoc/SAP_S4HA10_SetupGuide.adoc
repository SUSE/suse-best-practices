:docinfo:
= SAP S/4 HANA - Enque Replication 2 High Availability Cluster - Setup Guide
...
The described cluster solution could be used for {sapReg} {sapS4pl}.
...
availability cluster solution, e.g. as provided by {sles4sap}. The mentioned components have
proven their maturity over several years of productive operations for customers of different sizes
and branches.
...
You can also find numerous whitepapers, best-practices guides and other
...
We will publish and maintain in a higher frequency the TID for this document to fix very urgent small
things and deliver important information.
SAP S/4 HANA - Enque Replication 2 High Availability Cluster - Setup Guide - Errata:
(https://www.suse.com/support/kb/doc/?id=7023714)
Please check in addition to this guide the SUSE SAP Best Practice Guide Errata for other solutions.
...
- Plan a {sleHA} platform for {sapS4pl},
  including {sapERS}.
...
please follow the guides for the performance- or cost-optimized scenario. (see section "Additional
...
certification specifications and goals. In this Guide two nodes are used for the ASCS
central services instance and ERS replicated enqueue instance. This two nodes are controlled by
the SUSE HA cluster. A third node is used for running the database as well as PAS and AAS application
...
- For {s4Hana} the new concept also covers, that the ASCS does not need to be started at ERS
  side  after a resource failure. The new enqueue architecture is also named ENSA2.

=== Three Systems or more for ASCS, ERS, Database and additional SAP Instances

This guide describes the installation of a distributed {sap} system on three and more
...
cluster or by installing the database on either of the nodes. However we recommend to install the
database on a separate cluster. The cluster configuration for three and more node are described at the
...
Depending on your needs you could also increase the availability of the database, if your
database is not already high available by design.
...
An other option is to implement a second cluster for a database without SR aka
...
NOTE: Version for {sapS4pj} on LINUX on x86_64. More information about
supported combination of OS and Databases for {sapS4in} can be found at the
...
The new version of the {s4sClConnector3} now allows to start, stop and 'migrate'
...
cluster nodes {my2nd1} and {my2nd2} in the two node cluster example. Besides the partitions for the file systems
...
MD-Raid devices. This is out of the scope of this setup-guide.

After we have partitioned the shared disk on {my2nd1} we need to request a
...
During the SAP installation we need {myMpAscs} to be mounted on {my2nd1} and
{myMpErs} to be mounted on {my2nd2}.
////
...
# # we are on the 'correct' drive, right?
# parted -s {myDev} mklabel gpt
# parted -s {myDev} mkpart primary 1049k 160G
# parted -s {myDev} mkpart primary 160G 170G
# parted -s {myDev} mkpart primary 170G 180G
...
Check, if the /etc/hosts contains at least the following address resolutions.
...
We need to create the directory structure on all nodes which should be able to
...
({myNode1}) will be used to install the {sap} HANA database. Server 1
({myNode1}) will be used to install the PAS {sap} instance. Server 1
({myNode1}) will be used to install the AAS {sap} instances. Server 2
({my2nd1}) will be used to install the ASCS {sap} instances. Server 3
({my2nd2}) will be used to install the ERS {sap} instances.
...
The overall procedure to install the distributed SAP system is:
...
Take care for the file systems like {myDevPartAscs} and /var/lib/Landscape/ which might also need
to be mounted.
...
Ensure _/usr/sap/sapservices_ holds both entries (ASCS+ERS) on both cluster
...
[subs="attributes"]
----
Start_Program_00 = local $(_ENQR) pf=$(_PFL) NR=$(SCSID)
----
...
NOTE: The SBD device/partition need to be created beforehand. Double check which
device/partition to use! In this setup guide we already have reserved partition {myDevPartSbd}
for SBD usage.

.Tasks

. Setup chrony (best with yast2) and enable it
...
command line tool ha-cluster-init. The following example uses the command line wizard.

- Install and configure the watchdog device on the first machine

A hardware based watchdog device should be preferred used instead of the software based solution.
The following example uses the software device but can be easily adapt to the hardware device.
...
Some preparation steps on the second node.
...
You could use either YaST to configure the cluster base or use the interactive
command line tool ha-cluster-join. The following example uses the command line wizard.
...
We need a changed SAPInstance resource configuration for {sapS4} in order *not* to use
the Master-Slave construct anymore. We move to a more cluster like construct to
start and stop the ASCS and the ERS itself, but *not* the complete
master-slave.
...
With the new version of ENSA2 the ASCS could start on the same host and there is no longer a need
to follow the ERS. The ASCS receive the enqueue lock table from the ERS over the network. If no
other node is available the ASCS instance will be started on the same host where ERS is running.
...
Please verify the SBD cluster configuration and if needed please modify them as described.
...
The constraints between the ASCS and ERS instance are change compared to the ENSA1
configuration.
ASCS instance must not start-up on the cluster node running the ERS
instance after a failure (loc_sap_{mysid}_failover_to_ers). This constraint is
was needed in the past to ensure that the locks are not lost after a ASCS instance (or node)
failure. Today the ENSA2 setup is able to resync the lock table over the network.

If the ASCS instance has been started by the cluster on the ERS node. Then ERS instance should
be moved to an "other" cluster node (col_sap_{mysid}_no_both) if possible. This constraint
...
all two constraints to that file and load the configuration to the
...
NOTE: Before each test please verify that the cluster is in idle state. No migration constraints
are active. No resource failure messages are visible. Start each procedure with a clean setup.
...
{s4sClConnector3} to migrate the ASCS instance. As user _{mysapadm}_ you might call
...
With {sapCert} it is *not longer allowed to block resources* from beeing
  ...
  _/etc/sap_suse_cluster_connector_ is not allowed any more.
...
This set the ASCS and ERS cluster resource to unmanaged.
...
This set the ASCS and ERS cluster resource to managed.
...
This test will immediately hard-reboot the node.
...
This test should show that the cluster solution did not interact and try to restart the ASCS instance
during a maintenance procedure. And one the second hand that no locks are gone during the restart of
an ASCS instance during a RKS procedure. The cluster solution should recognize that the restart of
the ASCS instance was expected and no failure or error should be reported or counted.
Optional you can set some locks and verify if they still exist after this procedure. There are multiple
ways to do that. One example could be: login to your SAP system and open the transaction SU01 and start to
create a new user. With the SAP MC / MMC check if there are locks available. Open the ASCS instance
entry and go to Enqueue Locks. With the transaction SM12 you can also see the locks.
Please do this test multiple times in a short time frame. The restart of the ASCS instance in the
example below happens five times.
...
==== Do a Rolling Kernel Switch Procedure (RKS), if possible

The rolling kernel switch (RKS) is an automated procedure that enables the kernel in an ABAP system
to be exchanged without any system downtime. During a RKS, all instances of the system, and
generally all SAP start services (sapstartsrv) as well, are restarted.

- check note 953653 whether the new kernel patch is RKS compatible to your currently running kernel
- check also the SAP note 2077934 - Rolling kernel switch in HA environments
