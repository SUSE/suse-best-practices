...
Comprehensive security concepts usually pay high attention to database systems, since databases belong to one of the most critical components in any IT environment. Database systems that potentially store sensitive data are by nature very popular targets for hackers and must therefore be protected.
SAP HANA systems typically store business related information and are considered as being business critical. This is in particular the case for ERP systems using SAP HANA. In addition, many other SAP applications using SAP HANA, like BW systems, may store sensitive data.

Security for SAP HANA
~~~~~~~~~~~~~~~~~~~~~
SAP takes the security topic very seriously. For SAP HANA, there is a comprehensive security guide available. This guide describes in detail how to protect HANA from a database perspective. It can be accessed at http://help.sap.com/hana/SAP_HANA_Security_Guide_en.pdf. 
...
Security updates and patches:: SUSE constantly provides security updates and patches for their SUSE Linux Enterprise Server operating systems and guarantees highest security standards during the entire product life cycle.
...
Logging & reporting:: Changes the behavior of the system logging, syslog forwarding to a central syslog server, automatic creation of reports (such as security reports) and forwarding of security-relevant information via email
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Description::
Logfiles should be forwarded from an SAP HANA node to central *syslog* server. 
This prevents syslog files from being manipulated by an attacker. In addition, it allowing administrators to have a central view on the syslog files.
...
* This can be irritating for system administrators, but it also helps to prevent accidentally reboots.
...
^^^^^^^^^^^^^^^^^^^^^^
Description:
The `cron.allow` file specifies a whitelist of users that are allowed to execute jobs via the Linux cron system. 
...
^^^^^^^^^^^^^^^^^^^^
Description::
The `at.allow` files specifies a whitelist of users that are allowed to execute scheduled one-time running jobs, so-called 'at' jobs, via the Linux 'at' job execution system.
...
It does not provide options to set the password policy. All changes apply only to newly created accounts. To change existing accounts, use the `passwd` and `change` commands.
...
If your corporate policy requires to set up password failure counts for users, refer to the SUSE Linux Enterprise Server 15 Hardening Guide, section: 2.28.3 Locking User Accounts After Too Many Login Failures.
...
+
A new password must differ by at least 5 characters from the old password.
...
CAUTION: Do not use the `pam-config` utility here. It only supports `pam_access` as global module. 
...
The settings prevent certain kind of IP spoofing attacks, such as those used for DDoS attacks.
+
----
...
For configuring the HANA Firewall, follow the respective instructions detailed in the SUSE Linux Enterprise Server for SAP Applications Guide, section Configuring HANA-Firewall at https://www.suse.com/documentation/sles-for-sap-15/singlehtml/book_s4s/book_s4s.html#sec.s4s.configure.firewall-hana.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
As with commercial software, open source software is tested by hackers and security experts for vulnerabilities. 
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
To receive security updates (and other updated packages) on SAP HANA systems, the SUSE update channels must properly be configured. 
...
To verify that your HANA system has been properly configured to receive updates, check if it has been registered to the SUSE update channels:
----
zypper lr
----
...
Installing All New Updates Patches on a Regular Basis
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Description::
...
--------------------------------
The following table provides an overview of sources for further information about the topics discussed in this guide.
...
- Removed the folowing chapters (content was moved to the official Hardening Guide for SUSE Linux Enterprise Server):
