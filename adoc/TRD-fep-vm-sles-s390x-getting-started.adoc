...
SUSE Linux Enterprise Server is trusted by enterprises around the world for their mission-critical IT deployments. A multimodal operating system that paves the way for IT transformation in the software-defined era. The modern and modular OS helps simplify multimodal IT, makes traditional IT infrastructure efficient and provides an engaging platform for developers. As a result, organiztions can easily deploy and transition business-critical workloads across on-premise and public cloud environments.
...
IBM Z and LinuxONE systems are enterprise-class, purpose-built data serving platform for mission critical Linux business workloads that mandate the highest levels of security, scalability, performance and resilience. The platform supports traditional n-tier Linux applications and databases, as well as cloud-ready containerized workloads and microservices, delivering a true secure hybrid cloud platform for all organizations.


== High-level overview

This document will guide a administrator through the steps to deploy Fujitsu Enterprise Postgres (FEP) 12 on a SUSE Linux Enterprise Server (SLES) 12 SP5. SLES 12 SP5 is the version that supported by FEP12. The end result of these steps is FEP running on SLES with a test database.

While the steps in this chapter can be applied to installing FEP on SLES on any supported architecture (x86_64, ppc64le and s390x) regardless of being on physical hardware or in a virtual machine, this document was written using SLES on s390x (IBM Z and LinuxONE) as a KVM virtual machine for the FEP installation.  Automation is used throughout this chapter to minimize the time to deploy SLES and FEP.

This document describes the following:

* Benefits of using IBM Z or LinuxONE, SLES and FEP for enterprise Postgres deployments.
* Prerequisites.
...
== Benefits of using FEP with SLES on an IBM Z or LinuxONE for enterprise Postgres deployments
...
SUSE Linux Enterprise Server for IBM Z and LinuxONE the open source components that surface the hardware capabilities in IBM Z and LinuxONE as well as bring key open source technologies to the platform.  For example, Kernel-based Virtual Machine (KVM) is an open source virtualization technology included in SLES for IBM Z and LinuxONE.  Some of the benefits of KVM include:
...
* Being kernel-level yields performance advantages.
* It is well-known, allowing you to take advantage of existing skills and familiar tooling, like virsh.

The hardware and operating system come together to support the application which in this case is Fujitsu Enterprise Postgres (FEP) which is the focus on this getting started guide.

Combining IBM Z and LinuxONE with SLES for IBM Z and LinuxONE and FEP provide organizations a mission critical and highly secure environment to run many virtual instances of enterprise Postgres for development, test, quality assurance and most importantly production.

== Prerequisites

The following environment was used for verification purposes (your environment might be different):
...
The goal of this chapter is to install a SUSE Linux Enterprise Server (SLES) 15 SP2 based KVM hypervisor/host in an IBM Z LPAR (we chose SLES 15 SP2 as a KVM hypervisor because it contains the latest KVM features and has the longest support cycle of the currently supported SLES versions).
...
The goal of this chapter is to successfully install SLES 12 SP5.  To accomplish this the VM will be started and the autoyast installation will automatically begin.
...
This command should be run twice. The first run updates the zypper stack and the
second run applies all available updates.
