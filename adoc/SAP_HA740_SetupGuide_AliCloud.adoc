...
*Four* ECS instances are created in different Zones of the same VPC via *Console->Elastic Compute Service ECS->Instances->Create Instance*.
Choose the "SUSE Linux Enterprise Server for SAP Applications" image from the Image Market place.

In this example, *two* ECS instances (hostname: {myNode1} and {myNode3}) are created in the Region
*EU Central 1*, Zone A , within the VPC: *ASEHADR*, with SUSE Linux Enterprise Server for SAP Applications 15 SP1
image from the Image Market Place. In contrast, *two* ECS instances (hostname: {myNode2} and {myNode4})
...
image::Alicloud_ECS_ASE.png[instance,width="45%"]
...
=== Confirm OpenApi endpoint address inside VPC
...
----
////
In order to download/install the components in the following steps, the instance has to be able to access Internet. 
The easiest way is to purchase an Elastic IP (https://www.alibabacloud.com/product/eip) and assign it to the instance 
and unassign it once the configuration is done.
...
image::Alicloud_Policy.png[Create Custom Policy,width="73%"]

Create a RAM role “SAP-HA-ROLE” and assign above RAM policy to it.

image::Alicloud_assign_Policy.png[RAM role with RAM Policy,width="75%"]
...
image::Alicloud_RAM_bind.png[Bind/Unbind RAM Role,width="75%"]
...
Ecs Ram Role []: SAP-HA-ROLE   -- Input the RAM role created above
Default Region Id []: eu-central-1 -- Input current region-ID
...
////
Create two NAS storage  via **Console->nas->File System List->Create File System->General Purpose NAS(Pay-as-you-go)**,
...
NOTE: Please adapt the value as you like. These are examples and may not fit into your company policy.
...
Make sure to use the right virtual hostname for each installation step.
...
NOTE: Please adapt the value e.g. SID id, instance number, virtual hostname, ... . These
...
The storage sizing depends on many indicators. Please check the sizing recommendations for the planed use case.
...
Standard SAP on Alibaba Cloud for an HA setup is Multi-AZ deployment with ASCS, Primary DB running in
...
shared file system using NFS) is taken care by the SUSE Linux Enterprise High Availability Extension Cluster.
...
procedure describes the key points which you need to take care.
...
* In our *demo setup* we have deselect -> Use separate devices for sybmgmtdb database
  (please consider different settings for productive environments)
...
Please look at the HADR guide for an example for such an response file.
...
Check if the hostname and port number are correct and the database server is up and running. If everything is correct and network connection should be available, it might help to modify the _interface_ file. Try to add a new line in the _/sybase/<SID>/interfaces_ file for the <SID> section with the IP address of the corresponding hostname.
...
** a unused instance number of the SAP system which is already implemented in the cluster (ASCS/ERS pair)
[[chapter ascs ers]]
...
Site Name. Make sure to use the virtual hostname for the ASCS host. When the Fault Manager is installed,
...
_ha/syb/cluster_fmhost2_ provide the physical hostnames of both nodes where the FM can potentially run.
...
----
======================================
The example above show the FM integration as part of the ASCS instance.
...
* The FM instance is monitored and maintained by pacemaker as sub instance of the ASCS primitive.
That means the FM is started and stop and moved along with the ASCS instance.
...
Now we need a *new* instance profile for the FM. You can take a *copy* of the ASCS profile and adapt them carefully.
...
==== OS Modification for DB Failover
...
NOTE: If the system should start one by one, use the command _sapcontrol -nr <instance number> -function StartSystem_.
...
NOTE: If the system should stop one by one, use the command _sapcontrol -nr <instance number> -function Stop_ on each instance host.
...
primitive res_ALIYUN_STONITH_1 stonith:fence_aliyun \
	op monitor interval=120 timeout=60 \
	params plug=i-gw87xi82sj2dy2ysaw19 ram_role=SAP-ASE region=eu-central-1 \
	meta target-role=Started
primitive res_ALIYUN_STONITH_2 stonith:fence_aliyun \
	op monitor interval=120 timeout=60 \
	params plug=i-gw86pnh1jy1dw0vfer3w ram_role=SAP-ASE region=eu-central-1 \
	meta target-role=Started
