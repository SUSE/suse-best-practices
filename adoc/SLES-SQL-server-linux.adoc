...
=== Business case: Why SQL on SUSE Linux Enterprise
...
Depending on the infrastructure it might be necessary that the CPU setting must be disabled (for example log message: `cpu0: 'energy_perf_bias' = 'None', expected 'performance'`). 
...
With `Create a connection` the {sqls} can added. A pop-up window is asking for the required parameter. The connection can be established to a single node or the virtual IP of a cluster setup.
