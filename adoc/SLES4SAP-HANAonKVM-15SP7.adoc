...
Detailed information about how to setup bridged networking in {sles4sap} can be found in section 23 of the https://documentation.suse.com/sles/15-SP7/html/SLES-all/cha-network.html["SUSE Linux Enterprise Server 15 SP7 Administration Guide"].
...
[NOTE]
====
If you plan to dedicate one or more FC controllers to your virtual machine(s), please make sure that they are not configured in multipath on hypervisor level. Either blacklist them or deactivate multipath entirely (if possible).
====
...
The examples below show configuration snippets for full size single-VM layouts on a 4-node system containing {cascadelake} CPU's (first example) and on a 2-node system containing {sapphirerapids} CPU's (second example).
...
If you want to attach single devices (for example NVMe devices), it is important that you attach them to the VM on PCI level. For that you need to identify the device's PCI address, detach it from the host and then attach it to the VM. You may want to use a script similar to the one below:
