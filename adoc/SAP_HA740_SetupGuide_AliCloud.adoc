...
4 ECS instances are created in different Zones of the same VPC via *Console->Elastic Compute Service ECS->Instances->Create Instance*. Choose the "SUSE Linux Enterprise Server for SAP Applications" image from the Image Market place.

In this example, 2 ECS instances (hostname: {myNode1} and {myNode3}) are created in the Region *EU Central 1*, Zone A , within the VPC: *ASEHADR*, with SUSE Linux Enterprise Server for SAP Applications 15 SP1 image from the Image Market Place. In contrast, 2 ECS 2 ECS instances (hostname: {myNode2} and {myNode4}) are created in the Region *EU Central 1*, Zone A , within the VPC: *ASEHADR*, with SUSE Linux Enterprise Server for SAP Applications 15 SP1 image from the Image Market Place.

image::Alicloud_ECS_ASE.png[instance,width="45%"]

=== Creating NAT Gateway and configure SNAT entry
Now create an NAT Gateway attached to the given VPC. In the example at hand, an NAT Gateway named *ASEHADR* has been created:
...
image::Alicloud_AK_ASE.png[AccessKey,width="45%"]
...
image::alicloud_HA740_nas1[PNG]

Afterwards, please execute below command to mount the created NAS storage:
[subs="attributes"]
----
#sapapp1: mkdir {myMpAscs}
#sapapp1: mount {myDevPartAscs} {myMpAscs}

#sapapp2: mkdir {myMpErs}
#sapapp2: mount {myDevPartErs} {myMpErs}
----

Then, you will have below 2 mount settings.
...
////
In our example, we use block storage for database storage. Please kindly execute below command:
[subs="attributes"]
----
#sapdb1: mkdir {myMPDb}
#sapdb1: echo {myDevPartDb} {myMPDb} ext4 acl,user_xattr,noatime 1 1 >> /etc/fstab
...
//TODO: content for that and change MAXDB back to original???
