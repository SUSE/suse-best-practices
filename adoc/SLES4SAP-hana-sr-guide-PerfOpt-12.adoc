...
// TODO PRIO1: To install HA *and* SAPHanaSR now, because we need it during the setup of SAP HANA
...
// TODO PRIO1: Do we need to move that *before* we set-up the srHook?
...
.{testRecover}
.  Clean up the failed resource status of the secondary HANA database (on
  node 2) as root.
+
[subs="attributes,quotes"]
----
