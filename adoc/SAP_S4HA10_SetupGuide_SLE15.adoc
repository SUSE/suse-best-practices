...
This setup with cluster-controlled filesystem resources
will remain supported for the lifetime of {sles4sap} {slesProdVersion}.
For new deployment we recommend using the new so called simple-mount setup as
described in
"SAP S/4 HANA - Enqueue Replication 2 High Availability Cluster With Simple Mount - Setup Guide"
(https://documentation.suse.com/sbp/all/single-html/SAP_S4HA10_Setup_Simple-Mount-SLE15).
// TODO: verify simple-mount URL
