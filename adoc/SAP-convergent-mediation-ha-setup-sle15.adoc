...
{prodSP} as an high availability cluster for {ConMed} ControlZone services.

From the application perspective the following concept is covered:

- {ConMed} ControlZone platform and UI services are running together

- {ConMed} ControlZone software is installed on central NFS

- {ConMed} ControlZone software is copied to local disks of both nodes
...
bind-mount is /usr/sap/.check_{mySid} . Both mount points have to be created before
...
// TODO PRIO1: link to appendix [id="sec.appendix-crm"]
