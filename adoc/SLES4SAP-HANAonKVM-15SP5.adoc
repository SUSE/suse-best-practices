...
* *<<sec-hypervisor>>*: Host operating system running the hypervisor directly on the server hardware
* *<<sec-guest-vm-xml-configuration>>*: `libvirt` domain XML description of the guest VM
* *<<sec-guest-operating-system>>*: Operating system inside the VM where SAP HANA is running

Follow *<<sec-supported-scenarios-prerequisites>>* and the respective SAP Notes to ensure a supported configuration.
...
[[sec-sap-hana-virtualization-scenarios]]
...
[[sec-supported-scenarios-prerequisites]]
...
_Guest:_ {sles4sapAbbr} 15 SP5 onwards 

_Size:_ max. 2 sockets footnote:max2sockets[Maximum 2 sockets using Intel standard chipsets on a single system board], 4 TiB RAM
|
no
|
no
...
See <<sec-memory-backing>> for more details.
...
<<-sap-hana-core-to-memory-ratio-examples>> below has some current examples of SAP HANA memory-to-core ratios.
...
| {sapphirerapids} | OLTP | 4 TiB / 4096 GiB | 2 | 56 | 36.57 GiB/core
| {sapphirerapids} | OLAP | 4 TiB / 4096 GiB | 2 | 56 | 36.57 GiB/core
|===
...
* 3072 GiB is now the memory size to use in the VM configuration as described in <<sec-memory-backing>>.
...
* fulfill the SAP HANA Hardware and Cloud Measurent Tools (HCMT) storage KPI's as per {launchpadnotes}2493172[SAP Note 2493172 "SAP HANA Hardware and Cloud Measurement Tools"]. 
  Refer to <<sec-storage>> for storage configuration details.
...
[[sec-kvm-hypervisor-installation]]
...
[[sec-configure-networking-on-hypervisor]]
...
[[sec-assign-network-port-at-pci-nic-level]]
...
In all cases, the SAP HANA storage configuration recommendations from the respective hardware vendor and the https://archive.sap.com/kmuuid2/70c8e423-c8aa-3210-3fae-e043f5c1ca92/SAP%20HANA%20TDI%20-%20Storage%20Requirements.pdf [SAP HANA Storage Requirements for TDI] should be followed.
...
As described in <<sec-configure-networking-on-hypervisor>>, to reach the adequate level of performance, the storage drives for actual SAP HANA data are attached to the guest VM via directly assigning the SAN HBA controller to it.
...
The remaining storage configuration details are available in <<sec-storage>>. This includes also information about how to add the disks and the HBA controllers to the guest VM configuration file, 
...
[[sec-hypervisor-operating-system-configuration]]
...
[[sec-verify-tuned-has-set-cpu-frequency-governor-and-performance-bias]]
...
[[sec-customize-the-linux-kernel-boot-options]]
...
[[sec-technical-explanation-of-the-above-described-configuration-settings]]
...
[[sec-guest-vm-xml-configuration]]
...
[[sec-create-an-initial-guest-vm-xml]]
...
For example, if the host has 4294967296 KiB (that is, 4 TiB) of memory on two NUMA cells and you want to leave 91.75% of it to the hypervisor (see <<sec-memory-sizing>>), and there are 4 NUMA cells, each NUMA cell will have the following number of huge pages:
...
Such number must be passed to the host kernel command line parameter on boot (that is `hugepages=3758`, see <<sec-technical-explanation-of-the-above-described-configuration-settings>>).
...
Also refer to <<sec-memory-backing>> and <<sec-memory-sizing>> of the document at hand for further details.
...
One of the Virtual Functions prepared in <<sec-configure-networking-on-hypervisor>> must be added to the guest VM as (one of) its network adapter(s).
...
[[sec-storage-configuration-for-operating-system-volumes]]
...
[[sec-storage-configuration-for-sap-hana-volumes]]
...
Since storage controller passthrough is used (see <<sec-storage-hypervisor>>), any LVM (Logical Volume Manager) and Multipathing configuration should be made inside the guest VM, always following the storage layout recommendations from the appropriate hardware vendor.

The guest VM XML configuration must be based on the underlying storage configuration on the hypervisor (see <<sec-storage-hypervisor>>).
...
Note that this is a requirement for making it possible to load and use the "`cpuidle-haltpoll`" kernel module inside the guest VM OS (see <<sec-cpuidle-haltpoll>>).


[[sec-guest-operating-system]]
== Installing the guest operating system

[[sec-install-sles-for-sap-inside-the-guest-vm]]
...
[[sec-guest-operating-system-configuration-for-sap-hana]]
...
[[sec-customizing-linux-cmdline-guest]]
...
A detailed explanation of these parameters has been given in <<sec-technical-explanation-of-the-above-described-configuration-settings>>.

[[sec-enabling-host-monitoring-guest]]
==== Enabling host monitoring

The VM needs to have the `vm-dump-metrics` package installed, which dumps the metrics provided by the `vhostmd` service running on the hypervisor (see <<sec-vhostmd-guest>> for information how to set this up on hypervisor level). This enables SAP HANA to collect data about the hypervisor. 
...
[[sec-configuring-guest-at-boot-time]]
...
Kernel Same Page Merging (KSM) needs to be disabled, like on the hypervisor (see <<sec-no-ksm>>):
...
[[sec-guest-operating-system-storage-configuration-for-sap-hana-volumes]]
...
[[sec-performance-considerations]]
...
[[sec-useful-commands-on-the-hypervisor]]
...
[[sec-useful-commands-inside-the-vm-guest]]
...
* CPU pinning
** Note the alternating CPU pinning on the hypervisor, see <<sec-vcpu-and-vnuma-topology>> for details
** Note the topology of the guest VM mirrors the one of the hypervisor (2x56 CPU cores)
* `qemu:commandline` elements to describe CPU attributes, for details see <<sec-global-vcpu-configuration>>
