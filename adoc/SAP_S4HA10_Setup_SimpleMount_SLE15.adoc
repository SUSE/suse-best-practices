...
For additional information on the new simple mount architeture, please cf.

- Blog article "Simple Mount Structure for SAP Application Platform"
(https://www.suse.com/c/simple-mount-structure-for-sap-application-platform/)
on the {SUSE} blog (https://www.suse.com/c/blog/)

- TID ("Technical Information Document") 00019944 "Use of Filesystem resource
  for ASCS/ERS HA setup not possible"
(https://www.suse.com/support/kb/doc/?id=000019944) in the {SUSE} knowledge
base (https://www.suse.com/support/kb/).

The former setup with cluster-controlled filesystem resources as decribed in
...
will remain supported as for the lifetime of {sles4sap} {slesProdVersion}.
...
// TODO: use {} variables instead of plain text
...
Last but not least, there are manual pages shipped with the product.
...
//// TODO: create errata for simple-mount guide?
...
- Integrate the high availability cluster with the {sap} control framework via
  *{s4sClConnector3}* version 3, as certified by {sap}, and the new *SAPStartSrv* resource agent
...
the SUSE High Availability cluster. A third node is used for
running the database, a fourth and fifth node for the PAS and AAS application
server instances. An additional sixth node is used as NFS server.
...
// TODO: compare to Filesystem-based
...
// TODO: S/4HA with AnyDB or HANA only?
...
the supported combinations of OS and databases for {sapS4insm} can be found at the
...
=== Disks and NFS Shares

XFS is used for all local file systems. For _/sapmnt_ and _/usr/sap/SID_ NFS is used.

==== Shared NFS mounts for Cluster ASCS and ERS, shared disk for SBD

The disk for the fencing machanism SBD must be shared and assigned to the
...
{myDevA} is visible on {my2nd1} and {my2nd2}:
----
lsblk | grep {myDevA}
----
...
The disk {myDevB} for the database (160GB) is assigned to {myNode1} and formatted with XFS.
...
.NFS server
- {myNFSSrv}:{myNFSSapmnt} /sapmnt/{mySid}

- {myNFSSrv}:{myNFSUsrSap} /usr/sap/{mySid}
...
- Create mount points and mounting NFS shares on all non-HANA nodes, i.e. on
  {my2nd1}, {my2nd2} and, if the appication servers should also profit from the
  simplified storage setup, {my2nd3} and {my2nd4}.

.Mount NFS Shares on all non-HANA nodes
==============================================
[subs="attributes"]
----
# mkdir -p /sapmnt/{mySid} /var/lib/Landscape
# mkdir -p /usr/sap/{mySid}
...
If applicable, make sure to mount file systems like _/var/lib/Landscape/_.
...
=== OS Preparation and Cluster Software Installation
...
// DONE: remove any mentioning of master-slave aka multi-state
...
==== Preparations for the new SAPStartSrv resource agent implementation

As a prerequisite for having a single NFS mount for ASCS and ERS, the
_sapstartsrv_ instance agents of ASCS and ERS must not be started by the
_sapinit_ service during system startup, as these services are started and
...
moves _/usr/sap/sapservices_ out of the way. Consequently, the _sapstartsrv_
instance agents are not started automatically by _sapinit_. *sappong* runs after _sapinit_
and moves _/usr/sap/sapservices_ back to its original location.

On {my2nd1} and {my2nd2}, chekc for *sapstartsrv-resource-agents* package and
...
First, configure the resources for the IP address, the {sap} instance agent and the {sap}
instance. You need to adapt the parameters for your specific environment.

Please make sure that in the {sap} instance definition the parameter
*MINIMAL_PROBE* is set to *true*.
...
Second, configure the resources for the IP address, the {sap} instance agent
and the {sap} instance. You need to adapt the parameters for your specific
environment. 

Please make sure that in the {sap} instance definition the
...
// TODO: use resource maintenance instead of cluster maintenance
...
=== Setup Multinode Cluster from the Top with Diskless SBD
...
==== OS Preparation and Installation of the Cluster Software
...
// TODO: diskless sbd does not always work, see #1192467 

==========

==== Configure the Multi Node Cluster Base with Diskless SBD
...
==== OS Installation of The New Cluster Node Member
...
==== Preparations for the new SAPStartSrv resource agent implementation on 3rd node

-I nstall the sapstartsrv-resource-agents package and enable the
...
+
The following commans register the ASCS and the ERS {sap} instance:
+
[subs="specialchars,attributes"]
----
...
- Joining the cluster with _sleha-join_
...
is for the two node cluster. In multi node cluster you will find additonal node entries
like *node 3: {my3nd3}*.

// TODO: compare with CIB from valuga21
...
group grp_{mySid}_{myInstAscs} rsc_ip_{mySid}_{myInstAscs} rsc_SAPStartSrv_{mySID}_{myInstAscs}  \
    rsc_sap_{mySid}_{myInstAscs} \
    meta resource-stickiness=3000
group grp_{mySid}_{myInstErs} rsc_ip_{mySid}_{myInstErs} rsc_SAPStartSrv_{mySID}_{myInstErs} \
...
// DONE: below looks like at very beginning of this file: delete garbage.
