...
Check if the file _/etc/hosts_ contains at least the address resolution for
...
Check if the file _/etc/fstab_ contains the NFS shares for TODO
TODO on both nodes
...
Check if the file _/etc/passwd_ contains the mzadmin user {mySapAdm}.
TODO on both nodes
...
===============================================================================
^* ntp.longtime.ago              2  10   377   100  -1286us[-1183us] +/-   15ms
----
...
For more information on SBD configuration parameters, read the section Storage-based Fencing,
{SLEHA} Extension, and TIDs 7016880 and 7008216. See also manual pages sbd(8), stonith_sbd(7)
and cs_show_sbd_devices(8).
...
# systemctl status corosync | grep Active
 ...
 Active: active (running) since Tue 2024-05-14 16:37:28 CEST; 14min ago
...
In case of ControlZone UI failure (or monitor timeout), the UI resource gets restarted
...
TODO make a backup of the cluster resource configuration.
...
#
primitive rsc_fs_{mySid} ocf:heartbeat:Filesystem \
    params device=/usr/sap/{mySid}/ directory=/mnt/check/{mySid}/ \
    fstype=nfs4 options=bind,rw,noac,sync,defaults \
    op monitor interval=90 timeout=120 on-fail=restart \
    op_params OCF_CHECK_LEVEL=20 \
