...
Nowadays, it is viable to consider running SQL Server workloads on Linux.
...
to migrate to Linux. Or, who already run a primarily Linux environment and are adding an SQL Server requirement, and prefer tools that run only on Linux servers. Another reason may be lower negotiated pricing for Linux subscriptions to replace existing SQL Servers on Windows machines.
...
Then, to configure and start `msql-server` immediately after configuring, run the following command:
...
A sample `mssql.conf` file is shown below. Edit and restart `mssql-server` to apply changes.
