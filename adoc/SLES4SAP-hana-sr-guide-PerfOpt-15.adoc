...
+
A _Multiple Tier_ (multi-tier) system replication has an additional target. In the past this third
side must have been connected to the secondary (chain topology). With current {saphana}
versions also _multiple target topology_ (multi-target) is allowed by {sap}.
Please look at the scenarios and prerequisites section below or manual pages SAPHanaSR(7)
and SAPHanaSR.py(7) for details.
...
scale-up package.
* The user _{refsidadm}_ needs execution permission as user root for the command crm_attribute.
