...
This document will walk you through the deployment of a simple two-node SAP HANA HA Cluster using the SUSE SAP Automation Project and operating on Azure.
...
ifeval::[ "{cloud}" == "Azure" ]
The image used is set by default to the PAYG image for sles-sap-15-sp2:gen2. You do not need to change/set the variable
Only if you want to set something different, you need to add the variable "os_image"
endif::[]
...
----
hana_inst_master = "//YOUR_STORAGE_ACCOUNT_NAME.file.core.windows.net/mysapmedia/HANA2/51054623"
----
endif::[]
...
hana_archive_file = "51054623.exe"
----
