...
----
The _/usr/sap/sapservices_ directory looks as follows - typically one line per instance is displayed:
...
Another benefit of this concept is that you can work with local (mountable)
...
instance after a failure (loc_sap_{mysid}_failover_to_ers). This constraint was needed in the past 
to ensure that the locks are not lost after an ASCS instance (or node)
