...
cluster frameworks. This improvement also includes the implementaion of {saphana}
call outs for special events such as status changes for services or system replication
...
This step is mandatory to inform the cluster directly, if secondary gets out of sync.
is breaking. The hook is called by {SAPHANA} immediately when the secondary gets out of
sync. This is typically the case when the first commit pending is released. The hook is
also called by {SAPHANA} when the system replication is back and at the point of time the
secondary is in sync again.
...
More specific parameters option to meet a high security level. All Cmnd_Alias entries must be each defined as a single line entry.
