...
With this parameter, Terraform will use the AWS credentials file created above.
...
Next, set some options for the HANA instances, the following selects the instance name and hostname prefix for the deployment.
...
Finally, to ensure a fully automated deployment it is possible to set passwords within the *terraform.tfvars* file. If the parameters are not set in this file, they must be entered when running the deployment. 

Uncomment and set the following, note that all passwords must conform to SAP Password policies or the deployment will fail:
...
unattended, resulting in a failed deployment.  It is stongly advised to retain focus on the AWS Cloud Shell window to ensure the timeout does not occur.
