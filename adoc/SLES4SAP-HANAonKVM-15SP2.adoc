...
As described for Networking, in order to reach the adequate level of performance, the storage drives for actual SAP HANA data are attached to the Guest VM via directly assigning the SAN HBA controller to it.

First of all, find the SAN HBA that must be used, e.g., using the "`lspci`" command:
...
Of course, before starting the VM, the devices need to be detached from the host, e.g.:

----
virsh nodedev-detach pci_0000_85_00_0
virsh nodedev-detach pci_0000_85_00_1
----

This can be made happen automatically at boot time, following the same steps described in <<_sec_assign_network_port_at_pci_nic_level>> as, despite that section being about network adapters, the procedure is the same for any PCI device).
...
//TODO: will such Appendix still be there?
//(see Appendix for an <<_sec_lscpu_extended_example>> and an <<_sec_example_lstopo>>):
...
You can use either a vbd disk or a virtio-serial device (preferred) to set this up (see https://confluence.suse.com/display/SAP/HANAonKVM+vhostmd for details).
//TODO: Add a proper, external, link
