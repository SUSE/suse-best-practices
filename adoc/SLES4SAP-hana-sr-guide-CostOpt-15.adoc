...
register a failed primary after a takeover manually. For re-registration, use precisely the site names that are already known by the cluster. Use SAP tools like {saphana} cockpit or _hdbnsutil_.
...
site name. The site names must not be changed later when the cluster has been activated.
...
convenience, use YaST or the {slehainit} script. It is strongly
