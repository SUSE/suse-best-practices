...
database including the system replication is set up. As last the
...
For more details regarding both modes check SAP documentation such as
...
As standard STONITH mechanism impelemnt SBD based fencing. The SBD STONITH method is
...
In most installations, nly the parameter *ip* needs to be set to the virtual
...
In this version of the setup guide a plain list of test cases is provided. A more detailled description of the
...
* The **site** section includes the attributes per site and shows which site is
  the primary and the returncode of the landscapeHostConfiguration.py
...
The majority maker suse-mm does not run an {saphana} instance and thatfore
