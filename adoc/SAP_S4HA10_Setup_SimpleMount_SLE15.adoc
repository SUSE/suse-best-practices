...
For additional information on the new simple mount architecture, also read:
...
=== Install the Primary Application Server (PAS) on {myNode2}
...
Check that the SAP NFS shares are also mounted on the additional cluster node.
...
On {my2nd1} and {my2nd2}, check for *sapstartsrv-resource-agents* package and
...
## check, if cluster is in status idle 
# sapcontrol -nr {myAscsIno} -function Start
----
...
. OS installation of the new node
...
As the directories /sapmnt/{mySid}, /usr/sap/{mySid} need to be available at all time, make sure they are mounted during boot. This can be
achieved by putting the information into the /etc/fstab.

In the next step the following information is required:
...
- Joining the cluster with _*ha-cluster-join*_
