...
Check the state of the Linux cluster and ASCS/ERS:
...
Display the cluster summary, node list and resource list with the following command:
...
If the command returns no output, no undesired constraints exist.

Display a list of system instances:
...
Display a list of processes running on the ASCS and ERS instances:
...
Display details for the cluster, available high-availability (HA) nodes and the active instance node:
...
You can now perform maintenance on the Linux cluster or system. Before performinc maintenance, 
stop any SAP instances as necessary. If you stopped the SAP instances, start them before
activating the cluster.

When maintenance is complete, start the Linux cluster on all nodes:
...
Set the cluster to ready for operations:
...
To *migrate* the ASCS {sap} instance, use {sap} tools such as
the {sap} management console. This will trigger *{sapStartSrv}* to use the
{s4sClConnector3} to migrate the ASCS instance. As user _{mysapadm}_, run
...
We recommend to perform at least the following tests before you
...
Verify that the high-availability (HA) configuration tests pass without errors.
...
Verify that manually migrating the ASCS instance using HA tools works properly.
...
Verify that moving the ASCS instance using {sap} tools like {sapCtrl} works properly.
...
Verify that the ASCS instance moves correctly after a node failure.
...
The following test verifies that the cluster solution neither interacts with nor restarts the ASCS instance during a maintenance procedure.
In addition, it verifies that no locks are lost when restarting an ASCS instance
during a rolling kernel switch (RKS) procedure. The cluster solution should recognize
that the restart of the ASCS instance was expected. No failures or errors should
be reported or counted.

Optionally, you can set locks and verify that they still exist after the maintenance
procedure. There are multiple ways to do so. One example test can be performed as
follows:
...
Perform this test multiple times within a short time frame. In the following example, the ASCS instance
restarts five times.
...
. Check in SAP note 953653 whether the new kernel patch is RKS-compatible with your currently running kernel.
