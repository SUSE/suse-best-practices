...
- Plan a {sleHA} platform for {sapS4pl}, including SAP Stand Alone Enqueue Server (ENSA2) Replication.
- Set up a {linux} high availability platform and perform a basic {sapS4pl}
  installation including SAP Stand Alone Enqueue Server (ENSA2) Replication on {sle}.
...
.The following OS / databases combinations are examples for this scenario
...
* Partition two ({myDevPartAscs}) for the first file system (10GB) formatted with XFS
* Partition three ({myDevPartErs}) for the second file system (10GB) formatted with XFS
...
=== The Multinode Cluster Setup for an {sapS4}

Depending from your starting point it is possible to extend a two node cluster setup or directly start with
more than two nodes for the ASCS / ERS.
The example below will show the extension of an existing two node cluster pair. The major configuration changes
will be shown and the basic preparation of the new cluster member node.

==== Two Node Configuration - Starting point
...
The system is configured as described in our  the best practice
SAP S/4 HANA - Enqueue Replication 2 High Availability Cluster - Setup Guide.

.Backup the cluster configuration

Go to one of the cluster nodes and safe the cluster configuration with crm:
...
Take care for the existing /etc/corosync/corosync.conf and all files which maybe important for a restore.
This example is only one methode to create a configuration backup.

[subs="attributes"]
----
# tar cvf /<path to an external storage>/bck_2nodes_configuration.tar /etc/corosync/corosync.conf 2node.txt ..
----
...
Recommended is to use an automated installation which takes care that all your system set up identical.
A documentation which describes the changes from this automated setup is very helpful. In our example we
deploy our machines with an autoyast configuration file and run a post step script which does the basic
configuration.

===== Patching the existing nodes.

Depending on you infrastructure and installation setup it must be necessary to install the latest updates
and patches on your existing nodes. In case there are freezed repositories like SUSE Manager provides this.
The new system could be added to the same repositories and will have at least the same patch level as your
existing nodes.

Use *zypper patch* or *zypper update* depending on the company rules.

- It is recommended to install the latest available patches to guaranty the system stability and hardening.
Bug fixing and security patches helps to did not run into unplanned outages and make the system less vulnerable.
...
.Please take care for a valid DNS, time, network setup and patch level.
...
We experienced some trouble if there is no valid or no default route set.

- patch the new system like the existing nodes, verify that your system have the same patch level and
 required reboot are done and not outstanding
After installing all needed packages this could be one way to check the package versions.
...
.copy the file from one node to the other and compare it
...
- install *pattern ha_sles* on new cluster node
...
SAP will provide with SWPM 2.0 a new option which can do all the needed steps to prepare a fresh
install server to be able to fit into an existing SAP system. This new option will help us to prepare a
new host which could run later one the ASCS or ERS in our cluster environment.
...
As this directory should be available at all the time they can be mounted during boot. This can be
