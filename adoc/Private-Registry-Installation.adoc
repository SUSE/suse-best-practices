...
+
By default, the Redisfailover deployment has three sentinel replicas, three redis replicas, and will keep the data when the Helm chart is uninstalled. This behavior can be configured in the `redisfailover` section. See `harbor/charts/redisfailover/Values.yaml` for all configuration options.
+
