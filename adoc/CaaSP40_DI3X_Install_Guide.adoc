...
In order to install SAP Data Intelligence 3 on SUSE CaaS Platform 4, we need to
...
In order to be able to get all maintenance updates for your SUSE products, you
...
SLE 15 SP1 on your premises. We will use the SLE 15 SP1 installer ISO image for
starting the installation.

Make sure that the hostnames you will use for the installation can be resolved
...
Install SLE 15 SP1 or higher (as released for CaaS Platform 4.x) on all nodes.
...
+
(in case of SLES for SAP Applications, this is added automatically)
...
=== Prepare the SAP Data Intelligence 3 installation

Here are the steps that are needed to successfully install SAP Data Hub on SUSE
CaaS Platform.


==== Download the SAP Data Intelligence 3 software archive:
...
SUSE Storage cluster. They are located in `ceph.admin.keyring` and
`ceph.user.keyring`.
...
=== Installation of SAP Data Intelligence 3 using the Maintenance Planner with SL Plugin

That is, the `mpsl` installation method. SAP recommends this approach to install
SAP Data Intelligence 3.

This is a web-based installation method. SAP recommend it because it offers you
...
NOTE: You have to install the latest SAP Host Agent on the management host. You
...
=== Installation of SAP Data Intelligence 3 using the SL Plugin (mpfree method)
...
=== Installation of SAP Data Intelligence 3 using the command line (manual installation)
...
* Import any necessary Certification Authorities, for example the CA that signed
the certificate of the secure registry.


== Upgrade of SAP Data Intelligence 3
...
To satisfy the requirements of DataHub, you also need a Docker Registry.
...
provides common services (DNS, SMT, Docker-registry) for the Datahub stack.
...
To avoid dealing with Docker's insecure registry configuration, we will add SSL
to our setup.
...
Then, on all servers that need to interact with the Docker registry:
...
You can now log on Portus and configure the registry.

image::portus-registry.png[portus-registry.png,480,640]

==== Installation and configuration of a secure private registry using SLES and the SLE-Container-Module
...
storage class, but also for your Kubernetes Control plan (virtualised or not),
...
The following steps will deploy a minimalistic, virtualised, test-oriented
...
Enterprise Storage from https://scc.suse.com, or have a SMT/RMT properly set up
...
* You should already have set up a DNS zone. In our example, where all Datahub
components are in the same DNS zone and the same subnet, it should look like:
+
image::dns.png[dns,480,640]

* In order to be as efficient as possible when using interactive shell-scripted infrastructure deployment, we advice you to use an advanced terminal client or
...
** Once the nodes have rebooted, log in and finish the network/hostname and NTP
...
*** Once Stage 3 has completed successfully, check the cluster's health to
...
** In order to get the benefits of the OpenAttic WebUI, you must now initiate
...
** To provide a Datahub RBD device, we first need to create a related pool.
+
