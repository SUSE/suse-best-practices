...
* Run the `skuba` tool for initialisation of the cluster.
...
folder. If the folder `/etc/docker/certs.d` does not exist, create it.
+
----
...
create a folder `/etc/docker/certs.d/<FQDN>:<Port>` and copy the certificates to
this directory instead.

. In order to introduce the certificates to Docker, you must restart the Docker
daemon:
+
----
...
Enter `admin` as the username and enter the password specified in the `harbor.yml`
...
You should also check if the Docker clients on your Kubernetes nodes are able to
...
You will be prompted to enter a username and password. Use `admin` for the
username, and the password you set in `harbor.yml` (default: "Harbor12345").

If Docker is able to access the registry, you will see a line that says
"Login Succeeded" or similar.

If your machines are not able to resolve the FQDN of your registry, edit your
...
storage class but also for your Kubernetes control plane, virtualised or
...
. Now, either enter your email and the registration code you collected from the
...
"Synchronize with NTP-Server" option. You must then choose an NTP server and
...
Check that your machines have the correct hostnames:
...
. Once the deployment stage  has been successfully passed, check the cluster
...
. On the left side of "Applications", click on the pencil and choose `rbd`.
...
. Enter the name, and if it is not already selected, choose the
previously-created pool. Choose the size of the RBD and confirm creation by
...
Click on "Plan a New System" on the right hand side.
...
On the left hand side, there is a window with three toggle buttons. Choose
...
"SAP DATA HUB 2". Choose "SAP DATA INTELLIGENCE 3".
...
On the right hand side, a drop-down menu is shown with "Select Support Package
...
Select what you need and click on the "Confirm Selection" at the very right.
...
An overview of your selection is shown. If this fits your needs, click on the
...
Next, you will choose the operating system upon which SAP Data Intelligence will
...
The next page shows the preselected files to use and download. Once again, click
...
* Select the SAP Data Intelligence deployment required by your needs. Choose
"Next".
+
...
modules are loaded on all Kubernetes nodes. Check the checkbox and click "Next".
+
