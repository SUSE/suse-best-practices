...
////
// TODO remove: An example for legacy SystemV is outlined in the appendix
...
Some more {sap} related services might be enabled. For correct tuning the service `saptune` is required.

[subs="attributes,quotes"]
----
# systemctl list-unit-files "SAP*"
UNIT FILE                        STATE           PRESET
SAP{sapsid}_{sapino}.service                enabled         disabled
SAP.slice                        static          -
----
The SAP instance service SAP{sapsid}_{sapino}.service is enabled and SAP.slice is available, as required.
