...
include::Var_SLES4SAP-hana-classic-scaleup-costopt-{prodNr}.txt[]
include::Var_SLES4SAP-hana-classic-scaleup-costopt-{prodNr}-param.txt[]
...
if the primary {SAPHANA} system (like {sapsid}) fails, the anti-colocation rules for the {SAPHANA} non-replicated system (like QAS) are triggered
