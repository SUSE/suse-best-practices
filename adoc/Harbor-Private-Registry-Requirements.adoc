...
{caasp} 4.5 provides only very limited options of supported `StorageClass` configurations - see the SUSE CaaS Platform 4.5 documentation at https://documentation.suse.com/suse-caasp/4.5/html/caasp-admin/_storage.html for details.
...
Refer to the table in <<supported-depployment>> for more details.
...
For simple deployments, an external public service such as link:nip.io[nip.io] may be used to provide quick DNS mapping for IP addresses without requiring additional services or infrastructure configuration changes. For example, if the external IP address allocated to the {kube} Ingress Controller is 10.86.0.237, then the FQDNs used for the Harbor UI/API and notary endpoints might be:
...
===== EKS - Use the ALB Ingress Controller
...
// TODO
TBD (Dirk Mueller ): this section requires clarification
...
Detailed instructions on generating TLS certificates are outside the scope of this document, but some suggestions are provided in this section.
...
An internal database is provided with the {spr} helm chart, but this builtin service is not highly available nor scalable. For production deployments running in public cloud, it is recommended that a highly-available and scalable managed PostgreSQL database instance be created and configured as an external database for {spr}. This section covers only high-level instructions on creating a managed PostgreSQL database instance for Azure and AWS. For detailed instructions, please refer to the official documentation: link:https://docs.microsoft.com/en-us/azure/postgresql/[Azure Database for PostgreSQL] and link:https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_PostgreSQL.html[PostgreSQL on Amazon RDS].
...
image::registry-01-postgres-create.png[registry-01-postgres-create.png,scaledwidth=80%]
image::registry-02-postgres-creating.png[registry-02-postgres-creating.png,scaledwidth=80%]
image::registry-03-postgres-deploying.png[registry-03-postgres-deploying.png,scaledwidth=80%]
...
image::registry-05-postgres-firewall.png[scaledwidth=80%]
...
image::registry-04-postgres-connection.png[scaledwidth=80%]

// TODO AI (Dirk Mueller) needs to be filled in
==== AWS PostgreSQL
...
For detailed instructions, please refer to the official documentation: link:https://docs.microsoft.com/en-us/azure/azure-cache-for-redis/[Azure Cache for Redis] and link:https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/WhatIs.html[Amazon ElastiCache for Redis].

As an alternative, the SUSE Redis operator may be used to deploy a {kube} managed Redis service and not rely on a public cloud managed Redis service. This is covered in the {spr} installation instructions.
...
Refer to <<external-redis-setup>> in the Deployment section.

Alternatively, the Redis Cache can be created from Azure Portal page:

image::registry-06-redis-creating.png[]
...
* Internal TLS can be enabled to secure internal connections between the {spr} components. This doesn't currently cover the internal database and internal redis components.
...
+
This can easily be configured in the Helm chart for stateless components, without additional requirements.
...
* Persistent volumes for some components must be "Read-Write-Many" access
For some stateful components (registry, jobservice), increasing the number of replicas is only possible with persistent volumes that support the "Read-Write-Many" access mode.
...
See <<external-database-setup>> in the Deployment section.
* To enable high availability for the Redis component
** Use an external HA redis service (Azure Cache / Amazon ElastiCache)
    ...
    +
    or
** Run the SUSE Redis operator on the same cluster as the de facto "external" redis service
+
