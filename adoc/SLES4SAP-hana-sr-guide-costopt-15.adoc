...
Linux system. If that is not possible, additional measures are needed to ensure
reliable resolution of users, groups and permissions at any time. This might
include caching.
...
scenario.
// TODO Prio3: align below with perf-opt
* Only one system replication between the two {SAPHANA} database in the Linux cluster.
// TODO Prio3: Maximum one system replication to an HANA database outside the Linux cluster.
** Once an {HANA} system replication site is known to the Linux cluster, that exact
site name has to be used whenever the site is registered manually.
** The {SAPHANA} site name is from 2 up to 32 characters long. It starts with a
...
The systemd enabled saphostagent and instance ́s sapstartsrv is supported from
{SAPHanaSR} 0.155 onwards. Please refer to the OS documentation for the systemd version.
Please refer to {SAP} documentation for the {SAPHANA} version. Combining systemd style
hostagent with SystemV style instance is allowed. However, all nodes in one Linux
cluster have to use the same style.
...
See also manual pages SAPHanaSR(7) and SAPHanaSR.py(7) for more details and requirements.
...
Also, take a look at the {HANA} product availability matrix, which can for
example be found at
...
// DONE PRIO1: publish the below, once SAP ships systemd integration for HANA
// begin systemd style
=== Check if the SAP hostagent is installed on all cluster nodes
...
// DONE PRIO1: remove below, once SAP ships systemd integration for HANA
// begin systemV style
////
=== Check if the SAP hostagent is installed on all cluster nodes
...
Another hook is called by {SAPHANA} after an SR takeover has happened. This method can
...
The hook must be configured on the takeover node where usually the {SAPHANA}
