...
The disk {myDevB} for the database (260 GB) is assigned to {myNode1} and formatted with XFS.
...
. Prepare hte operating system and install the cluster software.
...
`sapinit` service during system startup, as these services are started and
...
Make sure that in the {sap} instance definition the parameter
