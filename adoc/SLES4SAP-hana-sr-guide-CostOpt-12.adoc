...
** Any STONITH mechanism supported for production use by SUSE Linux Enterprise {prodnr} High Availability Extension (like SBD, IPMI) is supported with SAPHanaSR.
...
==========
Add the memory limits to the global.ini. Again SUSE could not provide a sizing guide here. {HANA} sizing needs to be done according to respective SAP guidelines. 
...
directory like /hana/shared/srHook/. The hook must be available on all {SAPHANA} cluster nodes.
