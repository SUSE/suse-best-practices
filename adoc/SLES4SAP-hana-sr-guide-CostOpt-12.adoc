...
// TODO PRIO1: CostOpt12 - Still only non-productive databases on the secondary side?

{SAP} allows to run a non-productive instance of {SAPHANA} on the system replication site on
the secondary site. Such a non-productive database could be development (DEV), test (TST), or
...
shutdown of the non-productive {SAPHANA}.
...
consume much time and also the needed graceful stop of the non-productive system will take additional time. Thus
...
using the resource agents `SAPHana` and `SAPHanaTopology`. The handling of the non-productive database
...
The automated shutdown of the non-productive {SAPHANA} database (e.g. QAS) is achieved by cluster
rules. Concrete it is an anti-collocation of {SAPHANA} promoted vs. {SAPHANA} non-prod. This means
if the primary {SAPHANA} system (like HA1) fails the anti-collocation rules for the {SAPHANA} non-prod (like QAS) are triggered
and the `SAPInstance` resource agent shuts down the non-productive {SAPHANA} database.

The takeover to the secondary site takes a long period of time, because the non-productive database needs to
be stopped gracefully prior to takeover the productive database. This prolonged takeover time
is the main disadvantage of the cost optimization scenario. So, the cost optimized scenario might be combined with persitent memory to benefit from SAP HANA’s persistent memory features.
...
** Intel Optane DCPMM (aka PMEM) is supported since {sles4sap} {prodnr} SP4.
...
** {hana} 2.0 SPS03 or later provides the needed HA/DR provider hook method srConnectionChanegd() with multi-target aware parameters.
** The multi-target setup has been tested with {hana} 2.0 SPS04.
...
{HANA} 2.0 SPS04 and later do support persitent memory.
...
// DONE PRIO1: CostOpt12 - Should we add a step for CostOpt (install non-productive DB) at this level or should we 'hide' the step in 'stepHANA'?  => keep it inline
...
//        Database installation including non-productive (or non-SR SAP HANA system)
== {stepPlanning}
...
for simplyfing common adminstrative tasks. For more information, see Installation
and Basic Setup, {sleha}.
...
rsync -va {,<node1-site B>:}$PWD/data/SSFS_{refSID}.DAT
rsync -va {,<node1-site B>:}$PWD/key/SSFS_{refSID}.KEY
----
==========
...
==== Add the cluster resource for the non-productive {SAPHANA} database

// TODO PRIO1: CostOpt12 - Check the resource priories (of all resources)
// TODO PRIO1: CostOpt12 - Use variables as placeholders

For the non-productive {SAPHANA} database a new resource is added to the cluster.
...
==== Adding the cluster rules for the automatic shutdown of the non-productive {SAPHANA}
 ...
 ** Secondary is Promoted as Primary
...
.{testExpect}
 - Primary restarts on site A (PREFER_SITE_TAKEOVER=false) till failcount >= migration-threshold
 - If takeover occurs: Non-replicated database is stopped on node 2 (site B) and Secondary is Promoted as Primary

.{testRecover}
 - No recovery needed if no takeover did occur
 - Recovery after takeover:
. Register site A to site B
. Register cleanup for site A
==========
...
.{testExpect}
 - Cluster restart non-replicated database on node 2 (site B)
