...
// Ab's ToDo
// 1. Update Appendix -> CRM Configuration output.
// 2. DONE: Put '#!/bin/bash' in the first line of Bernd's script
// 3. DONE: Update Appendix -> Corosync Configurations output.
// 4. Creating a three-node cluster with diskless SBD and update SBD disk configurations, 'crm status' and 'corosync.conf' outputs
// 5. Find a way to use SWPM 2 SP27
//
...
- Create mount points and mounting NFS shares on the new node
...
=== Installing DB on {myNode1} (Example SAP HANA)
...
WARNING: Do *not* clean up the complete group of the ASCS resources. This might lead to an unwanted cluster action
...
. Create a new user. Don't finish the transaction to see the locks.
...
=== The Multi-Node Cluster Setup for an {sapS4}
...
more than two nodes for a ASCS / ERS high availability setup.
...
to have a shared device or an IPMI device is available. A solution for such environments could be the
...
----
	...
	nodelist {
		node {
			ring0_addr: 192.168.122.25
			nodeid: 1
		}

		node {
			ring0_addr: 192.168.122.24
			nodeid: 2
		}
...
=== Appendix: Extend The Two Node Cluster Configuration
...
- We recommended installing the latest available patches to guarantee system stability and hardening.
...
.Make sure to use valid DNS, time, network settings and verify the node's patch level.
...
[subs="attributes"]
----
# vimdiff rpm_{my3nd3}.log rpm_{my2nd1}.log
----
==========
...
NOTE: Ensure that the new node is connected to the same SBD disk connected to the existing two nodes

NOTE: Ensure that the new node has the same exact SBD configurations already exist on the two existing nodes
...
NOTE: This must be done for each instance.  ERS: /usr/sap/hostctrl/exe/sapstartsrv pf=/<path to profile>/<SID>_ERS<INO>_<virt. hostname name> -reg
 ASCS:  /usr/sap/hostctrl/exe/sapstartsrv pf=/<path to profile>/<SID>_ASCS<INO>_<virt. hostname name> -reg
...
==========
Please check if the SBD device is available in case the SBD stonith method is in place for the two nodes.
...
==== Example of corosync.conf for Three Nodes
...
- The cluster configuration for the three node example

[subs="specialchars,attributes,quotes"]
----
{my2nd1}:~ # crm configure show
node 1: {my2nd1}
node 2: {my2nd2}
node 3: {my3nd3}
primitive rsc_fs_{mySid}_ASCS{myAscsIno} Filesystem \
	params device="/dev/sda2" directory="/usr/sap/{mySid}/ASCS{myAscsIno}" fstype=xfs \
	op start timeout=60s interval=0 \
	op stop timeout=60s interval=0 \
	op monitor interval=20s timeout=40s
primitive rsc_fs_{mySid}_ERS{myErsIno} Filesystem \
	params device="/dev/sda3" directory="/usr/sap/{mySid}/ERS{myErsIno}" fstype=xfs \
	op start timeout=60s interval=0 \
	op stop timeout=60s interval=0 \
	op monitor interval=20s timeout=40s
primitive rsc_ip_{mySid}_ASCS{myAscsIno} IPaddr2 \
	params ip={myVipAAscs} \
	op monitor interval=10s timeout=20s
primitive rsc_ip_{mySid}_ERS{myErsIno} IPaddr2 \
	params ip={myVipAErs} \
	op monitor interval=10s timeout=20s
primitive rsc_sap_{mySid}_ASCS{myAscsIno} SAPInstance \
	...
	op monitor interval=11 timeout=60 \
	op_params on_fail=restart \
	params InstanceName={mySid}_ASCS{myAscsIno}_sap{mySidLc}as START_PROFILE="/sapmnt/{mySid}/profile/{mySid}_ASCS{myAscsIno}_sap{mySidLc}as" AUTOMATIC_RECOVER=false \
	meta resource-stickiness=5000 target-role=Started maintenance=false
primitive rsc_sap_{mySid}_ERS{myErsIno} SAPInstance \
	operations $id=rsc_sap_{mySid}_ERS{myErsIno}-operations \
	op monitor interval=11 timeout=60 \
	op_params on_fail=restart \
	params InstanceName={mySid}_ERS{myErsIno}_sap{mySidLc}er START_PROFILE="/sapmnt/{mySid}/profile/{mySid}_ERS{myErsIno}_sap{mySidLc}er" AUTOMATIC_RECOVER=false IS_ERS=true \
	meta target-role=Started maintenance=false
primitive stonith-sbd stonith:external/sbd \
	params pcmk_delay_max=30s pcmk_action_limit=-1
group grp_{mySid}_ASCS{myAscsIno} rsc_ip_{mySid}_ASCS{myAscsIno} rsc_fs_{mySid}_ASCS{myAscsIno} rsc_sap_{mySid}_ASCS{myAscsIno} \
	meta resource-stickiness=3000 target-role=Started
group grp_{mySid}_ERS{myErsIno} rsc_ip_{mySid}_ERS{myErsIno} rsc_fs_{mySid}_ERS{myErsIno} rsc_sap_{mySid}_ERS{myErsIno} \
	meta target-role=Started
colocation ASCS{myAscsIno}_ERS{myErsIno}_separated_{mySid} -50000: grp_{mySid}_ERS{myErsIno} grp_{mySid}_ASCS{myAscsIno}
order ord_sap_{mySid}_first_start_ascs Optional: rsc_sap_{mySid}_ASCS{myAscsIno}:start rsc_sap_{mySid}_ERS{myErsIno}:stop symmetrical=false
property cib-bootstrap-options: \
	have-watchdog=true \
