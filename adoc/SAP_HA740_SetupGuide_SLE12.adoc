...
////
Feedback Ab:
DONE SAP NetWeaver 7.40 -> SAP NetWeaver 7.50 (7.40 used, both possible)
DONE Hostname sap<sid>ci -> sap<sid>d1
DONE Check consistency of IP addresses (vIP)
DONE missing: mkdir /sapcd
DONE DVEBMGSxx -> Dxx (for NW7.50)
DONE ls with option "-a" or remove . and .. from list of directories (output)
DONE Check groupNr sapinst. Ab said sapinst was already existing with Nr 1001
DONE different SWMP menue-klicks for SAP NW 7.50
DONE sap-suse-cluster-connector is now version 3.1.0 (not 3.0.0)
DONE For SAP NW7.50 the parameter for Enqueue was already "Start_Program_xx"
DONE New cluster-init-command with option "-u"
DONE check the cluster-init-command either with all steps or add the ssh-step to the list of steps
DONE check cluster-join-command either with all steps or add the ssh-step to the list of steps
...
resources at the {sles4sap} resource
...
=== Install the Primary Application Server (PAS) on {myNode3}
...
=== Install an Additional Application Server (AAS) on {myNode3}
...
Keep in mind that a hardware watchdog is prefered instead of the softdog method.
...
You could use either YaST to configure the cluster base or use the interactive
command line tool ha-cluster-join. The following script could be used for
automated setups.
...
- After both nodes listed in the overview please verify the property setting of the basic cluster configuration. 
...
## wait till the ASCS is been moved to the ERS host
