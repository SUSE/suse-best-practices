...
rules. Concrete it is an anti-collocation of {SAPHANA} promoted vs. {SAPHANA} non-replicated. This means
if the primary {SAPHANA} system (like HA1) fails the anti-collocation rules for the {SAPHANA} non-replicated system (like QAS) are triggered
...
{SAP} documentation like "Using Secondary Servers for Non-Productive systems", ↑SAP HANA Master
Guide.
...
_<<Administration>>_ section of this document we describe how to 'migrate' the primary to the secondary site
...
are able to integrate {hana} system replications in their
...
high availability solution will also benefit from this development of SUSE.
...
* Name resolution of the cluster nodes and the virtual IP address must be done locally on all cluster nodes. If that is not possible, additional measures are need to ensure reliable resolution of hostnames at any time.
...
{prodnr} {prodsp} systems. In addtion a non-replicated {SAPHANA} is installed and
added to the cluster control.

.Cluster with {hana} SR - performance optimized
image::hana_sr_scaleup_costopt.svg[scaledwidth=100.0%]
...
==== Implementing SAPHanaSR Hook for srPostTakeover
...
class srTakeover(HADRBase):
  ...
  def startup(self, hostname, storage_partition, system_replication_mode, **kwargs):
      self.tracer.debug("called startup hook; %s" % locals())
      return 0

  def shutdown(self, hostname, storage_partition, system_replication_mode, **kwargs):
      self.tracer.debug("called shutdown hook; %s" % locals())
      return 0

  def failover(self, hostname, storage_partition, system_replication_mode, **kwargs):
      self.tracer.debug("called failover hook; %s" % locals())
      return 0

  def stonith(self, failingHost, **kwargs):
      self.tracer.debug("called stonith hook; %s" % locals())
      return 0
...
In the same directory (/hana/shared/srHook) you need to install some python files from the
...
the productive {HANA} database secondary and check if the parameters are working.
