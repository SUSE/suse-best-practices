...
ControlZone platform and UI resources _rsc_cz_{mySid}_ and _rsc_ui_{mySid}_ are grouped
with filesystem _rsc_fs_{mySid}_ and IP address resource _rsc_ip_{mySid}_ into group
_grp_cz_{mySid}_. The filesystem starts first, then platform, IP address starts before
