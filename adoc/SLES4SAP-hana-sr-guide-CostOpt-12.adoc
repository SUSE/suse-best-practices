...
.{testExpect}
 * Primary restarts on siteA (PREFER_SITE_TAKEOVER=false) till failcount >= migration-threshold
 * If takeover occurs 
 ** non-replicated database is stopped on node 2 (siteB) 
 ** Secondary is Promoted as Primary

.{testRecover}
 * No recovery needed, if no takeover did occur
 * Recovery after takeover:
. Register siteA to siteB
. Resource cleanup for siteA
==========
...
.{testExpect}
 * Primary restarts on siteB (PREFER_SITE_TAKEOVER=false) till failcount >= migration-threshold
 * Non-replicated database still stopped on node 2 (siteB)
 * If takeover occurs 
 ** Secondary is Promoted as Primary
 ** non-replicated database is started on node 2 (siteB) 
...
.{testDescr}
 - Kill Primary on siteA (node 1)

.{testProc}
. Kill (send signal to) the {hana} database
+
[subs="attributes,quotes"]
----
{sapnode1}# HDB kill-9
...
.{testExpect}
 - Primary restarts on siteA (PREFER_SITE_TAKEOVER==false) till failcount >= migration-threshold
 - If takeover occurs: Non-replicated database is stopped on node 2 (siteB) and Secondary is Promoted as Primary

.{testRecover}
 - No recovery needed if no takeover did occur
 - Recovery after takeover:
. Register siteA to siteB
. Register cleanup for siteA
==========
...
.{testExpect}
 - Cluster takeover to siteA
 - Non-replicated database not available (no takeover to siteA)
...
.{testDescr}
 - Stop Secondary Database on node 2 (is siteB)

.{testProc}
. Stop the secondary {hana} database
+
[subs="attributes,quotes"]
----
{sapnode2}# HDB stop
...
.{testExpect}
 - Cluster restarts Secondary on node 2 (siteB)
 - non-replicated database not affected on node 2 (siteB)
...
.{testDescr}
 - Crash Secondary Database on node 2 (is siteB)

.{testProc}
. Kill (send signal to) the secondary {hana} database
+
[subs="attributes,quotes"]
----
{sapnode2}# HDB kill-9
...
.{testExpect}
 - No takeover of node 2 resources to siteA
 - Non-replicated database not available (no takeover to siteA)
...
.{testDescr}
 - Stop non-replicated database node 2 (is siteB)
...
.{testExpect}
 - Cluster restart non-replicated database on node 2 (siteB)
...
.{testDescr}
 - Crash non-replicated database on node 2 (is siteB)

.{testProc}
. Kill (send signal to) the non-replicated {hana} database
+
[subs="attributes,quotes"]
----
{sapnode2}# HDB kill-9
