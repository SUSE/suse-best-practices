...
is configured to preload the tables the take-over time is typically very short.
...
start the take-over process. This allows to use the already loaded data at the
secondary site. Typically the take-over is much faster than the local restart.
...
* Both sites are either in the same network segment (layer 2) to allow an easy
  take-over of an IP Address, or you need a technique like overlay IP addresses
...
[IMPORTANT]
====
Automated registration of a failed primary after take-over is possible. But as a
...
In this case, you need to register a failed primary after a take-over manually.
...
Until performing a take-over and re-registration in the opposite direction, the
...
intention, this could trigger a take-over.
