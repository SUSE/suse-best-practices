...
Depending on your starting point, it is possible to extend a two-node cluster setup or directly start with
...
This example is only one method to create a configuration backup.
...
With SWPM 2.0, SAP provides a new option which can perform all necessary steps to prepare a fresh
