...
* For scale-up, the current resource agent supports {HANA} in system replication
...
* The {HANA} Fast Restart feature on RAM-tmfps as well as {HANA} on persistent
...
parallel to susChkSrv.py, if not documented contradictingly.
...
* The hook script runs in the {HANA} nameserver. It runs on the node where the
...
comes with {HANA} has to be used.

See also manual pages SAPHanaSR(7), SAPHanaSR.py(7), susTkOver.py(7) and
...
the product documentation of {sles4sap} in section _Setting Up an {SAPHANA} Cluster_
...
`srConnectionChanged()`, the related {SUSE} hook script is `SAPHanaSR.py`.
The hook script SAPHanaSR.py is defacto mandatory.
...
The hook script SAPHanaSR.py is needs no config parameters. 
...
the HA/DR hook scripts during start. Alternatively, `SAPHanaSR-manageProfile`
might be used for adapting the global.ini. See manual page SAPHanaSR-manageProfile(8)
for details.
...
.Adding susTkOver via global.ini
==========
Use the {HANA} tools for changing global.ini.
// TODO PRIO2: use official HANA python tool for changing global.ini
...
Use the hook from the SAPHanaSR package /usr/share/SAPHanaSR/susChkSrv.py.
The hook must be configured on all {HANA} cluster nodes.
In global.ini, the section `[ha_dr_provider_suschksrv]` needs to be created.
The section `[trace]` might be adapted.
Refer to the man page susChkSrv.py(7) for details on this HA/DR provider hook
script, see also SAPHanaSR-manageProvider(8).
...
.Adding susChkSrv via global.ini
==========
Use the {HANA} tools for changing global.ini.
// TODO PRIO2: use official HANA python tool for changing global.ini
...
Check for script susTkOver.py, if it is loaded and initialized correctly:
...
Check for script susChkSrv.py, if it is loaded and initialized correctly:
...
As user {refsidadm} check the {SAPHANA} tracefiles, if SAPHanaSR.py did successfully interact with the cluster or created a fallback file.
...
As user {refsidadm} check the {SAPHANA} tracefiles, if susTkOver.py did successfully blocked or permitted the takeover request.
...
of the {sleha} Extension Administration Guide, chapter _Upgrading Your Cluster and Updating Software Packages_ at
