...
As an example, here how a full SAP Application media tree (in a compressed format) for a S/4HANA install would look like:
----
../s4hana1809
      ├SWPM_CD
...
A Terraform template is provided. For a demo environment, this will need around
ifeval::[ "{cloud}" == "Azure" ]
{azure_par_count} parameters modifying.
endif::[]
ifeval::[ "{cloud}" == "AWS" ]
{aws_par_count} parameters modifying.
endif::[]
ifeval::[ "{cloud}" == "GCP" ]
{gcp_par_count} parameters modifying.
endif::[]
