...
Go to the Maintenance Planner at https://apps.support.sap.com/sap/support/mp published on SAP site and generate a Stack XML file with the container image definitions of the SAP Data Intelligence release that you want to install. Download the Stack XML file to a local directory.
...
|===
See SAP documenation here for details on input parameters.
...
----
 kubectl -n di310 get  -o jsonpath=$'{.spec.serviceAccountName}\n' pod/default-4wtmgwe-backup-hana-zbdlc
----
create secret

----
kubectl -ndi310 create secret docker-registry pull-secret --docker-server="lu0591.wdf.sap.corp:5000" --docker-username=regadmin --docker-password=datahub4SAP
----

patch sa

----
kubectl -ndi310 patch serviceaccount <service account> -p '{"imagePullSecrets": [{"name": "pull-secret"}]}'
----
