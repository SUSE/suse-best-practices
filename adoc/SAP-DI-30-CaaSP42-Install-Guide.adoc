...
TIP: This step is optional if you already have a private secure Docker registry. If you do skip this chapter, follow the instructions at <<SUSE Enterprise Storage>>

To satisfy SAP Data Intelligence 3 requirements you also need a Docker Registry.
The easiest way to build and manage it comes with the project Harbor.

** link:https://goharbor.io/[Harbor]

First you need to create a dedicated server for your Docker registry and Harbor stack.

WARNING: As docker does only allow characters within [a-z],[A-Z],[0-9] and '-' for domain names, make sure your FQDN does not contain other characters.

In our example the server will be connected to a local bridge which provides common services (DNS, SMT, Docker-registry) for the SAP Data Intelligence stack.
The FQDN of this server will be *Harbor-Registry.example.com*.

=== Prerequistes

The prerequisites for Harbor can be taken from link:https://goharbor.io/docs/2.1.0/install-config/installation-prereqs/

According to this, you'll need to install Docker and Docker Compose before you can setup Harbor.
...
To install Docker Compose, you'll need to download the executable from the GitHub repository and put it into a directory within you $PATH.
...
The next steps are about to generate the certificates used to make Harbor secure, which can also be found at link:https://goharbor.io/docs/2.1.0/install-config/configure-https/[Configure HTTPS Access to Harbor].

First step is to generate a CA certificate private key.

----
# openssl genrsa -out ca.key 4096
----

Then generate a certificate with the given key for your domain. For all further steps, replace <FQDN> with your fully qualified domain name. In our example this would be *Harbor-Registry.example.com*
----
...
Next, you'll need to generate a server certificate, which can be done by running:
...
Copy the .crt and .key file to the systems certificate directory:
...
As Docker interprets .crt files as CA certificates and .cert files as clients, you'll need to convert your .crt:

----
# openssl x509 -inform PEM -in <FQDN>.crt -out $<FQDN>.cert
----

You can now copy the create certificates to your docker certificate folder. If your folder '/etc/docker/certs.d' does not exist, create it.
...
INFO: If you want to expose your registry on a different port then 443, you'll need to create a folder '/etc/docker/certs.d/<FQDN>:<Port>' and copy the certs to this directory instead.

In order to introduce the certificates to Docker, you'll need to restart the docker daemon:
...
Within this directory you should find a file called 'harbor.yml.tmpl'. It holds the configuration for the Harbor Registry and needs to be adjusted.
First, you'll need to edit the field *hostname* and enter your FQDN.
Next is the *https* configuration. The subentries *certificate* and *private_key* need to be adjusted, so they point to the .crt and .key file you created in <<Prerequistes>>.
This then my look like:

image::harbor_yml.png[Harbor Configuration File.png,480,640]

If you want to expose registry on a different port than 443, you can change the subentry *port* to match your desired port.

Further you can change the admin password for Harbor which is defined at the field *harbor_admin_password: Harbor12345* by default.

Last mentionable filed is *data_volume: /data* which defines where all your Harbor data is goind to. If you want Harbor to store the data somewhere else, enter the path to the desired directory here.

When done save your changes and rename the file by removing the suffix *.tmpl*:

----
# mv harbor.yml.tmpl harbor.yml
----

AS Harbor uses nginx as a reverse proxy for all services, you'll need to run the prepare script to configure it correctly:
...
Distribute the CA certificate to all Kubernetes nodes, so they can access the registry.
Therefore you may run following commands on the Kubernetes nodes:
...
=== Verify configuration and setup Harbor projects

To verify Harbor is running, you may access its Web-Frontend by calling link:https://<FQDN>[] from your browser.
...
You're now redirected to the login page of your Harbor registry.

image::harbor_login.png[Harbor Login,480,640]

Enter the admin as the Username and enter the password specified in the 'harbor.yml' (By default this is *Harbor12345*).
You're now redirected to the projects page which holds the *libraries* project by default.

image::harbor_projects.png[Harbor Projects,480,640]

// TODO projects aufsetzen

You may further check if the docker clients on your Kubernetes nodes are able to access the registry.
Therefore run:
...
You will be prompted to enter a username a password.
Use *admin* for the username and the password you set in your 'harbor.yml' (default: *Harbor12345*).

If docker is able to access the registry, you will see a line like "Login Succeeded".


If your machines are not able to resolve the FQDN of your registry, edit your '/etc/hosts' and add a line like:

----
<IP>  <FQDN>  <Hostname>
----

In our example this then looks like this:
...
* in case selfsigned ssl certificates are used create a secret for the secure private registry follow this procedure:

** The certificate chain should be saved in pem format into one file called cert.
** export DI installation namespace into variable NAMESPACE
...
image::SAP-DI-30-CaaSP42-0030.png[title="Installation #1",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0031.png[title="Installation #2",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0032.png[title="Installation #3",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0033.png[title="Installation #4",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0034.png[title="Installation #5",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0035.png[title="Installation #6",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0036.png[title="Installation #7",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0037.png[title="Installation #8",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0038.png[title="Installation #9",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0039.png[title="Installation #10",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0040.png[title="Installation #11",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0041.png[title="Installation #12",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0042.png[title="Installation #13",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0043.png[title="Installation #14",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0044.png[title="Installation #15",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0045.png[title="Installation #16",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0046.png[title="Installation #17",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0047.png[title="Installation #18",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0048.png[title="Installation #19",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0049.png[title="Installation #20",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0050.png[title="Installation #21",480,640,scaledwidth=80%,align=center]
image::SAP-DI-30-CaaSP42-0051.png[title="Installation #22",480,640,scaledwidth=80%,align=center]
...
image::SAP-DI-30-CaaSP42-0054.png[title="Installation #25",480,640,scaledwidth=80%,align=center]
