= SAP HANA High Availability Cross-Zone solution on SUSE Linux Enterprise Server for SAP Applications
...
For details, refer to SAP Help Portal HANA system replication: 
...
SUSE High Availability Extension (HAE) is a high availability solution based on Corosync and Pacemaker. With SUSE Linux Enterprise Server for SAP Applications, SUSE provides SAP specific Resource Agents (SAPHana, SAPHanaTopology etc.) used by Pacemaker to help users to buildup the SAP HANA HA solution more effectively.

For details, refer to latest version of SAP HANA SR Performance Optimized Scenario at SUSE documentation center:  

https://www.suse.com/documentation/suse-best-practices.


=== Architecture Overview
This document guides you on how to deploy an SAP HANA HA solution cross different Zones. Following is a brief architecture:

. HAE of SUSE Linux Enterprise Server for SAP Applications is used to set up the HA Cluster;
...
In this example, run following two commands on boths sites:
...
First of all, create an NAT Gateway attached to the given VPC; In our example, we create an NAT Gateway named suse_hana_ha_GW as follows:

image::Alicloud_NAT_GW.png[NATGW,width="53%"]

After creating the NAT Gateway, you need to create a corresponding SNAT entry to allow ECS instances within the VPC can access public address on Internet.
...
For an HA solution, a fencing device is a must. Alibaba Cloud provides its own STONITH device, which allows the servers in the HA cluster to shut down the other node which is not responsive. The STONITH device leverages Alibaba Cloud OpenAPI underneath the ECS instance, which is similar to a physical reset / shutdown on an on-premise environment.
...
=== HAE installation
Both ECS instances are created with the SUSE Linux Enterprise Server for SAP Applications image. Both ECS instances should install the HAE component, and the package SAPHanaSR. In this example, we install HAE (major software component: Corosync and Pacemaker), and SAPHanaSR on both ECS instances as follows:
...
Install SAP HANA software on both ECS instances, and make sure the SAP HANA SID and Instance Number are the same (requirement by SAP HANA System Replication). It is recommended to use hdblcm to do the installation. For details refer to SAP HANA Server Installation and Update Guide: https://help.sap.com/viewer/2c1988d620e04368aa4103bf26f17727/2.0.03/en-US.
...
When you have finished HANA installation with hdblcm as mentioned above, the SAP Host Agent should already be installed on your server. To install it manually, refer to Installing SAP Host Agent Manually: https://help.sap.com/saphelp_nw73ehp1/helpdata/en/8b/92b1cf6d5f4a7eac40700295ea687f/content.htm?no_cache=true.
...
=== Backing up HANA on primary ECS instance
To do a backup on HANA, you can either use SAP HANA studio or hdbsql as the client command tool.
...
For HANA 2 with multitenant as default mode (You should back up systemDB and all tenantDB as shown below in our example):
...
In this example for HDB Studio, we execute the SAP HANA database backup on both ECS instances as follows:
...
**[HOSTNAME]** should be host name of the Secondary node;
...
Same as above for primary, but use IP and host name of primary node
...
After copy the corosync.conf to the secondary node, configure the bindnetaddr as above to the local heart beating IP address.

=== Configuration of pacemaker
For the SAP HANA HA solution, we need to configure 7 Resource Agents and corresponding constraints in Pacemaker.
...
This part defines an SAPHanaTopology RA, and a clone of SAPHanaTopology on both nodes in the cluster. Save following scripts in a file: crm-saphanatop.txt
```shell
...
This part defines an SAPHana RA, and a multi-state resource of SAPHana on both nodes in the cluster. Save following scripts in a file: crm-saphana.txt
```shell
...
Meanwhile, check if a new entry **[virtual_IP4_address]** is added into the route table of VPC.
