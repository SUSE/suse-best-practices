...
NOTE: If you do not want to create multiple imagePullSecrets, you can also add the application collection as a registry to your cluster. Therefore add *dp.apps.rancher.io* as a registry as described in <<rancherRegistries>>. If you do so, make sure to create an HTTP Basic Auth Secret with your access token or service account credentials.
endif::[]
