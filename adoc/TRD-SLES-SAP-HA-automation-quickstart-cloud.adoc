...
* Create a directory within the bucket.
* Upload the SAP media to the directory in the S3 bucket.
...
Finally, ensure the following lines are *commented out* using the hashtag sign `#`:
...
unattended, resulting in a failed deployment.  It is strongly advised to retain focus on the AWS Cloud Shell Window, perhaps pressing a key during the deployment to ensure the timeout does not occur.
