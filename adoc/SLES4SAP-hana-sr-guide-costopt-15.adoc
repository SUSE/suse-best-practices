...
\\ TODO PRIO1: check below version
** {hana} 2.0 SPS04 or later provides the needed HA/DR provider hook method
postTakeover().
...
integrate an already installed and working {hana} with System Replication.
...
The hook must be configured on the take-over node where usually the
{SAPHANA} stand-alone database and the system replication secondary is running.
Refer to the man page susCostOpt.py(7) for details on this HA/DR provider hook script.
