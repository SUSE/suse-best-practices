...
** In MDC configurations, the {SAPHANA} RDBMS is treated as a single system including
...
Another hook is called by {SAPHANA} after an SR take-over has happened. This method can
be used to remove the {SAPHANA} memory limit and enable table preload. 
...
The configuration for susCostOpt.py needs to be adapted
according to your specific system size and user keystore. 
{SAPHANA} must be stopped to change the global.ini and allow {SAPHANA} to integrate
the HA/DR hook scripts during start.
