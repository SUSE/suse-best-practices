...
Use the check list in the appendix to note down all information needed
...
Expected:
     ...
     6. After the manual register and resource cleanup the system replication
        pair is marked as in synchronization (SOK).
...
Expected:
     ...
     4. The cluster detects that the system replication is in synchronization again and
        marks it as ok (SOK).
...
Expected:
     ...
     6. The cluster detects that the system replication is in synchronization again and
        marks it as ok (SOK).
...
Procedure 7.3: Migrating a HANA primary using cluster migration commands
 ...
 1. Create a migrate away from this node rule.
    ...
    ** Because we used the migrate away rule, the cluster will stop the current
    ...
    was in synchronization before. You should not migrate the primary, if the status of
    the system replication is not in synchronization (SFAIL).
