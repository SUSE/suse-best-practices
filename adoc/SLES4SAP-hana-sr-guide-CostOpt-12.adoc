...
// TODO PRIO1: Check for the bugzilla entry regarding CostOpt12 - check, if the solution does work with SAP HANA 2.0
...
// TODO PRIO1: Name minimum permissions of database user!!

.Installing {haDrCostOptMemPy}
========
Copy example, adapt to your environment and set restrictive file permissions
[subs="specialchars,attributes"]
----
...
to enable the hook to run the database connect and SQL queries. You need the following files: dbapi.py, __init__.py, resultrow.py
...
Enable use of {haDrCostOptMem} by adpating global.ini on node 2
[subs="specialchars,attributes"]
----
...
After changing the global.ini on node 2 and implementing {haDrCostOptMem} hook you should start
the productive {HANA} secondary database and check if the parameters are working.

As user {refsidadm} on node 2 you might do
[subs="specialchars,attributes"]
----
{sapnode2} > cdtrace
