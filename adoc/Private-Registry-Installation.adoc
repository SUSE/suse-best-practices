...
it is necessary to configure traefik to not verify the backend SSL certificate (`insecureSkipVerify = true` option). Learn how to modify traefik settings in link:https://rancher.com/docs/k3s/latest/en/helm/#customizing-packaged-components-with-helmchartconfig[upstream documentation].
