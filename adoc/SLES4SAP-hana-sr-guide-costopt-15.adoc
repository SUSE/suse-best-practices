...
// TODO PRIO3: Scale-Up and Handshake take-over (scheduled for 'after next' publishing)
...
is configured to pre-load the tables, the take-over time is typically very
short.
+
...
stand-alone non-replicated {hana} RDBMS system (like QAS or TST). Whenever a take-over
...
take-over needs longer than in the performance optimized use case.
+
...
take-over occurs. This is why multi SID (also MCOS) must not be used in this scenario.
...
primary node crashed, the take-over process will be triggered.

In case of a take-over, the secondary (replica) of this {SAPHANA} on node 2 is promoted *after* the
...
to try to restart {SAPHANA} locally first, as a take-over with non-preloaded tables can
...
The take-over to the secondary site takes up a lot of time, because the non-replicated database needs to
be stopped gracefully prior to take-over the productive database. This extended take-over time
...
NOTE: If you want to achieve a very fast take-over, the performance optimized scenario is the better option.
...
The cluster *only allows a take-over* to the secondary site if the {hana}
...
{saphana} resources must never run on that third node. In this case the third
...
** For disk-less SBD, you need at least three cluster nodes. The disk-less SBD
mechanism has the benefit that you do not need a shared drive for fencing.
Since disk-less SBD is based on self-fencing, reliable detection of lost quorum
...
* Technical users and groups, such as _<sid>adm_ are defined locally in the Linux
system. If that is not possible, additional measures are needed to ensure reliable
...
recommendations.
* Automated registration of a failed primary after take-over prerequisites need
to be defined.
...
default. In this case, you need to register a failed primary after a take-over
manually. For re-registration, use precisely the site names that are already known
by the cluster. Use SAP tools like {saphana} cockpit or _hdbnsutil_.
** For optimal automation, we recommend to set `AUTOMATED_REGISTER="true"`.
** The cluster automates one single take-over in case of a failed primary. After
...
(performance based, cost optimized and multi-tier).
** In MDC configurations, the SAP HANA RDBMS is treated as a single system including
all database containers. Therefore cluster take-over decisions are based on the
...
* {saphana} 2.0 SPS04 or later provides the HA/DR provider hook method
...
scale-up package.
* {saphana} 2.0 SPS04 or later provides the HA/DR provider hook method postTakeover().
// TODO PRIO1: check above version
* The user {refsidadm} needs execution permission as user root for the command
crm_attribute.

Additional considerations for the {saphana} version are:

* Particularly the {HANA} system replication cost-optimized scenario can benefit
from {HANA}'s persistent memory features. Local restart and take-over are affected
...
for the secondary {saphana} database which has table preload inactive. Even for
tests the requirements can increase, depending on your test scenario.
...
- 1 additional IP address for take-over
...
[[parameter-sheet]]
Even if the setup of the cluster organizing two {saphana} sites is quite simple,
...
|SBD ||STONITH device (two for production) or disk-less SBD
...
|Database user key| `saphanasr_{sapsid}\ _costopt` |Database user key in {refsidadm}`s keystore
...
|SBD |`{sapsbd1}` |STONITH device (two for production) or disk-less
...
Then the {saphana} database including the system replication is set up. Finally
...
described in the _Deployment Guide_ for SUSE Linux Enterprise Server
({deploymentGuide15}).
...
that fit all requirements for {sapHana} are available from the SAP notes:
...
With {sles4sap}, {SUSE} delivers special resource agents for {sapHana}. With the
pattern _sap-hana_, the resource agent for {sapHana} *scale-up* is installed. For
...
are installed on all cluster nodes. If not, install and enable it now.

As Linux user _root_ use the command _systemctl_ and _systemd-cgls_ to check
...
As Linux user _root_ use the commands _systemctl_ and _saphostctrl_
...
As Linux user _{refsidadm}_ use the command line tool _HDB_ to get an overview
of running HANA processes. The output of `HDB` `info` should be similar to the
output shown below:
...
IMPORTANT: Without a valid backup, you can not bring {hana} into a system
replication configuration.

=== Enabling the primary node

As Linux user _<sid>adm_, enable the system replication at the primary node. You
...
*not supported with automated cluster take-over*. The mode depends on the
...
=== Manually testing the {HANA} SR take-over

Before you integrate your {HANA} system replication into the HA cluster, it is
mandatory to do a manual take-over. Testing without the cluster helps to make
sure that basic operation (take-over and registration) is working as expected.
...
out of sync. The hook is called by {SAPHANA} using the HA/DR provider interface
in point-of-time when the secondary gets out of sync. This is typically the case
when the first commit pending is released. The hook is called by {SAPHANA} again
when the system replication is back. This HA/DR provider is srConnectionChanged(),
the related {SUSE} hook script is SAPHanaSR.py.

Another hook is called by {SAPHANA} after an SR take-over has happened. This can
be used to remove the {SAPHANA} memory limit and enable table preload. This HA/DR
provider is postTakeover(), the related {SUSE} hook script is susCostOpt.py.


**Procedure**

. Implement the python hook script SAPHanaSR.py on both sites.
. Implement the python hook script susCostOpt.py on fail-over site.
. Configure system replication operation mode.
. Allow {refsidadm} to access the cluster on both sites.
. Create a database user key in {refsidadm}´s keystore on fail-over site.
. Test the hook integration.

// TODO PRIO2: Steps "Start" and "Test" are incomplete

This will implement two {hana} HA/DR provider hook scripts. SAPHanaSR.py needs
no config parameters. The configuration for susCostOpt.py needs to be adapted
according to your specific system size and user keystore. {SAPHANA} must be
stopped to change the global.ini and allow {SAPHANA} to integrate the HA/DR hook
scripts during start.
...
The hook must be configured on all {SAPHANA} cluster nodes.
In global.ini, the section `[ha_dr_provider_saphanasr]` needs to be created.
The section `[trace]` might be adapted.
Refer to the man page SAPHanaSR.py(7) for details on this HA/DR provider hook
script.
...
The hook must be configured on the take-over node where usually the {SAPHANA}
...
need be adapted.
Refer to the man page susCostOpt.py(7) for details on this HA/DR provider hook
script.
...
Until a take-over and re-registration in the opposite direction, the entry for the
...
site in the {saphana} system replication a hot standby system.
...
The current version of the SAPHanaSR python hook uses the command `sudo` to
allow the {refsidadm} user to access the cluster attributes.
...
==========
Basic sudoers entry to allow {refsidadm} to use the srHook.
...
In our example, we have four separate lines with Cmnd_Alias entries, one line
for the {refsidadm} user and one or more lines for comments. In the document at
...
The user keystore has to be created for the {refsidadm} of the replicating database. 
The keystore has to be created on node 2, where the replication secondary database
is running. 
...
----
# su - {refsidadm}
~> hdbuserstore SET saphanasr_{refsid}_costopt localhost:31013 SYSTEM {mySapPwd} 
~> hdbuserstore LIST saphanasr_{refsid}_costopt
----
==========
...
Please refer to {SAPHANA} documentation for further information on database
...
Please see also manual pages SAPHanaSR.py(7) and susCostOpt.py(7).
...
is part of {sles4sap}, and the {hana} Database integration.
...
If you use the storage-based fencing (SBD) mechanism (disk-less or disk-based),
you must also configure a watchdog. The watchdog is needed to reset a node if
the system cannot longer access the SBD (disk-less or disk-based).
...
// |SBD_WATCHDOG="yes" a|
Use watchdog. It is mandatory to use a watchdog. SBD does not work reliable
without watchdog. Refer to the {slsa} manual and SUSE TIDs 7016880 for setting
up a watchdog.
...
This section is only relevant if the recommended disk-based or disk-less SBD fencing is not used.
...
Please see the {sleHA} product documentation for details
({haAdminGuide15}/#pro-ha-storage-protect-fencing).
...
.{testRecover}
 * No recovery needed, if no take-over did occur
 * Recovery after take-over:
...
.{testExpect}
 - Cluster take-over to site A
 - Non-replicated database not available (no take-over to site A)
...
.{testExpect}
 - No take-over of node 2 resources to site A
 - Non-replicated database not available (no take-over to site A)
...
* Using {SAP} tools for attempting start/stop/take-over actions on a database
...
You can use the High Availability Web Console (HAWK), {hana} Studio and different
...
Before trying start/stop/take-over for the database, make sure the cluster is not
...
The cluster did jump in and initiated a take-over. At 20:15 (CET) the former secondary
...
down by intention, this could trigger a take-over.
...
// TODO PRIO2: Check for take-over with handshake
...
// TODO PRIO2: Check -sr_take-over with 'handshake'
...
* Start the take-over process on the secondary SAP HANA database system.
Enter the command in our example on node2 as user _<sid>adm_.
+
[subs="specialchars,attributes,quotes"]
----
~> hdbnsutil -sr_take-over
----
+
...
+
IMPORTANT: Migration without the *force* option will cause a take-over without the
...
==== Reverting to original {HANA} roles after take-over to secondary site
...
replication was in sync, process the take-over on the secondary site.
...
It makes sure to reconfigure the primary database after a take-over.
