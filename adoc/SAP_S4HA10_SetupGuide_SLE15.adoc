...
The here described HA setup is based on cluster-controlled filesystems for the
...
// DONE PRIO1: fixed simple-mount URL
...
- Use of native systemd integration for {sap} hostagent and instance´s sapstartsrv.
  Please refer to {sap} documentation for the neccessary  product version, see also SAP note 3139184.
  SUSE systemd version 234 at least is needed. For details please refer to the {sles4sap} product documentation.
...
----
The _sapservices_ file is still there fo compatibility. It shows native systemd commands, one per
