...
+++**+++ Only needed to set up {rancher} in a high availability setup
...
NOTE: The installation of {reg} is not a mandatory requirement for the deployment of {eic} and is thereby optional. To continue without using a {reg}, refer to the next chapter <<preparingStorage>>.

include::SAP-Edge-Registry.adoc[tags=prerequisites;deploy;prepare-registry;elm-registry-project;use-elm]
...
We alternatively describe how you could use a {hana} in favor of {pg} and/or {redis}.
...
Before getting started to prepare you {hana} for the usage with {eic}, enusre the requirements described at
https://me.sap.com/notes/3247839 are met.

include::SAP-EIC-HANA.adoc[leveloffset=2]
...
This section provides instructions for configuring Restricted Access option for {eic}.
...
This section guides you through configuring the {istio} Service Mesh for the Restricted Access option on {eic}.
...
Since you have already initiated, paused, and resumed the deployment wizard in the {elm} UI to apply the specific manifests (as described in the previous section), the core deployment step is already complete. You should continue following the official SAP guide to verify the installation and perform any necessary post-deployment configurations.
====
...
=== Legacy Restricted Access Configuration (EIC < 8.41)

include::SAP-EIC-General.adoc[tags=legacyRestrictedAccess]
