...
* Deploy infrastructure - including VPC, subnet, firewall rules etc.
...
The following steps show the minimum steps required to prepare GCP infrastructure to host the SAP HANA environment:
...
. Using the GCP cloud Shell, create a new GCP Key for the default GCP Service Account. The key will be used by Terraform to access the GCP infrastructure. Following is an example to create a new GCP Service Account Key:
+
...
+
TIP: For the sake of simplification, the default GCP Service Account will be used in this guide.
// $ gcloud iam service-accounts keys create sbp-key --iam-account=1035353367007-compute@developer.gserviceaccount.com
//created key [3e7e726f1e061095f119a9940205c5ce2346719a] of type [json] as [sbp-key] for [1035353367007-compute@developer.gserviceaccount.com]
endif::[]
...
+
image::TRD_SLES-SAP-HA-automation-quickstart-cloud-gcp-bucket.png[width=75%]
endif::[]
...
TIP: If SSH keys already exist, the next step can be skipped.
...
$ cd suse-sap-automation
$ 
$ ssh-keygen -q -t rsa -N '' -f hana-nodes-key
----
...
A template is provided. For a demo environment, this will need around 13 parameters modifying.
...
Then, choose the GCP Service Account Key file path:
----
# Credentials file for GCP
gcp_credentials_file = "<GCP Service Account Key>"
----
With this parameter, Terraform will use the GCP Service Account Key file created above.
endif::[]
...
If successful, the output will be the public IP addresses for the cluster nodes will be similar to the following:
+
-----
