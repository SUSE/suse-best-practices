...
Please look at the scenarios and prerequisites section below or
manual pages SAPHanaSR(7) and SAPHanaSR.py(7) for details.
...
to be defined.
** As a good starting configuration for projects, we recommend to switch off the automated 
...
(performance-optimized, cost-optimized, multi-tier, mulit-target and read-enabled).
...
// you stop a tenant.
////
* Only one system replication between the two {sap} HANA database in the Linux
...
** See also the dedicated section on requirements for SAPHanaSR.py.
* For scale-up, the current resource agent supports SAP HANA in system replication
beginning with HANA version 1.0 SPS 7 patch level 70, recommended is SPS 11. With
HANA 2.0 SPS04 and later multi-target system replication is possible as well. Even
...
// use the same style.
////
* The RA's monitoring operations have to be active.
...
scoring finally depends on system replication status an must not be over-ruled by
additional constraints. Thus it is not allowed to define rules forcing a SAPHanaController
or SAPHana master to follow another resource.
* Reliable access to the /hana/shared/ filesystem is crucial for HANA and the
Linux cluster.
* HANA feature Secondary Time Travel is not supported.
* The {sap} HANA Fast Restart feature on RAM-tmfps as well as HANA on persistent
memory can be used, as long as they are transparent to Linux HA.

// TODO PRIO3: align with man pages SAPHanaSR(7) and SAPHanaSR.py(7)
...
scale-up package.
* {saphana} 2.0 SPS04 or later provides the HA/DR provider hook method preTakeover().
...
used with {sles4sap} {prodNr} SP1 or newer.
...
Another hook is called by {SAPHANA} before an SR take-over is processed. This method can 
be used to block a manual take-over during normal cluster operation.
...
The default for the parameter sustkover_timeout is set to 30 seconds and is good 
...
Use the hook from the SAPHanaSR package /usr/share/SAPHanaSR/susTkOver.py.
The hook must be configured on all {SAPHANA} cluster nodes.
In global.ini, the section `[ha_dr_provider_sustkover]` needs to be created.
The optinal section [trace]` needs be adapted.
Refer to the man page susTkOver.py(7) for details on this HA/DR provider hook
script.
...
// "sudo /usr/sbin/SAPHanaSR-hookHelper *"
...
{refsidadm} ALL=(ALL) NOPASSWD: /usr/sbin/SAPHanaSR-hookHelper *
----
...
Cmnd_Alias HOOK_HELPER  = /usr/sbin/SAPHanaSR-hookHelper *
...
Start {SAPHANA} as user {refsidadm}.
...
As user {refsidadm} check the {SAPHANA} tracefiles, if the HA/DR provider scripts are loaded successfully and are called successfully during an
{SAPHANA} event.

Check for script SAPHanaSR.py, if it is loaded and initialized correctly:
...
Check for script susTkOver.py, it it is loaded and initialized correctly:
...
As user {refsidadm} check the {SAPHANA} tracefiles, if SAPHanaSR.py did succesffuly interact with the cluster or created a fallback file.
...
After an event has been processed by the HA/DR provider scripts, check for the correct behaviour: 

To test script susTkOver.py with stopped cluster, procedure <<cha.s4s.testtakeover>> can be used here again. While the cluster is not set up
at this moment the takeover will not be blocked.

As user root check the system messages, if the sudo permissions for susTkOver.py calling SAPHanaSR-hookHelper are set successfully.
...
As user {refsidadm} check the {SAPHANA} tracefiles, if susTkOver.py did succesffuly blocked or permitted the take-over request.

----
 ~> cdtrace
# check for permitted take-over requests
grep susTkOver.preTakeover.*permit nameserver_*.trc
# check for blocked take-over requests
...
If you set `stonith-watchdog-timeout` to a negative value, Pacemaker will automatically calculate this timeout and set it to twice the value of `SBD_WATCHDOG_TIMEOUT` starting with {sleha} {prodNr}.
...
The multi state resource was added to the cluster with in maintenance mode. To get the resource operated
by the cluster the maintenance must be ended by the command:

[subs="attributes,quotes"]
----
# crm resource refresh msl_SAPHana_{sapsid}_HDB{sapino}
# cs_wait_for_idle -s 5
# crm resource maintenance msl_SAPHana_{sapsid}_HDB{sapino} off
----

Remark: The command `cs_wait_for_idle` is part of the package `ClusterTools2`.
...
[subs="attributes,quotes"]
----
# crm resource refresh rsc_SAPHana_{sapsid}_HDB{sapino} {sapnode1}
----
...
[subs="attributes,quotes"]
----
# crm resource refresh rsc_SAPHana_{sapsid}_HDB{sapino} {sapnode2}
----
...
==== Test: Crash primary sode on site A (node 1)
...
//   - prepared for SAP native systemd support
// REVISION 1.5 2022/06
//   - susTkOver.py
//   - corosync example with two rings
