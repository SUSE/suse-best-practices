...
* using {SAP} tools for attempting start/stop/take-over actions on a database while the cluster is in charge of managing that database.

IMPORTANT: As "migrating" or "moving" resources in crm-shell, HAWK or other tools would add client-prefer location rules,
support is limited to maintenance procedures described in this document. See <<cluster-tests>> and
