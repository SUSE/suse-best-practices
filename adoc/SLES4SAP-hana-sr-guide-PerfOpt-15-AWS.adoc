...
= SAP HANA High Availability Cluster for the AWS Cloud - Setup Guide (SLES15 SP1)
...
On-premises users and clients, like SAP HANA Studio, will not be able to reach the Overlay IP address because the AWS Virtual Private Network (VPN) gateway is not able to route traffic to the Overlay IP address. To overcome this limitation refer to AWS' Overlay IP documentation and learn how to use native AWS services with the Overlay IP address for your on-premises clients and users:
...
To add a tag to an EC2 instance, refer to AWS Documentation:
