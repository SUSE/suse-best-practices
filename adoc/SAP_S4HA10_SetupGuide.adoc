...
More whitepapers, guides and best practices guides referring to SUSE Linux Enterprise Server and SAP can be 
...
To deliver urgent smaller fixes and important information in a timely manner, the TID for this document 
...
It focuses on the high availability of the central services. For {saphana} system replication,
refer to the guides for the performance- or cost-optimized scenario (see section "Additional
...
- The described new concept differs from the old stack with the master-slave architecture.
  ...
  This means: on one machine there is the ASCS with its own resources and on
  the other machine there is the ERS with its own resources.
...
To create the partitions, you could either use YaST or available command line tools. 
...
Check if the file /etc/hosts contains at least the following address resolutions.
