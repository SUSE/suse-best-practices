...
The {saphana} scale-out system replication provides for replication of all data in
...
For the latest SUSE product documentation updates, see http://www.suse.com/documentation.
...
For productive system ask your hardware vendor or use official SAP sizing
tools and services.
...
of a {sle} 12 High Availability Extension cluster. It gathers information about the statuses and
...
down without any {SAPHANA} standby nodes left, which could perform a host
...
* The setup implements the performance optimized scenario but not the cost
  optimized scenario.
* The {sapHostAgent} must be running. {sapHostAgent} is needed to translate between
...
In this document first {sles4sapAbbr} is installed and configured. Then the {saphana}
...
There are multiple installation guides already existing, and many reasons are available
to set up the server in a certain way. Below it is outlined where this information can be found.
And you will find important points to be considered to get a system which is well prepared to deliver {sapHana}.

==== Install Base Operating System

Depending on your infrastructure and hardware which is used, you need to adapt the installation.
...
{SUSE} delivers with {sles4sapAbbr} special resource agents for {sapHana}. With the pattern _sap-hana_ the resource
...
subscription for {sles4sapAbbr}. There are multiple ways to get updates via {SUSE} Manager,
SMT, or via a direct connection to the {SUSE} Customer Center (SCC).
...
points to a tool which can help you to take care of some settings. Manually verify each parameter mentioned in the SAP note.
...
INFO: Follow your company security policy to set up access to the systems.
...
After the _ssh-keygeer the _ssh-keygen_ is set up, you will have two new files under _/root/.ssh/_ .
...
After collecting all host keys store them in a file named _authorized_keys_. Push the complete
...
reboot-persistent. You can achieve this with entries in the _/etc/fstab_ .
...
In the example at hand, and to make it more easy to follow the documentation, the
...
as shown in the following screenshot for *both* sites:
+
...
Local System Rm Replication State
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.Result of replacg {refsidLC} wLC} with {mysidLc}
===================================
----
# SAPHanaSR-ScaleOut needs for srHook
...
IMPORTANT: Trigger the watchdog without continuously updating the watchdog does reset/switchoff the system.
...
For more detailed information about ha-cluster-* tools, see section _Overview of the Bootstrap Scripts_ in  https://www.suse.com/documentation/sle_ha/
...
the *firstt* cluster nodee *first* cluster node. Than all needed configuration files are copied over.
...
Check the cluster status with _crm_mon_. Use the option *-r* to also see
...
{sleha} Administration Guide (see https://www.suse.com/documentation/sle-ha-12/singlehtml/book_sleha/book_sleha.html#cha.ha.manual_config).
...
As user {refsidadm} create a file named for g.txt.
...
You need to potentially change the *SID* and *instance  number* (bold) to your values.
...
_man ocf_suse_SAPHana_Controller_.
...
Use the command _man ocf_heartbeat_IPAddr2_ for details on additional parameters.
...
is running. The system replication is in synchronization represented by 'SOK'.
The cluster is idle, no actions are pending, no migration constraints left over, no failcounts left over.

In this version of the setup guide a plain list of test cases is provided. A more detailed description of the
test cases is planned for future versions of this document. Either these details will be provided in an update of this guide
or the test cases will be extracted to a seperate test plan document.

=== Generic Cluster Tests

These kind of cluster tests cover the cluster reaction during operations. This includes starting and stopping
...
These kind of tests are checking the reaction on several failures of the primary site.
...
The tests listed here are checks about the {saphana} and cluster reactions triggered by application failures such as a terminated {saphana} instance.
...
These kind of tests are checking the reaction on several failures of the secondary site.
...
* Kill the {saphana} instance of any standby node.
* Kill _sapstartrv_ of any {saphana} instance of the secondary.
...
Be extremely careful with changing any parameter or the topology of the
...
For all actions which change the system replication it is recommended to first check
...
[subs="specialchars,attributes"]
----
HDBSettings.sh sh landscapeHostConfiguration.py
----
...
Following the SAP HA guideline, the SAP HANA resource agent interprets the
...
* 611361 Host names of SAP servers
...
* 1890444 Slow HANA system because of CPU power save mode
