...
SUSE Linux Enterprise Server for SAP Applications based
...
system replication was in synchronization until the point when the service of the primary
...
This step is mandatory to inform the cluster immediately if the secondary gets out of synchronization.
The hook is called by {SAPHANA} using the HA/DR provider interface in point-of-time when the secondary gets out of
synchronization. This is typically the case when the first commit pending is released. The hook is
...
cluster will not start this SAP HANA RDBMS until it is registerd to avoid
...
replication pair is marked as in synchronization (SOK).
...
.  The cluster restarts the secondary HANA database on the same node
(node 2).
.  The cluster detects that the system replication is in synchronization again
and marks it as ok (SOK).
.  The cluster "failed actions" are cleaned up after following the
recovery procedure.
==========
...
replication was in synchronization before. You should not migrate the primary if
the status of the system replication is not in synchronization (SFAIL).
+
...
replication was in synchronization, process the takeover on the secondary site.
