...
recovered, or if Pacemaker decides that the node requires fencing. Diskless SBD is based on “selffencing”
...
With SWPM 2.0 (SP27 or later), SAP provides a new option which can perform all necessary steps to prepare a fresh
...
- Create mount points and mount NFS shares on all nodes:
...
As this directory needs to be available at all time, make sure is is mounted during boot. This can be
...
# /usr/sap/hostctrl/exe/sapstartsrv pf=/usr/sap/<SID>/SYS/profile/HA1_ERS10_sapha1er -reg
# /usr/sap/hostctrl/exe/sapstartsrv pf=/usr/sap/<SID>/SYS/profile/HA1_ASCS00_sapha1as -reg
## or copy the file
...
# crm configure colocation ASCS00_ERS10_separated_EN2 -50000: grp_EN2_ERS10 grp_EN2_ASCS00
----
==========
