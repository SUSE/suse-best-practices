...
Please follow the SAP HANA specific best practices of the storage system vendor. 
...
The following parameters need to set in `sapconf` version 5. Edit the file `/etc/sysconfig/sapconf` to reflect the settings below, and then restart the `sapconf` service.
...
If you use `saptune`, please configure it accordingly:
...
Both `sapconf` and `saptune` apply their settings at boot time automatically and don't need to be included in the script above.
