...
for {HANA} scale-out system replication automation in the performance
optimized scenario.
...
{HANA} to a second {HANA} system. The {HANA} itself replicates all of
its data to a secondary {HANA} instance. It is an out-of-the-box, standard
feature.
...
When the {HANA} database has been restarted after the changes, check if the hook script is called correctly.
...
Further, the integration of {HANA} System Replication with the {sleha} cluster is explained. The integration is done
