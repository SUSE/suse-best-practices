...
:emeraldrapids: 5th Generation Intel Xeon Scalable Processor (Emerald Rapids)
...
// :anysupportedos: footnote:[Any OS that is generally supported with SUSE KVM and is supported for the use with the desired CPU platform and SAP HANA, may be used as guest OS.]
...
_Size:_ max. 4 socketsfootnote:max4sockets[Maximum 4 sockets using Intel standard chipsets on a single system board.], 3 TiB RAM
|
no
|
no
...
{cascadelake}
|
_Hypervisor:_ {sles4sapAbbr} 15 SP7 

_Guest:_ Any supported guest OSfootnote:anysupportedos[Any OS that is generally supported with SUSE KVM and is supported for the use with the desired CPU platform and SAP HANA, may be used as guest OS.]

_Size:_ max. 4 socketsfootnote:max4sockets[], 6 TiB RAM
|
_Hypervisor:_ {sles4sapAbbr} 15 SP7 

_Guest:_ Any supported guest OSfootnote:anysupportedos[]
...
_Size:_ max. 4 socketsfootnote:max4sockets[], 8 TiB RAM
|
_Hypervisor:_ {sles4sapAbbr} 15 SP7 
...
_Size:_ max. 2 socketsfootnote:max2sockets[Maximum 2 sockets using Intel standard chipsets on a single system board.], 4 TiB RAM
|
_Hypervisor:_ {sles4sapAbbr} 15 SP5 

_Guest:_ Any supported guest OSfootnote:anysupportedos[]

_Size:_ One VM per socket (max. 2 VMs per host)
|
no
|===
...
Reserve a minimum of about 8% of the hosts's main memory for the hypervisor.
...
Reserve at least 8% of the total memory reported by "`/proc/meminfo`" (in the "`MemTotal`" field) for the hypervisor.
...
| {emeraldrapids} | OLTP | 4 TiB / 4096 GiB | 2 | 64 | 32 GiB/core
| {emeraldrapids} | OLAP | 4 TiB / 4096 GiB | 2 | 64 | 32 GiB/core
|===
...
** kernel (Only major version 6.4, minimum package version 6.4.0-150700.53.31)
** libvirt (Only major version 11.0, minimum package version 11.0.0-150700.4.13.1)
** qemu (Only major version 9.2, minimum package version 9.2.4-150700.3.17.1)
...
* be a {sles} supported VM guest as per Section 7.4 "Supported guest operating systems" of the https://documentation.suse.com/sles/15-SP7/html/SLES-all/book-virtualization.html[SUSE Virtualization Guide].
* comply with KVM limits as per https://documentation.suse.com/sles/15-SP7/single-html/SLES-virtualization/#virt-hypervisors-limits["SUSE Linux Enterprise Server 15 SP7 Hypervisor Limits"].
...
In addition, it is also useful to install the `lstopo` tool which is part of the `hwloc` package available from the *HPC Module* for {sles4sap}.

[[_sec_configure_networking_on_hypervisor]]
=== Configuring networking on the hypervisor

To achieve maximum performance in a low latency environment (like it is needed for SAP HANA) it is always best to reduce overhead as much as possible. The best way to do that in a virtualized environment is by using SR-IOV on capable network cards to give the VM direct access to a physical NIC on the server. The difference between this and using bridged networking for the VM running SAP HANA has been shown negligible. Both is supported. You may choose whatever fits your needs best.

[[_sec_configure_networking_on_hypervisor_bridged]]
==== Using bridged networking

Detailed information about how to set up bridged networking in {sles4sap} can be found in section 23 of the https://documentation.suse.com/sles/15-SP7/html/SLES-all/cha-network.html["SUSE Linux Enterprise Server 15 SP7 Administration Guide"].
Setting up the bridged networking for the VM is handled in <<_sec_network>>.

[[_sec_configure_networking_on_hypervisor_sriov]]
==== Using virtual functions
To achieve maximum performance for productive SAP HANA workloads, one of the host networking devices can be assigned directly to the KVM guest VM.
...
There are three supported storage options to use for the SAP HANA database inside a VM and on top of a {sles4sap} hypervisor: Fibre Channel (FC) storage, Network Attached Storage (NAS) and local storage.
...
.No multipath for attached FC controllers on hypervisor level
[NOTE]
====
If you plan to dedicate one or more FC controllers to your virtual machine(s), make sure that they are not configured in multipath on hypervisor level. Either blacklist them or deactivate multipath entirely (if possible).
====
...
Therefore, we recommend to only use the state *C0*.
...
. Edit [path]_/etc/default/grub_ and add the following boot options to the line *GRUB_CMDLINE_LINUX_DEFAULT* (a detailed explanation of these options will follow). Be aware that the line below most likely cannot be simply copied and pasted as the 'hugepages=' parameter needs to be calculated for your system.
+
...
----
intremap=no_x2apic_optout intel_idle.max_cstate=0 processor.max_cstate=0 intel_iommu=on iommu=pt transparent_hugepage=never default_hugepagesz=1GB hugepagesz=1GB hugepages=<number of hugepages> numa_balancing=disable kvm_intel.ple_gap=0 kvm_intel.ple_window=0 mitigations=auto kvm.nx_huge_pages=off security=apparmor
----
...
[NOTE]
====
This section describes the creation of a single VM on a single system. For the configuration of multiple VM's, 
most of the values regarding CPU count and/or memory amount need to be 

* divided by the total socket count of the system 
* and multiplied by the desired socket count of the single VM. 

Pay attention to the memory assignment. 
Try to assign memory that is located on the socket you are using for the VM.
====
...
Refer to section 10 "Guest Installation" of the https://documentation.suse.com/sles/15-SP7/html/SLES-all/cha-kvm-inst.html[SUSE Virtualization Guide].
...
* if the host has 4294967296 KiB (that is, 4 TiB) of memory on two NUMA cells, 
* and you want to leave 91.75% of it to the hypervisor (see <<_sec_memory_sizing>>),
* and there are 4 NUMA cells, 

each NUMA cell will have the following number of huge pages:
...
The examples below show configuration snippets for full size single-VM layouts 

* on a 4-node system containing {cascadelake} CPU's (first example) 
* and on a 2-node system containing {sapphirerapids} CPU's (second example).
...
We recommend to pin sibling pairs of vCPUs to the (corresponding) sibling pairs of host CPUs.
For example, vCPU 0 should be pinned to pCPU 0 and 112, and the same applies to vCPU 1.
As long as both vCPUs always run on the same physical core, the host scheduler can execute them on either thread—for example, if one thread is free while the other is handling host or hypervisor tasks.
...
If you choose to use a bridged network setup you should have done all the preparations on the hypervisor by now. The configuration of the virtual devices attached to the VM looks like this:

----
<interface type='bridge'>
  <mac address='0c:fd:37:95:e6:42'/>
  <source bridge='br0'/>
  <target dev='vnet2'/>
  <model type='virtio'/>
  <alias name='net0'/>
...
For details refer to section 15 https://documentation.suse.com/sles/15-SP7/html/SLES-all/cha-kvm-inst.html[SUSE Virtualization Guide].

==== Virtual functions (SR-IOV based networking)
...
Since the storage for HANA (`/data`, `/log` and `/shared` volumes) is performance-critical, we recommend to take advantage of an SAN HBA that is passed through to the guest VM.
...
More details about how to directly assign PCI devices to a guest VM are described in https://documentation.suse.com/sles/15-SP7/html/SLES-all/cha-libvirt-config-virsh.html#sec-libvirt-config-pci-virsh["Adding a PCI Device" of the Virtualization Guide].

===== Local storage

To achieve the best possible performance, directly attach the block device(s) and/or raid controllers, which will be used as storage for the SAP HANA data files. 
...
If you want to attach single devices (for example NVMe devices), it is important that you attach them to the VM on PCI level. 
For that you need to identify the device's PCI address, detach it from the host and then attach it to the VM. 
You should use a script similar to the one below:

----
    #!/bin/bash
    NVME=/sys/block/nvme0n1 # CHANGE THIS!
    
    DEV=$(cat $i/device/address | tr ':' '_' | tr '.' '_')

    DOMAIN=$(virsh nodedev-dumpxml pci_${DEV} | grep '<domain>' | awk -F '>' '{print $2}' | awk -F '<' '{print $1}')
    BUS=$(virsh nodedev-dumpxml pci_${DEV} | grep '<bus>' | awk -F '>' '{print $2}' | awk -F '<' '{print $1}')
    SLOT=$(virsh nodedev-dumpxml pci_${DEV} | grep '<slot>' | awk -F '>' '{print $2}' | awk -F '<' '{print $1}')
    FUNCTION=$(virsh nodedev-dumpxml pci_${DEV} | grep '<function>' | awk -F '>' '{print $2}' | awk -F '<' '{print $1}')

    TMPFILE=$(mktemp -u)

    echo "<hostdev mode='subsystem' type='pci' managed='yes'>" > ${TMPFILE}
    echo "    <source>" >> ${TMPFILE}
    echo -n “        “ >> ${TMPFILE}
    echo $(printf "<address domain='0x%x' bus='0x%x' slot='0x%x' function='0x%x'/>\n" $DOMAIN $BUS $SLOT $FUNCTION) >> ${TMPFILE}
    echo "    </source>" >> ${TMPFILE}
    echo "</hostdev>" >> ${TMPFILE}

    virsh nodedev-detach pci_${DEV}
    virsh attach-device --domain ${VMNAME} --file ${TMPFILE} --config
----
...
To persist these configurations, we recommend to put the commands provided below into a script which is executed as part of the boot process.
...
===== Activating and configuring `saptune`

Activate saptune with the following options:
...
Detailed documentation on `saptune` is available in chapter https://documentation.suse.com/sles-sap/15-SP7/html/SLES-SAP-guide/cha-tune.html[Tuning systems with `saptune`] of the {sles4sap} Guide.
...
`saptune` automatically applies their settings at boot time and does not need to be included in the script above.
...
* https://documentation.suse.com/sles/15-SP7/html/SLES-all/cha-libvirt-managing.html[Section 11 "Basic VM Guest Management"] and others in the https://documentation.suse.com/sles/15-SP7/html/SLES-all/book-virtualization.html[SUSE Virtualization Guide for SUSE Linux Enterprise Server 15]
* https://documentation.suse.com/sles/15-SP7/html/SLES-all/article-virtualization-best-practices.html[SUSE Virtualization Best Practices for SUSE Linux Enterprise Server 15]
