...
_{sles4sap}_ for {saphana} scale-out system replication in a multi-target architecture.
...
machines (or nodes) are named _{mySite1FirstNode}_, ... _suseXX_. The nodes
({suse01}, {suse02}) will be part of site "A" ({mySite1Name}), the nodes
({suse03}, {suse04}) will be part of site "B" ({mySite2Name}), and the nodes
({suse05}, {suse06}) will be part of site "C" ({mySite3Name}).
...
| {{suse02}} | yes    |... master 2    | slave       | worker      | slave
...
{{suse02}}, {inst}, 5{inst}13, 5{inst}14, 0.3, HDB|HDB_WORKER, GREEN
----
...
The {saphana} database instance on the secondary site must be stopped before the
...
| -------- | ------ | .. --------- | --------- | .. --------- | .. ------
| SYSTEMDB | {suse01} | .. WDF1      | {suse03}  | .. ROT1      | .. **ACTIVE**
| {sapsid}      | {suse01} | .. WDF1      | {suse03}  | .. ROT1      | .. **ACTIVE**
| {sapsid}      | {suse01} | .. WDF1      | {suse03}  | .. ROT1      | .. **ACTIVE**
| {sapsid}      | {suse02} | .. WDF1      | {suse04}  | .. ROT1      | .. **ACTIVE**
...
the guide
https://help.sap.com/docs/SAP_HANA_PLATFORM/6b94445c94ae495c83a19646e7c3fd56/676844172c2442f0bf6c8b080db05ae7.html["How To Perform System Replication for SAP HANA"].
...
[landscape]
...
master = {suse01}:3{Inst}01 {suse02}:3{Inst}01
worker = {suse01} {suse02}
active_master = {suse01}:3{Inst}01
----
...
// TODO PRIO2: hana{so0,so1,so2,so3,mm} -> use hostname variables
...
----
Stack: corosync
Current DC: {susemm} (version 1.1.16-4.8-77ea74d) - partition with quorum
Last updated: Tue Jan 25 16:55:04 2024
Last change: Tue Jan 25 16:53:58 2024 by root via crm_attribute on {suse03}

5 nodes configured
1 resource configured

Online: [ {susemm} {suse01} {suse02} {suse03} {suse04} ]
...
Prepare the RA configuration in a text file, for example _crm-saphanacon.txt_,
and load these with the `crm` command.
...
primitive rsc_SAPHanaCon_{refSID}_HDB{refInst} ocf:suse:SAPHanaController \
    ...
    params SID="{refSID}" InstanceNumber="{refInst}" \
        ...
        HANA_CALL_TIMEOUT="120" ON_FAIL_ACTION="fence"
...
If srHook=SOK, in case of monitor failure, a node fencing could be triggered.
...
See also SAPHanaSR-alert-fencing(8).
|===
...
location SAPHanaFil_not_on_majority_maker cln_SAPHanaFil_**{SID}**\_HDB**{Inst}** -inf: **{{susemm}}**
----
...
After all changes are applied, as final steps, end the cluster maintenance mode,
let the cluster detect the {HANA} status and set the SAPHanaController resource
out of maintenance.
...
[subs="specialchars,attributes"]
----
~> hdbnsutil -sr_register --name=FRA1 --remoteHost={suse01} --remoteInstance=00 --replicationMode=async --operationMode=logreplay
----
...
-------------------------------------------------------------------
global Tue Jan 06 00:44:25 2024 false       WDF1 ROT1 {sapsid} ScaleOut
...
---------------------------------------------------------------
FRA1                                  SOK
ROT1 30         4   {suse03} logreplay SOK    sync   SOK    S
WDF1 1657838665 4   {suse01} logreplay PRIM   sync   PRIM   P

Hosts   clone_state node_state roles                        score  site srah
-----------------------------------------------------------------------------
{susemm}              online
{suse01} PROMOTED    online     master1:master:worker:master 150    WDF1 -
{suse02} DEMOTED     online     slave:slave:worker:slave     -10000 WDF1
{suse03} DEMOTED     online     master1:master:worker:master 100    ROT1 -
{suse04} DEMOTED     online     slave:slave:worker:slave     -12200 ROT1
----
...
are installation defaults. For the progressive conifiguration susChkSrv.py parameter
action_on_lost=*fence* is used.
...
* Start with susChkSrv.py parameter **action_on_lost = fence**.
...
Cluster Summary:
  * Stack: corosync
  * Current DC: {suse02} (version 2.0.5+20201202.ba59be712-150300.4.21.1-2.0.5+20201202.ba59be712) - partition with quorum
  * Last updated: Tue Jan  6 00:12:46 2024
  * Last change:  Tue Jan  6 00:11:40 2024 by root via crm_attribute on {suse01}
  * 5 nodes configured
...
Active Resources:
  ...
  * Clone Set: cln_SAPHanaTop_{sapsid}_HDB{sapino} [rsc_SAPHanaTop_{sapsid}_HDB{sapino}]:
    * Started: [ {suse01} {suse02} {suse03} {suse04} ]
  * Clone Set: cln_SAPHanaFil_{sapsid}_HD{sapino} [rsc_SAPHanaFil_{sapsid}_HDB{sapino}]:
    * Started: [ {suse01} {suse02} {suse03} {suse04} ]
  * Clone Set: mst_SAPHanaCon_{sapsid}_HDB{sapino} [rsc_SAPHanaCon_{sapsid}_HDB{sapino}] (promotable):
    * Masters: [ {suse01} ]
    * Slaves: [ {suse02} {suse03} {suse04} ]
  * rsc_ip_{sapsid}_HDB{sapino}    (ocf::heartbeat:IPaddr2):        Started {suse01}
----
...
The majority maker _{susemm}_ does not run an {saphana} instance and therefor
...
| {suse02} | yes    | ... slave    | slave       | worker      | slave       |

overall host status: ok
----
==========

Following the SAP HA guideline, the _SAPHanaController_ resource agent interprets
...
|4
|{saphana} database is up and OK. The cluster interprets this as correctly
running database.

|3
|{saphana} database is up and in status INFO. The cluster interprets this as a
correctly running database.

|2
|{saphana} database is up and in status warning. The cluster interprets this as
a correctly running database.

|1
|{saphana} database is down. If the database should be up and is not down by
...
// REVISION 0.3 (2026-04-09)
//	- SAPHanaController ON_FAIL_ACTION=fence, susChkSrv.py action_on_lost=fence
//
