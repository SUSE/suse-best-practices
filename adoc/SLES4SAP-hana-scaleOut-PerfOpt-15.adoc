...
:Revision: 0.1
// TODO PRIO2: check what we can borrow from AWS scale-out 15 guide
...
// TODO PRIO2: link to requierements section
...
A dedicated setup guide describes how to manage a specific HANA scale-out configuration
without standby-nodes. You will find ackground information on this so-called ERP-style setup  
in a series of blog articles at {#towardsZeroDowntime}, e.g. 
...
of a {sle} High Availability Extension {pn15} cluster. It gathers information about the statuses and
...
\\ TODO PRIO1: * There is only one {saphana} system replication like from site "A" to site "B" but no
...
* All {saphana} instances controlled by the cluster must not be activated via
  _sapinit_ autostart.
// TODO PRIO1: align with REQUIREMENTS from SAPHanaSR-ScaleOut(7), ocf_suse_SAPHanaController(7), SAPHanaSrMultiTarget.py(7)
...
// TODO PRIO2: explain new default SAPHanaSrMultiTarget.py, even for non-multi-target
...
The hook must be available on all cluster nodes, including the majority maker.
...
your primary site. The default and preferred mode for HA is _logreplay_.
...
to start the vi editor for the */etc/sudoers.d/SAPHanaSR* configuration file.

The user {refsidadm} must be able to set the cluster attribute hana_{refsidLC}_site_srHook_* .
...
// TODO PRIO2: align with manual page SAPHanaSrMultiTarget.py(7)
////
...
----
////
Manual page SAPHanaSrMultiTarget(7) contains additional details.
...
// TODO PRIO2: align check with manual page SAPHanaSrMultiTarget(7)
