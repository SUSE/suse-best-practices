...
Implement two {hana} HA/DR provider hook scripts. One hook script is named SAPHanaSR.py
and needs no config parameters. The other is named susCostOpt.py and needs to be
adapted according to your specific system size and user key store.

{SAPHANA} must be stopped to change the global.ini and allow {SAPHANA} to integrate
the HA/DR hook scripts during start.

- Create a database user key store for 
- Integrate the hook scripts into global.ini ({saphana} needs to be stopped)
- Check integration of the hook scripts during start-up
...
Refer to the man page SAPHanaSR.py(7) for details on this HA/DR provider hook script.
...
running. Refer to the man page susCostOpt.py(7) for details on this HA/DR
...
// TODO PRIO1: merge below section with above section on susCostOpt

.Enable use of susCostOpt
========

Enable the use of susCostOpt by adpating `global.ini` on node 2.
[subs="specialchars,attributes"]
----
[ha_dr_provider_suscostopt]
provider = susCostOpt
...
After changing the `global.ini` on node 2 and implementing the susCostOpt hook, you
should start the productive {HANA} secondary database and check if the parameters
are working.

As user {refsidadm} on node 2, you might perform the following commands:
[subs="specialchars,attributes"]
----
...
.  Manually register the old primary (on node 2) with the new primary
  after take-over (on node 1) as _<sid>adm_.
+
[subs="attributes,quotes"]
----
...
.  Manually register the old primary (on node 1) with the new primary
  after take-over (on node 2) as _<sid>adm_.
+
[subs="attributes,quotes"]
----
...
----
+
.  Manually register the old primary (on node 1) with the new primary after take-over (on node 2) as _<sid>adm_.
+
[subs="attributes,quotes"]
----
{sapnode1}# hdbnsutil -sr_register --remoteHost={sapnode2} --remoteInstance=10 \
...
system replication takes place and is therefore no longer a valid take-over destination.
