...
cluster. The more cluster configuration three and more node are described at the end of this
document.
////
...
cluster nodes {my2nd1} and {my2nd2} in the two node cluster example. Beside the partitions for the file systems
...
* Deselect using FQDN
* Database parameters enter Database ID (DBSID) is {bsSidDB};
...
We need a changed SAPInstance resource agent for {sapS4} in order *not* to use
...
configuration.
ASCS instance must not be start-up on the cluster node running the ERS
...
With {sapCert} it is *not longer allowed to block resources* from beeing
  ...
  _/etc/sysconfig/sap_suse_cluster_connector_ is not allowed any more.
