...
// = {title}
= SQL Server on SUSE Linux Enterprise: Getting Started
// SUSE Linux Enterprise Server 12 SP3-5 / SQL Server 2019
// :author: James Yang
:revnumber: 0.0.1
:toc2:
:toc-title: {title}
...
=== Background
Since Microsoft released SQL Server on Linux in 2017 footnote:[https://cloudblogs.microsoft.com/sqlserver/2017/10/02/sql-server-2017-on-windows-linux-and-docker-is-now-generally-available], 
...
Nowadays, it is viable to start considering running SQL Server workloads on Linux.

=== Audience
This guide is intended for SQL Server DBA’s, Developers, and DevOps/SRE engineers who are familiar with SQL Server on Windows and are looking 
to migrate to Linux or already run a primarily Linux environment and are adding a SQL Server requirement, 
but want to retain utilizing tools that run only on Linux servers. 
Another reason may be lower negotiated pricing for Linux subscriptions which can now be filled to replace existing SQL Server on Windows machines.

=== Scope
The guide covers a basic installation of SQL Server on {sles}. It is meant to be agnostic of underlying infrastructure excepting the nuance of server registration discussed xref:server_registration[below].
...
If you have a SUSE https://documentation.suse.com/sles/12-SP5/single-html/SLES-smt/#book-smt[SMT] (Subscription Management Tool) 
or SUSE https://documentation.suse.com/sles/15-SP2/single-html/SLES-rmt/#book-rmt[RMT] (Repository Mirroring Tool) 
or https://documentation.suse.com/external-tree/en-us/suma/4.1/suse-manager/index.html[SUSE Manager] server you want to use, 
...
To install the SQL Server package non-interactively, run:
...
[TIP]
--
For convenience, you can add it to your `PATH` to avoid typing the full path each time.
...
Then, to configure and start (`mssql-conf` starts the `msql-server` immediately after configuring), do:
...
or disable the history prior to configuring SQL Server with `set +o history`, and reenabling it afterward with `set -o history` (bash).
...
Settings are stored in https://en.wikipedia.org/wiki/INI_file[INI] format.

A sample `mssql.conf` file is shown below. Simply edit and restart it for `mssql-server` to apply changes.
...
Now that SQL Server is running, you can begin to query it. The `mssql-tools` package includes `sqlcmd`, which is a shell to query SQL Server. 
...
Available options can be found https://docs.microsoft.com/en-us/sql/tools/sqlcmd-utility?view=sql-server-ver15#syntax[here]. 
...
You can find more details https://www.freedesktop.org/software/systemd/man/systemd.unit.html[here]. 
To control the `mssql-server` service, use `systemctl` to retrieve the `status`, `start`, `stop`, `restart`, `enable`, `disable` the service.
...
To stay up to date on the latest SQL Server on Linux features bookmark this https://docs.microsoft.com/en-us/sql/linux/sql-server-linux-release-notes-2019?view=sql-server-ver15[page].
