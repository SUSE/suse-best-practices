...
device. For details on disk-based SBD, read the product documentation for {sleha} and the manual pages
...
* Automated registration of a failed primary after take-over prerequisites need to be defined.
** As initial configuration for projects, we recommend to switch off the automated registration of a failed primary. 
The setup `AUTOMATED_REGISTER="false"` is set as default. In this case, you need to register a failed primary after a take-over manually. 
...
** The cluster automates one single take-over in case of a failed primary. After that happened, the initial state needs to be restored by the administrative
...
** In MDC configurations, the SAP HANA RDBMS is treated as a single system including all database containers. Therefore cluster take-over
...
* {sles4sap} versions:
** You need at least {SAPHanaSR} version {sapHanaSrMinVers} and at least {sles4sap} 15 SP2.
...
* {sap} HANA 2.0 SPS04 or later provides the HA/DR provider hook method
...
The first helps collecting data for support requests, the second simplifies common
...
Installation Guide. The {hana} database client will be installed together with
...
As Linux user _{refsidadm}_ use the command line tool _HDB_ to get an
...
On the secondary node, the mode should be one of "SYNC" or "SYNCMEM". "ASYNC" is *not supported with automated cluster take-over*.
...
Before you integrate your {HANA} system replication into the HA cluster, it is mandatory to do a manual take-over. 
Testing without the cluster helps to make sure that basic operation (take-over and registration) is working as expected.
...
The hook is called by {SAPHANA} using the HA/DR provider interface in point-of-time when
the secondary gets out of sync. This is typically the case when the first commit pending
is released. The hook is called by {SAPHANA} again when the system replication is back.
This HA/DR provider is srConnectionChanged(), the related {SUSE} hook script is SAPHanaSR.py.

Another hook is called by {SAPHANA} after an SR take-over has happened. This can be
used to remove the {SAPHANA} memory limit and enable table preload. 
This HA/DR provider is postTakeover(), the related {SUSE} hook script is susCostOpt.py.
...
available was _delta_datashipping_. Today the preferred modes for HA are
...
----
{refsidadm}@{sapnode2}:/usr/sap/{refsid}/HDB{refInst}> cdtrace
{refsidadm}@{sapnode2}:/usr/sap/{refsid}/HDB{refInst}> grep HADR.*load.*susCostOpt nameserver_*.3*.trc
