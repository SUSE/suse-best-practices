...
** {hana} 2.0 SPS04 or later provides the needed HA/DR provider hook method
srConnectionChanged() with multi-target aware parameters. It also provides method
postTakeover().
...
NOTE: For security reasons on production systems always a dedicated database user
with restricted permission needs to be created. The database user SYSTEM must never
be used.
...
{refsidadm}@{sapnode2}:/usr/sap/{refsid}/HDB{refInst}> grep HADR.*load.*SAPHanaS nameserver_*.trc
----
...
{refsidadm}@{sapnode2}:/usr/sap/{refsid}/HDB{refInst}> grep susCostOpt.init nameserver_*.trc
----
