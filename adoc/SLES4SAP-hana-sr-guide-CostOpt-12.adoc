...
`SAPInstance` is used for the non-synchronized {SAPHANA} database. In the past the architecture used
`SAPDatabase` instead, but the move to `SAPInstance` was needed to get rid of the error-prune
...
a former failed primary to get the new secondary.
