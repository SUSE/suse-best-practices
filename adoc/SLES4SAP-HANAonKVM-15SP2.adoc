...
Always make sure that the properly SR-IOV capable driver is loaded. For example, for an "`Intel Corporation Ethernet Controller X710`" NIC, the driver resides in the "`i40e`" kernel module.
...
When the proper module is loaded, creating at least one VF happens with the following command (which creates 4 of them):
...
Do make sure though that the VM has access to one or more dedicated 10Gbit Ethernet interfaces for the network traffic to the network attached storage.
...
To apply some less specific, but nevertheless effective, tuning to the host, the "TuneD" (https://tuned-project.org/) tool can be used.
...
To avoid that, and achieve better and more consistent the performance, the CPUs should not be allowed to go into too aggressive power saving mode (known as C-states).
...
For being able to directly assing host devices (like storage controllers and NIC Virtual Functions), with PCI Passthrough and SR-IOV, the IOMMU must be enabled. On top of that, "`iommu=pt`" makes sure that we set up the devices for the best performance (that is, passthrough mode)
...
For example, if the host has 3169956100 KiB (that is, 3 TiB) of memory and we want to leave 91.75% of it to the hypervisor (see <<_sec_memory_sizing>>), and there are 4 NUMA cells, each NUMA cell will have the following number of huge pag>

* (3169956100 * (91.75/100)) / 1048576 / 4 = 693

Which means that, in total, there will need to be the following number of huge pages
...
In fact, this allows HANA to spread its own workload threads across many virtual CPUs and NUMA nodes.
...
** The distances between the cells are identical to those of the physical hardware (as per the output of "`numactl --hardware`").
...
The storage configuration is critical, as in plays an important part in terms of performance.
...
Make sure though that the VM has access to one or more dedicated 10Gbit Ethernet interfaces for the network traffic to the network attached storage.
...
* Memory
** The Hypervisor has 3 TiB RAM (or 3072 GiB), of which 2772 GiB has been allocated as 1 GB Hugepages and therefore 2772 GiB is the max VM size in this case
** 2772 GiB = 2906652672KiB
