...
For more detailed information, read the https://docs.rke2.io/install/quickstart.html[RKE 2 Quick Start guide]
...
Install NetApp Trident according to the following documentation https://netapp-trident.readthedocs.io/en/stable-v21.01/

* Create a storage class on top of NetApp Trident and make it the default storage class

* Create igroup and add initator names from RKE 2 workder nodes:

** Log into RKE2 worker nodes and query initiator names from /etc/iscsi/initiator.iscsi.

** Make sure initiator names are unique! If not replace it with new one, generated with iscsi-iname.

** Create an igroup inside your vserver and add all initator names:

==== NetApp StorageGRID

If a SAP Data Intelligence checkpoint store must be used, or AI/ML scenarios should be used, an object store is required.
...
** Use the instructions on thePub: Deploying StorageGRID in a Kubernetes Cluster to install StorageGRID in the cluster. https://netapp.io/2019/01/15/deploying-storagegrid-in-a-kubernetes-cluster/

** Select Manage Certificates, and make sure that the certificates being used are issued from an official Trust Center: NetApp- StorageGRID/SSL-Certificate-Configuration. https://github.com/NetApp-StorageGRID/SSL-Certificate-Configuration

** Following the instructions in Configuring tenant accounts and connections, configure a tenant. https://docs.netapp.com/sgws-114/topic/com.netapp.doc.sg-swift/GUID-F0D7793E-678D-42C1-91DA-29697823B492.html

** Following the instructions in Creating an S3 bucket, log in to the created tenant and create a bucket (such as datahub). https://docs.netapp.com/sgws-114/index.jsp?topic=%2Fcom.netapp.doc.sg-tenant-admin%2FGUID-AC8EC92E-1E79-41FA-8A8B-960484C5699D.html&resultof=%22%63%72%65%61%74%65%22%20%22%63%72%65%61%74%22%20%22%62%75%63%6b%65%74%22%20

** Use the instructions in Creating another user's S3 access keys to define an access key; then save the generated access key and the corresponding secret. https://docs.netapp.com/sgws-114/topic/com.netapp.doc.sg-tenant-admin/GUID-042C8C01-E479-4696-AFA2-A9212C10E723.html
...
=== Testing the SAP DaP Data Intelligence 3 installation
...
* the https://help.sap.com/viewer/a8d90a56d61a49718ebcb5f65014bbe7/3.1.latest/en-US/1ca2ac1d9c5a4bd98c5aaf57e53a81bf.html[SAP DI 3.1 documentation about upgrading Kubernetes] and

* section https://docs.rke2.io/upgrade/basic_upgrade.html[Upgrade Basics] of the RKE 2 documentation.
...
* Run the update of RKE 2 according to the https://docs.rke2.io/upgrade/basic_upgrade.html[RKE 2 documentation].
