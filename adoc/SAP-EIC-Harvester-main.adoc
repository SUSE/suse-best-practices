...
+++**+++ Only needed to set up {rancher} in a high availability setup without the {harvester} Load Balancer feature.
...
NOTE: To test upgrades of {harvester}, we recommend setting up an additional {harvester} cluster to isolate the production {eic} instances from the development and test instances.
...
When the Kubernetes cluster is ready, your next step is to install {pg} and {redis}.
...
NOTE: The installation of {reg} is not a mandatory requirement for the deployment of {eic} and is thereby optional. To continue without using a {reg}, continue with the next chapter <<installDatabases>>.
