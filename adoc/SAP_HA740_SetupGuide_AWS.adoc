...
string like "local-ipv4" in the user data section!
...
This forwarding of name resolution requests acquires an extra configuration in
the AWS VPC.
...
Checking the log file is also a suitable measure to validate the status.
...
If Fault Manager is monitoring the Primary and Companion database and Fault Manager is monitored by Pacemaker there is a special procedure necessary to start and stop the system.
