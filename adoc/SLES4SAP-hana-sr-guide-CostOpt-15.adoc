...
particularly on public cloud platfoms (e.g. https://www.suse.com/c/automating-the-sap-hana-high-availability-cluster-deployment-for-microsoft-azure/). Please
ask your public cloud provider or SUSE contact person for details.
...
a former failed primary to become the new secondary. Please refer to the man pages SAPHanaSR(7) and ocf_suse_SAPHana(7) for details on all supported parameters and features.
...
additional majority maker in case of cluster separation.
...
register a failed primary after a takeover manually. For re-registration please use precisely the site names that are already known by the cluster. Use SAP tools like {saphana} cockpit or _hdbnsutil_.
...
After that happened, the initial state has to be restored by the administrative
...
the product documentation of {sles4sap} in section _Setting Up a SAP HANA Cluster_
...
. Check if the SAP Host Agent and the HANA database client are installed on all cluster nodes. If these
SAP components are not installed, install them now.
...
site name. The site names must not be changed later once the cluster has been activated.
...
Before you integrate your {HANA} system replication into the HA cluster,
...
directory, such as /hana/shared/srHook/. The hook must be available on all {SAPHANA} cluster nodes. Please refer to the man page SAPHanaSR.py(7) for details on this HA/DR provider hook script.
...
When your system is connected as a SAP HANA SR target, you can find an entry in the _global.ini_
...
Please see also the example at the end of this document and refer to the man pages corosync.conf(5), votequorum(5) and corosync_overview(8) for details on parameters and features.
...
It is a requirement that they are tuned to your environment. Please refer to the
...
To do so, call `crm_mon`. Use the options _"-r1"_ to also see the resources that are
...
Administration Guide for {sleha} {pn15} {psp15} at {haAdminGuide15}#cha-ha-manual-config. The man page crm(8) might be useful, too.
...
Availability Extension at {haAdminGuide15}. For public cloud environements please refer to your cloud provider's documentation on supported fencing mechanisms.
...
the cluster is not performing any action, i. e. is in the state _S_IDLE_.
...
To check the status of a SAP HANA database and to find out if the
...
.Migrating a {HANA} primary using {sap} Toolset
==============================
...
.Moving a {HANA} primary using the Cluster Toolset
==============================
...
The site name to be registered is {sapsite1}. Please make sure this is the exact same as before.
