...
* 2 Elatic Compute Service (ECS) instances in different zones of the same VPC;
https://www.alibabacloud.com/product/ecs[Elastic Compute Service]
* 2 Elasit Network Interfaces (ENI) - one for each ECS instance;
...
**[HOSTNAME]** should be the host name of the Secondary node;
...
Perform the same steps as outlined above for the Primary node on the Secondary node. However, do not forget to use here the IP and host name of the Primary node instead of the Secondary node.
...
For the SAP HANA High Availability solution, you need to configure seven Resource Agents and the corresponding constraints in Pacemaker.
...
Save the following scripts in a file: _crm-stonith.txt_.
```shell
