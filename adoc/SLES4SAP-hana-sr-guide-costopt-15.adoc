...
no config parameters. susCostOpt.py needs to be adapted according to your
specific system size and user keystore. {SAPHANA} must be stopped to change the
global.ini and allow {SAPHANA} to integrate the HA/DR hook scripts during start.
...
provider = susCostOpt
path = /usr/share/SAPHanaSR/
userkey = saphanasr_{refSid}_costopt
execution_order = 2
