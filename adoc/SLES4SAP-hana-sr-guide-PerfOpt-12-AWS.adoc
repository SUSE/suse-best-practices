...
SAP environment. In many cases, the “SAP Business Suite* powered by SAP HANA*”
scenario is already being discussed in concrete terms.

Naturally, SUSE is also accommodating this development by providing SUSE Linux
...
partners, therefore, SUSE provides two resource agents for customers to ensure
...
the database data
from one computer to another computer in order to compensate for database
...
mode, and the slave is responsible for instances that are operated in
synchronous (secondary) status.

To make configuring the cluster as simple as possible, SUSE also developed it's
[.underline]#SAPHanaTopology# resource agent. This runs on all nodes of an
SLE 12 HAE cluster
...
* Cost optimized (A => B, Q): This scenario and setup is described in an
    ...
    the table preload must be switched off and a possible takeover needs longer
...
* Multi-tenancy or MDC:
    ...
    scenario is supported since SAP HANA SPS09. The set-up and configuration
    from cluster point of view is the same for multi-tenancy and single
    container, so you can use the above documents for both kinds of scenarios.
    In our notation we mark a MDC like %A. This means MDC, performance
...
With both the SAPHana and SAPHanaTopology resource agents, customers will
therefore be able to integrate SAP HANA system replications in their cluster.
...
interruption while noticeably reducing needed budgets. SUSE provides the
...
solution will also benefit from this SUSE Linux development.
...
* If the cluster nodes are installed in different data centers or data center
    ...
    maximum distance between the nodes. Please review our product documentation
...
* Cost optimized setups with a non-productive SAP HANA (like QAS or TST) on
    ...
    have a SCN article for this kind of setups
...
This document describes how to setup the cluster to control SAP HANA in System
...
This setup builds a SAP HANA HA cluster in two data-centers in Walldorf (WDF)
and in Rot (ROT), installed on two SLES for SAP 12 SP3 systems.
...
Overlay IP addresses have one disadvantage, they have to to have a CIDR range
...
Create the two EC2 instances to build up your SLE HAE cluster.
...
Use this AMI to create a SAP HANA
compliant configuration. Register the systems!
...
NOTE: Use a "SLES for SAP" AMI which includes the SLES HA Extension.
SUSE will not allow to migrate from generic SLES AMIs to
...
The screen shot below has been created by identifying an EC2 instance
at the console. The last tab Tags has been clicked.
The button Add/Edit Tags has then been clicked.
A new tag with the key pacemaker and the hostname has been created.
...
The source/destination check has to be disabled. This can be done
...
The HANA database server and replication server will run the SLES Pacemaker software and the agents.
...
The policy doesn’t contain any instance specific privileges.
...
The IP address has to be outside of the CIDR range of the VPC.
...
Amazon strongly recommends to disable remote root login. In order to run the
SLES *[.underline]#crm report#* command in such an environment you have to
enable the hacluster OS-user to execute crm shell commands as well as
the hb_report command.
A certificate based passwordless login of the hacluster user has to be
...
4. Now that passwordless ssh works and hacluster can execute crm commands
...
SAP HANA with already set up system replication into the pacemaker cluster,
this chapter summarizes the test environment. Please always use the official
documentation from SAP to install SAP HANA and to setup the system replication.
...
Procedure 4.1: Installation and Checks
 ...
 2. Check if the SAP Host Agent is installed on all cluster nodes. If this SAP
    service is not installed, please install it now.
...
This step must be done on both sites. SAP HANA must be stopped to change the global.ini
and allow SAP HANA to integrate the HA/DR hook script during start.
...
If the user key hanabackup has not been defined, please use an instance/user/
...
Please perform the following commands as super user on all cluster nodes:
...
cluster node. The two IP configuration will use the second IP, in the event that
...
Now it's time to check and optionally start the cluster for the first time on
both nodes.
...
It'll create a result like the following one for a cluster node with the IP
address 10.79.8.23:
...
This is desirable in order to avoid split brain scenarios on the AWS platform.
// end AWS
...
("cluster" in this example) will have to match the previously configured
AWS profile.

Name this file for example aws-stonith.txt and add this file to the
configuration. The following command has to be issued as super user. It uses
...
// begin AWS
A working STONITH method is mandatory in order to run a supported SUSE cluster
on AWS.
...
The name of the profile (cluster in this example) will have to match the
...
time difference is less than the time gap, than the
cluster hold one or both instances in a "WAITING"
status. This is to give a admin the chance to react
...
"only" the SAP HANA RDBMS has crashed, then the
...
Add this file to the configuration. The following command has to be issued as
...
As with any cluster testing is crucial. Please make sure that all test cases
...
////
// begin on-premise
     1. If SBD fencing is used then pacemaker will not automatically restart
...
Expected:
     ...
     5. Once the fenced node (node 2) rejoins the cluster the former secondary
...
If you set up the cluster using ha-cluster-init and you have installed all
...
To show some of the SAPHana, SAPHanaTopology resource agent internal values,
...
Check the status of a SAPHana database and figure out if the cluster
...
actions in order to prevent data corruption.
...
Re-integrating a faulty cluster node into the cluster has to be performed
manually since it'll take an investigation why the cluster node didn't
operate as expected.
...
Update of SUSE Linux Enterprise Server for SAP Applications packages including
cluster software you should follow the rolling update procedure defined in the
product documentation of SUSE Linux Enterprise High Availability _Upgrading Your
...
*[.underline]#crm node maintenance#* _[.underline]#node#_.
 ...
 4. As we expect the primary/secondary roles to be exchanged after the
    maintenance, tell the cluster to forget about this states and to reprobe
...
following procedure shows how to tell the cluster to "accept" a role change
...
Procedure 7.3: Migrating a HANA primary using cluster migration commands
 ...
 1. Create a migrate away from this node rule.
    ...
    ** Because of we used the migrate away rule the cluster will stop the current
 ...
 2. If you have setup [.underline]#AUTOMATED_REGISTER="true"# you can skip
 this step.
...
Procedure 7.4: Migrating a HANA primary using cluster node status standby
 ...
 2. If you have setup [.underline]#AUTOMATED_REGISTER="true"#
...
The STONITH agent will shutdown the other node if he thinks that this node
isn't anymore reachable. The agent can be called manually as super user on a
...
This command will shutdown cluster node 2. Check the errors reported during
execution of the command if it's not going to work as planned.
...
* Fail-Safe Operation of SAP HANA®: SUSE Extends Its High-Availability
...
* Current online documentation of SUSE Linux Enterprise High Availability
    Extension https://www.suse.com/documentation/sle_ha/
...
suse02) and a SAP HANA database with SID HA1 and instance number 10. The
...
The following file shows a typical corosync configuration with one ring. Please
view SUSE product documentation about details and about additional rings.
