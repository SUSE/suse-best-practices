...
To do so, apply all configuration as usual and hit the *Edit as YAML* button in the creation step, as shown below:

image::SAP-Rancher-Create-Config-YAML.png[title=Rancher create custom cluster yaml config,scaledwidth=99%]

The excerpt is to be located under *spec.rkeConfig*. An example can be seen here: 
