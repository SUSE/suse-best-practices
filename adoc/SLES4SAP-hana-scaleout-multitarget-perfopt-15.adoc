...
include::Var_SLES4SAP-hana-classic-scaleout-multitarget-perfopt-15.txt[]
include::Var_SLES4SAP-hana-classic-scaleout-multitarget-perfopt-15-param.txt[]
:docinfo:
//
...
| -------- | ------ | .. --------- | --------- | .. --------- | .. ------
| SYSTEMDB | {suse01} | .. {sapsite1}      | {suse03}  | .. {sapsite2}      | .. **ACTIVE**
| {sapsid}      | {suse01} | .. {sapsite1}      | {suse03}  | .. {sapsite2}      | .. **ACTIVE**
| {sapsid}      | {suse01} | .. {sapsite1}      | {suse03}  | .. {sapsite2}      | .. **ACTIVE**
| {sapsid}      | {suse02} | .. {sapsite1}      | {suse04}  | .. {sapsite2}      | .. **ACTIVE**
...
Last change: Mon July 25 16:53:58 2022 by root via crm_attribute on {suse03}
...
[subs="specialchars,attributes"]
----
~> hdbnsutil -sr_register --name={sapsite3} --remoteHost={suse01} --remoteInstance=00 --replicationMode=async --operationMode=logreplay
----
...
---------------------------------------------------------------------
{sapsid}    Fri Jul 15 00:44:25 2022 false       {sapsite1} {sapsite2} SOK        ok  

Sites lpt        lss mns     srHook srr 
----------------------------------------
{sapsite3}                         SOK        
{sapsite2}  30         4   {suse03} SOK    S   
{sapsite1}  1657838665 4   {suse01} PRIM   P   

Hosts   clone_state gra gsh node_state roles                        score  site 
--------------------------------------------------------------------------------
{susemm}                      online                                             
{suse01} PROMOTED    2.0 2.2 online     master1:master:worker:master 150    {sapsite1} 
{suse02} DEMOTED     2.0 2.2 online     slave:slave:worker:slave     -10000 {sapsite1} 
{suse03} DEMOTED     2.0 2.2 online     master1:master:worker:master 100    {sapsite2} 
{suse04} DEMOTED     2.0 2.2 online     slave:slave:worker:slave     -12200 {sapsite2} 
...
Cluster Summary:
  ...
  * Last change:  Sat Aug  6 00:11:40 2022 by root via crm_attribute on {suse01}
  * 5 nodes configured
...
Active Resources:
  ...
  * Clone Set: msl_SAPHanaCon_{sapsid}_HDB{sapino} [rsc_SAPHanaCon_{sapsid}_HDB{sapino}] (promotable):
    * Masters: [ {suse01} ]
...
Global cib-time                 maintenance prim sec  sync_state upd 
---------------------------------------------------------------------
{sapsid}    Thu Mar 19 08:59:09 2026 false       {sapsite1} {sapsite2} SOK        ok
...
The third HANA site {sapsite3} is not controlled by the Linux cluster. Therefor it doesnot have any attribute except the srHook status.
...
|0
|Internal Script Error – Database status unknown.
