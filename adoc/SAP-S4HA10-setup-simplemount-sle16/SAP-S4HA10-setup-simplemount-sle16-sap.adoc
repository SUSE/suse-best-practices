...
Temporarily set the service IP address which you will
later use in the cluster as the local IP address, because the installer needs to be able to resolve and
use it. Use the correct virtual hostname for each installation step.
If applicable, mount file systems such as _/sapmedia/_.
...
After registration, the list of systemd services must be the same and should look
like:
...
This step is required to transfer control of the instance to the HA cluster.
...
Starting the instance services will not start the {sap} instance.
...
and https://help.sap.com/docs/SAP_HANA_PLATFORM/eb3777d5495d46c5b2fa773206bbfb46/0b15a92d554c4941a452c9ca127f8c70.html[SAP HANA TDI].
