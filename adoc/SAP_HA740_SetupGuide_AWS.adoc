...
NOTE: If the system is reinstalled and the companion system reports *Missing read/write permissions* for this directory _/tmp/.SQLAnywhere_ check the permission on both node. In the case the ownership must be changed run the setup again on both nodes. Start with the *Primary*.
...
To implement an FM controlled ASE replication setup with pacemaker integration for FM we have to follow some special rules. First of all we will detail how to check the status of the replication and FM itself.
This chapter will also give guidelines to improve the takeover time and how to control such an environment.
