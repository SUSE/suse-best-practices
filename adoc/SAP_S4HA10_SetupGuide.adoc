...
availability cluster solution, for example as provided by {sles4sap}. The mentioned components have
...
follow the guides for the performance- or cost-optimized scenario. (see section "Additional
...
the SUSE HA cluster. A third node is used for running the database and PAS and AAS application
...
NOTE: Version for {sapS4pj} on Linux on AMD64/Intel 64. More information about
...
{sap} instances in a highly available environment is, that if an {sap}
...
Temporarily we need to set the service IP address we will have later in the
...
We need a changed SAPInstance resource configuration for {sapS4} to *not* use
...
The constraints between the ASCS and ERS instance are changed compared to the ENSA1
configuration.
An ASCS instance must not start up on the cluster node running the ERS
instance after a failure (loc_sap_{mysid}_failover_to_ers). This constraint is
was needed in the past to ensure that the locks are not lost after an ASCS instance (or node)
failure. Today the ENSA2 setup can resynchronize the lock table over the network.
...
NOTE: Before each test verify that the cluster is in idle state. No migration constraints
...
Do this test multiple times in a short time frame. The restart of the ASCS instance in the
