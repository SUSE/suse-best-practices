...
replicate the database data from one computer to another computer to compensate for
database failures (single-box replication).
...
+
A _Multiple Tier_ (multi-tier) system replication has an additional target. In
the past this third side must have been connected to the secondary (chain topology).
With current {saphana} versions also _multiple target topology_ (multi-target) is
allowed by {sap}. Please look at the scenarios and prerequisites section below or
...
call outs in case of special events such as status changes for services or system
replication channels. These calls outs are also called HA/DR providers. This
interface can be used by implementing {saphana} hooks written in python. {suse}
improved the SAPHanaSR package to include such {saphana} hooks to optimize the
cluster interface. Using the {saphana} hook described in this document allows to
inform the cluster immediately if the {saphana} system replication breaks. In
addition to the {saphana} hook status the cluster continues to poll the system
replication status on regular base.
...
** Any STONITH mechanism supported by SUSE Linux Enterprise {prodnr} High
Availability Extension (like SDB, IPMI) is supported with SAPHanaSR.
...
{sleha} and the manual pages sbd(8) and stonith_sbd(8).
** For disk-less SBD you need at least three cluster nodes. The disk-less SBD mechanism
has the benefit that you do not need a shared drive for fencing. Since disk-less
SBD is based on self-fencing, reliable detection of lost quorum is absolutely
crucial.
...
* Both {hana} instances (primary and secondary) have the same SAP Identifier
(SID) and instance number.
* If the cluster nodes are installed in different data centers or data
center areas, the environment must match the requirements of the {sleha}
...
product documentation for {sleha} about those recommendations.
...
setup (performance-optimized, cost-optimized, multi-tier, mulit-target, read-enabled).
...
complete {hana} instance using _HDB kill_ does not work, because the tenants
are running with different Linux user UIDs. _{refsidadm}_ is not allowed to terminate
the processes of the other tenant users.
////
...
Follow the instructions below on each node if you have installed the systems based on SAP note 2578899.
...
As Linux user _{refsidadm}_, enable the system replication at the
...
==== Setting up initial cluster using `{slehainit}`
...
See manual pages sbd(8) and stonith_sbd(7) for details.
...
name of the first cluster node. Then all needed configuration files are
...
STONITH mechanism. The {sleha}  product also supports additional fencing mechanism
not covered here.
...
Additional information about all parameters of the SAPHanaTopology RA can be found with the
command:
...
Additional information about all parameters of the SAPHana RA could be found with the
command:
...
// TODO PRIO3: Check -sr_takeover with 'handshake'
