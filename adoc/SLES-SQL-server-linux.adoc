...
image:sqlserver.svg[SQL Server,100]
== Motivation
...
IMPORTANT: The last step to make the changes persistent is to disable the sysctl rerun after TuneD has modified the system, otherwise sysctl reverted settings to a value that is not expected from the config above.  
Disable the reapply_sysctl by changing the 1 to a 0.
...
# using 950000MB of 1048576MB available RAM for MSSQL DB instead of 80% default, example for 1TB
...
Until the service is restarted, tempdb continues to function in its existing location.
