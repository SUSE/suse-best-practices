...
HANA filesystems and NFS shares are not managed by the Linux cluster.

An iSCSI target for SBD might be placed at the cluster third site. The SBD is
accessed from all five cluster nodes. But the target is hosted outside the Linux
cluster. The SBD based fencing needs up to three block devices, shared across
the three cluster sites. 
// TODO PRIO2: see ... for details on SBD
