...
2. **Phase 2: Access Provisioning for {eic} Deployment:** Applying solution-specific policies and granting permissions for {eic} when the deployment is "Prepared".
...
When the Edge Node is "Available", initiate the {eic} solution deployment in the {elm} UI. When the solution reaches the **Prepared** state, return here to apply the specific solution resources.

Download the second zip (for example, `restricted-resources-Edge_Integration_Cell-<version>.zip`) from the {elm} UI and extract it to a directory (for example, `eic-resources`). Apply it to the cluster:
...
When the permissions are successfully bound, return to the {elm} UI and click **Resume deployment**.
