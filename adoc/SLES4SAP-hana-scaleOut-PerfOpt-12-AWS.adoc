...
they need HTTP/HTTPS access to AWS API endpoints. Systems which don't offer
...
Make sure that the SUSE instance is able to reach the EC2 instance metadata address URL http://169.254.169.254/latest/meta-data as multiple system components will required to access it. Therefore it is recommended to disable any firewall rules that restricts it, and to disable proxy access to this URL.
...
The majority maker suse-mm does not run a {saphana} instance and therefore
