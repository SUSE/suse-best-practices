...
NOTE: If the installation stops with an error message like this:
...
Check if the host name/port number are correct and database server is up and running. If the network connection should be possible it could help to modify the _interface_ file. Try to add a new line in the _/sybase/<SID>/interfaces_ file for the <SID> section with the IP address of the corresponding hostname.
...
till the ASCS failover is complete and the FM is up and running in the 2nd Availability Zone (AZ2). The FM then needs to read the boot file
to get the state of the ASE DB prior. This is mandatory that FM can trigger the failover correctly. The parameter
