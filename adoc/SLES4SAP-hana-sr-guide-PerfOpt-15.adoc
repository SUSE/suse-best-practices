...
For SystemV style, the sapinit script needs to be active. For systemd style, the
...
strongly  recommended. This might become mandatory in furture versions.
...
** In MDC configurations the HANA database is treated as a single system including
all database containers. Therefore, cluster takeover decisions are based on the
complete status independent of the status of individual containers.
...
// TODO PRIO1: align with man pages SAPHanaSR(7) and SAPHanaSr.py(7) 
// TODO PRIO1: versions for multi-target
// TODO PRIO1: versions for systemd 
// TODO PRIO1: remove HANA 1.0?
////
...
Enter the command in our example on node2 as user _{refsidadm}_.
+
[subs="specialchars,attributes,quotes"]
----
