...
This guide describes planning, set up, and basic testing of
the SUSE Linux Enterprise Server for SAP Applications based
high availability solution scenario
...
device. For details on disk-based SBD look into our product documentation for
SUSE Linux Enterprise High Availability Extension as well as the manual pages
...
- 1 optional IP address for HAWK Admin Gui
...
This step is mandatory to inform the cluster immediately, if secondary gets out of sync.
...
watchdog driver. It's strongly recommended to use a watchdog with hardware #
...
It is recommended to add a second corosync ring. If you did not started {slehainit}
...
After setup of the corosync config and SBD parameters, start the cluster again.
...
The parameter must also be alligned with the pacameker cluster property stonith-watchdog-timeout.
...
Starting with SUSE Linux Enterprise High Availability Extension 15, if you set `stonith-watchdog-timeout` to a negative value, Pacemaker will automatically calculate this timeout and set it to twice the value of `SBD_WATCHDOG_TIMEOUT`. 
...
aggressive then the typical 5s.
...
colocation col_saphana_ip_{sapsid}_HDB{sapino}_readenabled 2000: \
    rsc_ip_{sapsid}_HDB{sapino}:Started msl_SAPHana_{sapsid}_HDB{sapino}:Slave
----
...
`HDB kill-9`. This happened round about 9:10 pm.
