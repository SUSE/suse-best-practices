...
// = {title}
= Microsoft SQL Server on {sles}: Getting Started
// SUSE Linux Enterprise Server 12 SP3-5 / SQL Server 2019
// :author: James Yang
:revnumber: 0.0.1
:toc2:
:toc-title: Microsoft SQL Server on SUSE Linux Enterprise Server: Getting Started
:toclevels: 4
...
CAUTION: It is recommended to change the SA password later with `mssql-conf set-sa-password` 
...
A sample `mssql.conf` file is shown below. Simply edit the `mssql.conf` file and restart `mssql-server` to apply changes.
...
The full `sqlcmd` documentation can be found at https://docs.microsoft.com/en-us/sql/tools/sqlcmd-utility?view=sql-server-ver15[sqlcmd 
Utility].
