...
for "SAP HANA Scale-Out Multi-Target System Replication - ERP style" scenario.
...
as majority maker) based on {sles4sap} {pn15} SP4 and how to add an
...
We will first set up a {sles4sap} cluster controlling
two sites of {saphana} scale-out in a system replication configuration. We will
next set up a third site which is outside {sleha} cluster but acts as additional
{saphana} target forming a multi-target architecture.

.Cluster with {SAPHana} Multi-Target SR - performance optimized
image::Multi-Target.svg[scaledwidth="100%"]
...
https://documentation.suse.com/sbp/all/single-html/SLES4SAP-hana-scaleOut-PerfOpt-15/ .
...
NOTE: Although, it is possible to have standby nodes in scale-out multi-target setup,
however this guide is created with a focus of ERP style setup and involves no standby
nodes. All 3 sites have 2 nodes each.

.Simplified NFS share structure of a (2+0 \<= 2+0 \-> 2+0) {saphana} multi-target system replication
image::SAPHanaSR-ScaleOut-MultiTarget-1.svg[scaledwidth="100%"]

Requirements with 2 {saphana} instances per site (aka [ 2+0 <= 2+0 ] -> 2+0) - with a
majority maker:
...
The SBD based fencing needs up to 3 block devices, shared across the two cluster sites.
...
image::SAPHanaSR-ScaleOut-Cluster-Resources-1.svg[scaledwidth="100%"]
...
to be restarted, or an hardware error causes the indexserver to restart locally.
...
Use SAP tools like {saphana} Cockpit or *hdbnsutil*. Please make sure to use
...
SAPHanaSrMultiTarget.py(7), SAPHanaSR-manageAttr(8) and susChkSrv.py(7).
...
Please also make sure all systems have indentical package versions.
...
NOTE: Please note that the new srHook script "SAPHanaSrMultiTarget.py" necessary for multi-target setup is not available in the installation ISO media and only available in update channels of SLES for SAP 15 SP3 or earlier. So, for a correctly working multi-target setup a full system patching is mandatory. From SLES for SAP 15 SP4 onwards the "SAPHanaSrMultiTarget.py" will be included in the the ISO.
...
takeover. For this a configuration 'register_secondaries_on_takeover=true' needs
to be added in the system_replication block of the global.ini file. This
...
=== Adapting {saphana} nameserver configuration

We need  change the 'nameserver.ini' for the two sites controlled by the Linux
cluster. This change ensures that there is no second master nameserver candidate
as this is an ERP style setup. By default during the {saphana} installation the
second node at each site will be configured as the master nameserver candidate.
...
Please refer to {saphana} documentation for details.
// TODO PRIO2: link to SAP docu
// TODO PRIO1: re-registering neccessary for cativating nameserver changes?
// TODO PRIO2: detailled command example for above change
...
It is again reminded that the srHook script "susChkSrvpy" is not available in
the installation ISO media. It is only available in update channels of {sles4sap}
15 SP4 or earlier. So, for a correctly working setup a full system patching is
mandatory after registering the system to SCC, RMT or SUSE Manager.
From {sles4sap} 15 SP5 onwards the "SAPHanaSrMultiTarget.py" will be included in the ISO.
...
More complete checks wil be done later, once the Linux cluster is up and running.

==== Checking for SAPHanaSrMultiTarget.py

Check if {saphana} has initialised the SAPHanaSrMultiTarget.py hook script for
the srConnectionChanged events. Check the HANA nameserver trace files as well
as the specific hook script trace file. Do this on both site´s master nameserver.
See also manual page SAPHanaSrMultiTarget.py(7).
----
~> cdtrace
...
Check if {saphana} has initialised the susChkSrv.py hook script for
the srServiceStateChanged events. Check the HANA nameserver trace files as well
as the specific hook script trace file. Do this on all nodes.
See also manual page susChkSrv.py(7).
----
~> cdtrace
~> grep HADR.*load.*susChkSrv nameserver_*.trc | tail -3
~> grep susChkSrv.init nameserver_suschksrv.trc | tail -3
----
// TODO PRIO2: output example
...
script, check for the correct behaviour. Do this on all nodes.
See also manual page susChkSrv.py(7).
----
~> cdtrace
...
In case of an hardware watchdog a desired action is predefined after the timeout
...
HANA secondary master name server.
Replace the bold string with your instance number, {saphana} system ID and the 
virtual IP address. 

NOTE: In scale-out this works only for a {saphana} topology with exactly one
...
For more information, please read the related blog article
...
Once the existing or newly created part of scale-out performance optimized cluster is ready then we can follow the following steps to configure the third {saphana} site into multi-target.

==== Install {saphana} at the third site

You can follow the steps described for the secondary site, as in chapter
<<SAPHanaInst>>.
Just make sure to use the identifiers for the third site, as defined in the
parameter sheet.
...
* Kill the {saphana} instance of the worker node but not the master nameserver of the primary. 
...
* Kill hdbindexserver processs on the master nameserver node of the primary.
...
* Re-join of a previously power-off worker node.

Expected Observation: Cluster notices re-join of previously power-off node. The HANA instance gets started. System replication gets back to okay (srHook SOK).
...
* Kill hdbindexserver processs on the master nameserver node of the secondary.
...
Expected Observation: Cluster notices the nodes that are back. HANA instance get started. After a while, system replication for secondary site gets okay (srHook SOK).
