...
{sles4sap} is optimized in various
...
Multiple installation guides are already existing, with different reasons why to set up the server in a certain way. 
...
After the _ssh-keygeer_, the _ssh-keygen_ is set up; you will have two new files under _/root/.ssh/_ .
...
overview of all running {saphana} processes. The output of _HDB_ info should look like
...
The file list in _/hana/shared/myHooks_ should now also contain a file with
extention **pyc**.
...
In case the softdog module is used the following aciton can be performed:
...
As user {refsidadm} create a file named for _g.txt_.
...
Enter the following to :crm-cs.txt_:
...
The tests described in this sectoin are checking the reaction on several failures of the secondary site.
