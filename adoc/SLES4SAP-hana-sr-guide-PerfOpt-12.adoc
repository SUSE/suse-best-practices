...
{sap} did improve the interfaces between {saphana} and extrenal software such as
cluster frameworks. These improvements also include the implementaion of {saphana}
call outs in case of special events such as status changes for services or system replication
channels. These calls outs are also called HA/DR providers. Thes can be used by
...
{saphana} hook described in this document alows to inform the cluster immediately,
...
„SYNCMEM“. „ASYNC“ is also a posible replication mode but not supported with
...
More specific sudoers enties to meet a high security level. All Cmnd_Alias entries must be each defined as a single line entry.
In the following example the lines might include a line-break forced by document formatting. In our example we would have
four seperate lines with Cmnd_Alias entries, one line for the {refsidadm} user and one or more lines for comments.
...
could not longer acces the SBD (disk-less or disk-based).
