...
_"SUSE Linux Enterprise Server for SAP Applications"_ in AWS. Therefore, use a "SUSE Linux Enterprise Server for SAP" AMI which includes the SUSE Linux Enterprise High Availability Extension.
...
To operate correctly, the cluster requires specific AWS IAM privileges.
...
The policy shown below can be used by all SAP systems as the _“AWS Data Provider for SAP”_. You can have only one policy per AWS account.
...
* Select route table used by the subnets from one of your SAP EC2 instances and their application servers.
...
AWS "SUSE Linux Enterprise Server for SAP" AMIs already have all High Availability Extension packages installed.
...
resources and cluster bootstrap parameters. Read our product
