...
// = {title}
= Microsoft SQL Server on SUSE Linux Enterprise: Getting Started
...
to migrate to Linux or already run a primarily Linux environment and are adding an SQL Server requirement, 
but want to retain using tools that run only on Linux servers. 
...
It is meant to be agnostic of underlying infrastructure excepting the nuance of server registration discussed in <<server_registration>>.
