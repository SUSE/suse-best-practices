...
The described cluster solution could be used for {sapReg} S/4 HANA and for
{sapReg} {sapNW}.
...
practice "Simple Stack" available on the SUSE Best Practices documentation page (see section "Additional
...
-	Created subnets in the two AZs which can host the two nodes of a SUSE Linux Enterprise High Availability Extension
   cluster
...
Use the check list in the appendix to make a note of all information needed
...
The cluster agents will need to be able to identify the EC2 instances in the
...
transparent Internet access will need to provide an HTTP/HTTPS proxy.
...
zone. We will refer to this name as "efs-name" when we will need to mount the
file systems.
...
. Setup NTP (best with YaST). Use AWS time service at 169.254.169.123 which
...
This guide will not duplicate the official HADR documentation. The following procedure will describe the
key points which you have to take care of.
...
Next step installing the SRS software with a response file and enter the following command as user syb<sid>:
...
NOTE: If there is an error message like this:
Clean up environment.
Environment cleaned up.
Error: Fail to connect to "PRIM" site SAP ASE at "<hostname>:4901".  Check the host name/port number are correct and server is running.

and it seams the network connection should be possible. Try to add a new line in the _/sybase/<SID>/interfaces_ file for the <SID> section with the IP address.
...
NOTE: In some circumstances the installation is not successful. It could help to set up the primary system again and install the companion afterward.

NOTE: If the system is reinstalled and the companion system reports *Missing read/write permissions* for this directory _/tmp/.SQLAnywhere_ check the permission on both node. After a change for the ownership was needed, run the setup again on both nodes. Start with the *Primary*.
...
Checking the log file is also a good point to validate the status.
...
Details of the all the FM parameters can be found in the *SAP ASE HA DR User Guide*. Those highlighted
in bold are of interest for the setup on AWS. Since the FM is installed with the ASCS which can
failover from Node 1 to Node 2 if Node 1 goes down, the parameters _ha/syb/cluster_fmhost1_ and
_ha/syb/cluster_fmhost2_ provide the physical host names of both of the nodes where the FM can potentially run.
...
The application server environment should be adapted in case the DB has been switch (takeover). On each host which is providing a dialog server (PAS; AAS) the _.dbenv.sh_ and or _.dbenv.csh_ file needs to be extended.

.Checking the status of the database situation
==========

Check the status and locate the primary site.
_As user _{mysapadm}_ on the ASCS host
[subs="specialchars,attributes"]
----
...
_As user _root_ on the database host
[subs="specialchars,attributes"]
----
...
_As user _syb<sid>_ on the database host
[subs="specialchars,attributes"]
----
...
* starting companion database + replication server
* starting primary database + replication server
* change cluster maintenance mode to false
** starting ASCS with FM (automatic)
** starting ERS (automatic)
* starting PAS and AAS instances
...
_As user _root_ on companion database host
[subs="specialchars,attributes"]
----
...
_As user _root_ on primary database host
[subs="specialchars,attributes"]
----
# /usr/sap/hostctrl/exe/saphostctrl -function StartDatabase -dbname <SID> -dbtype syb
# /usr/sap/hostctrl/exe/saphostctrl -function StartDatabase -dbname <SID>_REP -dbtype syb
...
_As user _root_ on one of the Pacemaker host for ASCS and ERS
[subs="specialchars,attributes"]
----
# crm configure property maintenance-mode=false
----

_As user _<sid>adm_ on the host for PAS or AAS
[subs="specialchars,attributes"]
----
# sapcontrol -nr <instance number> -function StartSystem
----

NOTE: If the system should start one by one use the command _sapcontrol -nr <instance number> -function Start_. The direction must be: ASCS; ERS; PAS; AAS.

.In general these steps are important to *stop* the system:

* set cluster maintenance mode to _true_
* stopping PAS and AAS instances
* stopping ASCS with FM
* stopping ERS
* stopping primary database + replication server
* stopping companion database + replication server
...
_As user _<sid>adm_ on one of the Pacemaker host for ASCS and ERS or PAS / AAS
[subs="specialchars,attributes"]
----
# sapcontrol -nr <instance number> -function StopSystem
----

NOTE: If the system should stopped one by one use the command _sapcontrol -nr <instance number> -function Stop_ on each instance host. The direction must be: AAS; PAS; ASCS; ERS.
...
IMPORTANT: The Pacemaker controlled server must be stop in a proper way too. Depending on the stonith methode which is implemented different procedures are available.

_As user _root_ on one cluster node
[subs="specialchars,attributes"]
----
# crm cluster run "crm cluster stop"
----

_As user _root_ on each node
[subs="specialchars,attributes"]
----
# reboot
## or
...
Important for each High Availablity solution is an extensive testing procedure. That takes care that the solution is working as expected in case of a failure.

.Triggering a Database failover and monitor if FM is working
================
...
############ host is coming back online ################
    ...
    2019 09/13 11:18:45.589 (3290) db status is:
     DB_OK.
...
.Triggering a  FM failure
================

Killing the FM process more than 5 times will bring pacemaker in action. Up to 5 time the saphostagent will take care of the SAP process. If this failcount is reached in a specific time windows the service will not be restart.

_As user _{mysapadm}_
[subs="specialchars,attributes"]
----
# pkill -9 sybdbfm
...
Now pacemaker will restart the FM instance localy first.
_As user _root_
[subs="specialchars,attributes"]
----
# crm_mon -1rfn
