...
= Using SUSE Automation to Deploy an SAP HANA Cluster on {cloud} Cloud Platform: Getting Started


== About the guide

This document will walk you through the deployment of a simple two-node SAP HANA HA Cluster using the SUSE SAP Automation Project and operating on {cloud}.
...
ifeval::[ "{cloud}" == "AWS" ]
image::TRD_SLES-SAP-HA-automation-quickstart-cloud-aws-automation-architecture.jpg[title=AWS Automation Architecture,%width=470]
endif::[]
...
. The compressed archives (SAR and RAR) provide simple copy to the cloud, but a bit longer install times due to extracting it during the process
. The uncompressed / extracted media are the fastest install, but there need a huge number of files copied to the cloud share, which also takes a lot of time in forehand as preparation.

// we need to agree on this and unifiy the doc
...
With the correct entitlement, SAP HANA media can be downloaded from the SAP Web site at
...
// this need to be agreed on:
// For illustration, the final extracted directory name is `{hana_archive_version}`.

It's a good practice to have the SAP Media versioned on cloud share in order to build a library for automatic installs and (re)deploments.
So think about you SAP media structure.

As a example here how a full SAP Application media tree for a S/4 install would look like
----
../s4hana1809
      ├SWPM_CD
...
In the further steps we only use as example only a HANA install download.
...
----
$ cd ~
$ cp <GCP Service Account Key> ha-sap-terraform-deployments/gcp
----
endif::[]


Then, generate SSH key pairs to allow for accessing the GCP SAP HANA instances:
NOTE: If SSH keys already exist, the step can be skipped.
----
...
ifeval::[ "{cloud}" == "GCP" ]
----
$ cd ~/ha-sap-terraform-deployments/gcp
----
endif::[]
...
ifeval::[ "{cloud}" == "Azure" ]
Enable the Bastion Host Server creation parameter `bastion_enabled`:
----
...
The default is set to some standart types, and you only need to enable and change the variable if you want other sizes
...
----
hana_inst_master = "//YOUR_STORAGE_ACCOUNT_NAME.file.core.windows.net/mysapmedia/HANA2/{hana_archive_version}"
----
endif::[]
