...
Virtual Machine:: is an emulation of a computer. (abbr.: VM)
...
* run {sles4sap} 15 SP4 or later.
* be a {sles} supported VM guest as per Section 7.1 "Supported VM Guests" of the https://documentation.suse.com/sles/15-SP4/single-html/SLES-virtualization/#book-virtualization[SUSE Virtualization Guide].
...
To avoid that, and achieve better and more consistent the performance, the CPUs should not be allowed to switch those power saving modes (known as C-states) and stay in normal operation mode all the time.
It therefore is recommended that only the state C0 is used.
...
*Huge pages*
----
default_hugepagesz=1 GiB hugepagesz=<1 GiB hugepages=number of hugepages>
----
...
The VM needs to have the `vm-dump-metrics` package installed, which dumps the metrics provided by the `vhostmd` service running on the hypervisor. This enables SAP HANA to collect data about the hypervisor. 
