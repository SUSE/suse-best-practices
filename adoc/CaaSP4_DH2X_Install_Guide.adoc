...
* Add SUSE Linux Enterprise  High Availability 15 SP1 repository (in case of SUSE Linux Enterprise Server for SAP Applications, this automatically added)
...
The following sections contain the description of the preparation and installation of SAP Data Hub 2 on a SUSE CaaS Platform 4 cluster.


=== Preparing the SAP Data Hub 2 installation

These are the steps that are needed to install SAP Data Hub on SUSE CaaS Platform successfully:
...
This document focuses on the Maintenance Planner and SL Plugin installation method.
...
SAP recommends the installation using the Maintenance Planner with SL Plugin (mpsl method) to install SAP Data Hub 2.

This is a Web-based installation method recommended by SAP which offers the option to send analytic data and feedback to SAP. 
All the necessary prerequisites have been satisfied by applying all the steps described above.

NOTE: You have to install the latest SAP Host Agent on the management host. 
...
=== Installating SAP Data Hub 2 using the SL Plugin (mpfree method)
...
Now you can sinmply clone the Portus repository. Adapt the `.env` and the `nginx` configuration to your naming convention.
...
To avoid dealing with Docker insecure registry configuration, you need to add SSL to your setup.
...
Next, all aou have to do is to make the servers aware of this certificate:
...
On all servers that need to interact with the `docker-registry`, perform the following:
...
If you plan to use SUSE Enterprise Storage not only for your Kubernetes dynamic storage class, but also for your Kubernetes Control plan (virtualised or not),
...
In the following example, we will build a four-nodes (1 admin + 3 OSD) Ceph cluster.

Before you start:

* Obtain registration codes for SUSE Linux Enterprise Server 12 SP3 and SUSE Enterprise Storage from https://scc.suse.com,
or have a SMT/RMT properly set up and already mirroring theses products:

** link:https://scc.suse.com[SCC]
+
image::scc-sle.png[scc-sle,scaledwidth=95%]


** link:https://documentation.suse.com/sles/12-SP4/html/SLES-all/book-smt.html[SMT]
+
image::scc-ses.png[scc-ses,scaledwidth=95%]
...
In the following example, each node is powered by 16GB of RAM, 4 VCPU, 40 GB for the root disk, 4 x 20GB OSDB disk.
...
Verify that the drive we will allocate for SUSE Entreprise Storage OSDs are clean by wiping them.
...
In our example, it contains our `salt-master` FQDN : `master_minion: ses55-admin.suse-sap.net`
...
image::ceph-stage-0.png[ceph-stage-0.png,scaledwidth=95%]
...
image::ceph-stage-1.png[ceph-stage-1.png,scaledwidth=95%]
...
image::ceph-stage-2.png[ceph-stage-2.png,scaledwidth=95%]
...
image::ceph-stage-3.png[ceph-stage-3.png,scaledwidth=95%]
...
image::ceph-health.png[ceph-health.png,scaledwidth=95%]
...
image::ceph-stage-4.png[ceph-stage-4.png,scaledwidth=95%]
...
image::openattic-pool.png[openattic-pool.png,scaledwidth=95%]

Then provide access to this pool through an RBD device:

image::openattic-rbd.png[openattic-rbd.png,scaledwidth=95%]

You can now go to Section 4.1.2 of this document and follow the prerequisites for a SUSE CaaS Platform 3 Cluster.
