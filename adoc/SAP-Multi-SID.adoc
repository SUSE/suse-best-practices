...
:docCostOpt: Setting up a SAP HANA SR Cost Optimized Infrastructure
...
= {sap} {sapS4} and {sapNW} Multi - SID Cluster Guide
...
Other architectues like the {saphana} performance optimized scenario will follow.
...
In the following common {hana} scenarios are described. In addition you will find a note, if the
...
+
NOTE: The performance optimized scenario could be implemeted also for multi SID setups,
if it is guaranteed that all system replication pairs are handled by the cluster seperately.
It is not allowed to force an {hana} system to process a takeover only because an other one is taking over.
...
The reason is that in that case a second memory setup for the primary systems would be needed
when ever one of the primarys would need to be moved to the secondary node. Otherwise the 
removed memory limitation would tell the {hana} to consume all available memory. Currently there
is no globaly definded space where to set such parameters to be used after a takeover.
...
+
NOTE: This type of scenarios is from cluster perspective very similar to the
performance optimized scenario. As long as only A and B are driven by the cluster
this scenario is also expected to work with multi SID setups. However so far this
has not been tested in the labs. Support for this scenario is expected to be added in the
future.
...
scenarios.
+
NOTE: Multi-tenancy has no direct side effect to multi SID setups. So multi-tenancy is
...
- Each {sap} system (SID) must be independent from each other
- ENSA2 loadbalancing (under observation)
...
Repeat the step for preparing the hosts and infrastructure.
...
The installation process itself is similar to the first setup. The major different is the ERS
...
** e.g. for {sid2}: __/sapmnt/{sid2}/profile/{sid2}_ASCS{inst-ascs2}_{v-ascs2host}__
** e.g. for {sid2}: __/sapmnt/{sid2}/profile/{sid2}_ERS{inst-ers2}_{v-ers2host}__
...
.Configuration Example for a 3rd SID e.g. {sid3}
====
...
.Configuration Example for SID a 4th SID and following e.g. {sid4}
====
...
_As user_ *root* set cluster into maintenance and load the configuration into the cluster
...
You should get back the prompt only. No further messages should be shown. If there are messages there is maybe something wrong in the configuration file.
...
_As user_ *root* release cluster from maintenance and check that the new cluster resources became active
//[subs="specialchars,attributes,verbatim,quotes"]
//======
...
Each group consists of three resources, rsc_ip, rsc_fs and rsc_sap. The resource rsc_ip_hawk is the optional
...
=== Example ENSA2 in a Two Node Cluster
//[[2nensa2]]
As known from the setup based on our *Best Practice* for {sapS4}
(https://documentation.suse.com/sbp/all -> {sap} {sapS4} - Enqueue Replication 2 High Availability Cluster - Setup Guide)
the cluster integration can be done with a configuration file. Create one configuration file for each SID
and load them one by one into the cluster. The following description will help to do the steps in the right direction.
...
====
=== Example ENSA1 in a Multi Node Cluster
This base setup is already documented in our *Best Practice* for {sapNW}
(https://documentation.suse.com/sbp/all -> {sap} {sapS4} - SAP NetWeaver High Availability Cluster 7.40 - Setup Guide - Best Practice Guide).
...
=== Example ENSA2 in a Multi Node Cluster
This base setup is already documented in our *Best Practice* for {sapS4}
(https://documentation.suse.com/sbp/all -> {sap} {sapS4} - Enqueue Replication 2 High Availability Cluster - Best Practice Guide).
The OS and node preparation must be done for each future cluster member:
...
As known from the setup based on our *Best Practice* for {sapNW} and {sapS4}
(https://documentation.suse.com/sbp/all -> {sapNW} High Availability Cluster 7.40 - Setup Guide)
(https://documentation.suse.com/sbp/all -> {sap} {sapS4} - Enqueue Replication 2 High Availability Cluster - Setup Guide)
the cluster integration can be done with a configuration file. Create one configuration file for each SID
and load them one by one into the cluster. 

We expect the cluster is already up and running and still run one ASCS / ERS configuration based on the group concept.

NOTE: Please follow and combine the chapter "Example ENSA1 in a Two Node Cluster" and "Example ENSA2 in a Two Node Cluster".
...
The setup here is simular to that what is done in the <<Example ENSA1 and ENSA2 in a Two Node Cluster>> but only with more nodes in the cluster.

After the basic cluster part is done the procedure is the same as for two node cluster setup. Please check this chapter for further instructions:
<<Example ENSA1 and ENSA2 in a Two Node Cluster>>.
...
=== Workload balacing of Central Services

//TODO: loadbalancing

NOTE: Under observation.

==== Automatic loadbalancing by pacemaker
There is no additional task necessary.

==== Manual loadbalancing by pacemaker
...
NOTE: Under observation. Please send an e-mail to saphana@suse.com if this might be interesting for you.
