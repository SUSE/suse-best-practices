...
*Follow this {DocumentName} - {sles4sap} {slesProdVersion}
        ...
        to create a supported SAP HANA on KVM configuration. {sles4sap}
        must be used for both Hypervisor and Guest.*
...
//TODO variables don't work inside tables, e.g. :sles4sapAbbr:
...
| 
- Hypervisor: SLES for SAP 12 SP2 - Guest: SLES for SAP 12 SP1 onwards - Size: max.
4 socket
...
|
- Hypervisor: SLES for SAP 15 SP2 - Guest: SLES for SAP 15 SP2 onwards - Size: max.
4 socket
// <superscript>1</superscript>
, 3TiB RAM
|
...
// <superscript>1</superscript>
 Maximum 4 sockets using Intel standard chipsets on a single system board, for example Lenovo* x3850, Fujitsu* rx4770 etc.
...
* SAP Note tbd "`SAP HANA on KVM included with SLES for SAP Applications 15 SP2 in production`" (https://launchpad.support.sap.com/#/notes/tbd)
...
* Be a SUSE Linux Enterprise Server Supported VM Guest as per Section 7.1 "`Supported VM Guests`" of the SUSE Virtualization Guide (https://documentation.suse.com/sles/15-SP2/html/SLES-all/cha-virt-support.html#virt-support-guests).
