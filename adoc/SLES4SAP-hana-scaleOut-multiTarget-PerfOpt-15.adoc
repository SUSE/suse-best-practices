...
.Simplified NFS structure of a {saphana} multi-target system replication cluster
...
devices are accessed from all five cluster nodes. But the block devices are
backed by storage outside the cluster.

.Simplified SBD structure of a {saphana} multi-target system replication cluster
