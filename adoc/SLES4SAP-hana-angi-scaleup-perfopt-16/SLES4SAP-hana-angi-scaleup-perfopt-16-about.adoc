...
{SUSE} supports customers by offering {sles4sap},
...
solutions for customers to ensure high availability for {HANA} data bases.

==== Abstract

This guide describes planning, setup, and basic testing of {sles4sap} based on
the high availability solution scenario "{SAPHANA} {usecase}".
...
See <<cha.hana-sr.setups>> for details.

NOTE: In this guide the software package `{SAPHanaSR}` is used. This package replaces
the two packages `SAPHanaSR` and `SAPHanaSR-ScaleOut`. Therefore, deploy new deployment 
with {SAPHanaSR} only. For upgrading existing clusters to {SAPHanaSR},
read the blog article at https://www.suse.com/c/how-to-upgrade-to-saphanasr-angi/ .

==== The concept of the performance optimized scenario

This document describes the *performance optimized* scenario [_A' => B_].
An overview of available scenarios is decribed in our white paper. 
...
In the performance optimized scenario, an {HANA} RDBMS site A synchronizes
with an {HANA} RDBMS site B on a second node. As the {HANA} RDBMS on the second
node is configured to pre-load the tables, the takeover time is typically very
short.

One big advantage of the performance optimized scenario of {HANA} is the
option to allow read access on the secondary database site. To support
...
The cluster allows a takeover to the secondary site only if the {HANA}
system replication was in sync until the service of the primary site
was lost. This ensures that the last commits processed on the primary site are
...
SAPHanaSR(7) and ocf_suse_{SAPHanaRA}(7) for details on all supported parameters
and features.

IMPORTANT: The solution is not designed to manually 'migrate' the primary or
secondary instance using the HAWK Web console or any cluster client commands. In the
...
Find numerous whitepapers, best practices, setup guides, and
