...
Hardening Settings for SAP HANA systems
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
IMPORTANT: The measures in this chapter are described for the x86 architecture only, but might apply for the Power architecture with changes as well. +
Also the graphical user interface is not covered. Running a GUI on a secure server should be avoided. +
Please also read the "Security Guide" and the "Hardening Guide" for additional measures: https://documentation.suse.com/ +
...
+
TIP: It is also possible to allow a root login only with key authentication. Please read the man page of `sshd_config` for different `PermitRootLogin` values.
...
More details about seccheck can be found in `/usr/share/doc/packages/seccheck/README` or  https://www.suse.com/documentation/sles-12/singlehtml/book_hardening/book_hardening.html#sec.sec_prot.general.seccheck.
+
...
Set default inactive time to 1 day
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Description::
By default, there is no timeout for idle user sessions. We recommend to set the timeout to one day. + 
+
It is possible to set a timeout for most shells to terminate automatically, usually by a variable. 
You can set the timeout for login shells only or for all interactive shells. 
If set for all interactive shells the timeout is individual for each instance. +
Keep this in mind when cascade shell instances. 
+
The following table contains details for all shells shipped with SLES for SAP Applications:
+
[cols="1m,1m,^1m,^1,1m,1m,1m", options="header"]
|===    
| package | incarnation | shell variable | unit | Setting variable readonly ^a| 
config file +
(only login shell) ^a| 
config file +
(every shell)
...
Procedure::
Set the timeout variable readonly in the appropriate configuration file. +
This is an example for `bash`, `sh`, `ksh`, `lksh`, `mksh`, `pdksh` and `zsh` to only have login shells (no sub-shells) timeout after one day.
+
...
#
# Timeout in seconds until the bash session is terminated
# in case of inactivity.
# 24h = 86400 sec
readonly TMOUT=86400
----

NOTE: You can export the variable to have sub-shells, which support the variable, also time out. +
Each sub-shell has its own counter, which means that after terminating a sub-shell the timeout counter of the parent shell would start. Depending of the amout of sub-shells the timeout value is multiplied. 


TIP: There is a risk of breaking out of the shell initialization to avoid setting the timeout. +
As an alternative you can set the variables in a shell script called by `pam_exec.so`. 


Impact::
Long running idle user sessions, mostly forgotten, are terminated after 1 day. +
+
We recommend to use `screen` in order detach sessions before logging out. Screen sessions are not terminated and can be re-attached whenever it is required. Active session can be locked without logging out (read about `C-a x` / `lockscreen` in `man screen` for details).
+
Priority: medium
...
If your corporate policy requires to setup password failure counts for users, please refer to the 'SUSE Linux Enterprise Server Security and Hardening Guide, section: 3.29.3. Locking User Accounts After Too Many Login Failures'
...
Procedure::
. Ensure that `/etc/pam.d/login` contains the 'pam_securetty' module in the 'auth' block:
+
----
...
----
+
. Comment out or remove all tty's in the file `/etc/securetty` except of tty1.
+
----
#
...
The chapter 'Configuring HANA-Firewall' in  the 'SLES for SAP Applications Guide' (https://www.suse.com/documentation/sles-for-sap-12/book_s4s/data/sec_s4s_configure_firewall-hana.html) describes the SAP HANA Firewall YaST module and lists some advice regarding multi-tenant HANA databases.
...
. Add all installed SAP HANA systems and instances to the parameter `HANA_SYSTEMS` as a space separated list. Use the format `<sid><instance-nr>`, i.e. HDB00
...
----
+
WARNING: Open the SSH port on all interfaces should be done only temporarily during testing. In the final configuration `OPEN_ALL_SSH` should be set to "no".

'Interfaces' section
++++++++++++++++++++
`INTERFACE` and `INTERFACE SERVICES` parameters map services to network interfaces. `INTERFACE` parameters have to be in the format `INTERFACE_<0-n>` and contain names of valid network interfaces, like 'eth0' or 'bond0'.
...
Service names can be all services defined in directory `/etc/hana-firewall.d`
...
The SAP HANA firewall service definitions are stored in the directory `/etc/hana-firewall.d`. Each file (in capital letters) defines one service. The service name equals to the file name and can immediately be used in the 'Interfaces' section of the main configuration.
Each service file currently requires two parameters (TCP, UDP) that specify the TCP and UDP ports and/or port-ranges. Ports and port ranges have to entered as a space separated list. Port ranges are defined in the format: `<start port>:<end port>`, i.e. `10000:20000`.
...
|`NFS_SERVER` a|
In order to allow access to an NFS server, you have also to set fixed ports for certain NFS services in `/etc/sysconfig/nfs`.
...
First, you should simulate the start with the 'dry-run' option. This option just prints the iptables commands to STDOUT without of actually executing the iptables commands.
----
hana-firewall dry-run
----
...
IMPORTANT: Please do not forget to set the global parameter `OPEN_ALL_SSH` to 'no' and to  configure the SSH service for the appropriate interfaces.
...
IMPORTANT: Since SLES12 HANA firewall is part of SuSEfirewall2 and will start it automatically, it does not matter whether the SuSEfirewall2 service is enabled or not.
Please also be aware that the resulting iptables rules are a combination from *BOTH*, HANA firewall and SuSEfirewall2!
...
SAP HANA is a very complex application, shipped in different versions and having many additional components available. This makes it difficult to follow the concept of installing only a minimal set of packages.
Therefore, the current approach to minimal package selection is to use the SLES installation patterns 'Base System' + 'Minimal System' and  add the 'SAP HANA Server Base' optionally.
...
TIP: If you want to enable X11 forwarding for remote ssh connections (`ssh -X` or `ssh -Y`), the additional package xauth is required. X11 forwarding via ssh is useful  i.e. when using the graphical HANA installer.
...
This command lists the available Software repositories of a SUSE Linux Enterprise Server instance. The output should show the update channel  for the particular Service Pack of  SUSE Linux Enterprise Server 12. On SUSE Linux Enterprise Server for SAP Applications 12, also the update channels for the Service Pack of  SLES for SAP-Applications and the for the HA extension should be present.
...
If you have any questions, comments or feedback on this document, please don not hesitate to contact us under the following email address: saphana@suse.com.
