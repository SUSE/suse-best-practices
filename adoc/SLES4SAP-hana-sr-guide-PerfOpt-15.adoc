...
Defines the timeout, in seconds, the watchdog will wait before paniking the node if no-one tickles it.

This parameter must match your storage environment. If you expect your storage could hang for 30s, then
this parameter must be set to a higher value. On the other hand it should be usually <= 0.5 times the SBD msgwait.

The parameter must also be aligned with the pacameker cluster property stonith-watchdog-timeout.
The property stonith-watchdog-timeout must be set >= 1.2 times the value of SBD_WATCHDOG_TIMEOUT.
